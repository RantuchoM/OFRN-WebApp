-- Sincronizar `entrada_programa.nombre` con `programas.nombre_gira` (no nomenclador/mes_letra).

update public.entrada_programa ep
set nombre = coalesce(nullif(trim(p.nombre_gira), ''), ep.nombre)
from public.programas p
where ep.slug_publico = 'ofrn-programa-' || p.id::text
  and nullif(trim(p.nombre_gira), '') is not null
  and (
    trim(coalesce(ep.nombre, '')) = ''
    or trim(ep.nombre) = trim(coalesce(p.nomenclador, ''))
    or trim(ep.nombre) = trim(coalesce(p.mes_letra, ''))
  );
