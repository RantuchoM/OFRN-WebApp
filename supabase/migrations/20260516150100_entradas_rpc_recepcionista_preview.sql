-- RPC: cierre de reservas, preview/validación con recepcionista, códigos legacy
...
-- QR de reserva/entrada (ENTR-RSV-uuid / ENTR-TCK-uuid), no confundir con codigo_reserva
create or replace function public.entrada_es_token_qr_ofrn(p_input text)
returns boolean
language sql
immutable
as $$
  select upper(trim(coalesce(p_input, ''))) ~ '^ENTR-(RSV|TCK)-[A-F0-9]{32}$';
$$;

create or replace function public.entrada_es_codigo_reserva_texto(p_input text)
returns boolean
language sql
immutable
as $$
  select case
    when public.entrada_es_token_qr_ofrn(p_input) then false
    when trim(coalesce(p_input, '')) ~ '^[0-9]{10}$' then true
    when upper(trim(coalesce(p_input, ''))) ~ '^ENTR-C[0-9]+-[0-9]{10}$' then true
    when upper(trim(coalesce(p_input, ''))) ~ '^ENT-RSV(?:-[A-Z0-9]+)*-[0-9]{10}$' then true
    when upper(trim(coalesce(p_input, ''))) ~ '^ENT-[A-Z0-9]{8}$' then true
    else false
  end;
$$;
...
begin
  ...
  if v_concierto.limite_cierre_reservas_at is not null and now() > v_concierto.limite_cierre_reservas_at then
    raise exception 'El plazo para reservar entradas para este concierto ya cerró';
  end if;
...
-- Basado en 20260426080000 + recepcionista y código legacy completo
