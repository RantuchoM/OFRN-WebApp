-- Recepción: deshacer ingresos recientes + ordenes ingresadas en respuesta de consumo QR

create or replace function public.entrada_recepcion_revertir_ingresos(
  p_reserva_id bigint,
  p_ordenes integer[]
)
returns jsonb
...
begin
  ...
  if coalesce(cardinality(p_ordenes), 0) = 0 then
    raise exception 'Indicá al menos una plaza para deshacer.';
  end if;
  ...
  if v_revertidas = 0 then
    raise exception 'Ninguna plaza ingresada coincide con la selección.';
  end if;
  if v_revertidas <> cardinality(p_ordenes) then
    raise exception 'Algunas plazas no están en estado ingresada.';
  end if;

  insert into public.entrada_ingreso_evento(
    tipo_qr, reserva_id, concierto_id, resultado, detalle, scanner_user_id
  )
  values (
    'reserva',
    p_reserva_id,
    ...
    'ok',
    'Recepción: deshecho ingreso plaza(s) ' || array_to_string(p_ordenes, ','),
    auth.uid()
  );
...
grant execute on function public.entrada_recepcion_revertir_ingresos(bigint, integer[]) to authenticated;
