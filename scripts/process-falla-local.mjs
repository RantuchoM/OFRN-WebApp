/**
 * Divide PDFs IMSLP combinados, recorta portadas y renombra particellas Falla.
 */
...
function resolveWorkDir() {
  const target = join(PARA_ACOMODAR_ROOT, FALLA_WORK.targetFolder);
  if (existsSync(target)) return target;
  const src = join(PARA_ACOMODAR_ROOT, FALLA_WORK.sourceFolder);
  if (existsSync(src)) {
    if (!dryRun) {
      renameFolderIfNeeded(
        PARA_ACOMODAR_ROOT,
        FALLA_WORK.sourceFolder,
        FALLA_WORK.targetFolder,
        false,
      );
    }
    return target;
  ...
  const hit = readdirSync(PARA_ACOMODAR_ROOT, { withFileTypes: true })
    .filter((d) => d.isDirectory() && d.name.toLowerCase().startsWith("falla"))
    .map((d) => join(PARA_ACOMODAR_ROOT, d.name))[0];
  if (!hit) throw new Error("No se encuentra carpeta Falla en Para acomodar");
  return hit;
}
...
console.log(`Para acomodar / Falla: ${workDir}`);
console.log(dryRun ? "=== DRY RUN ===" : "=== APLICANDO ===");

const meta = {
  workNumber: FALLA_WORK.workNumber,
  workTitle: FALLA_WORK.titulo,
  composerTag: FALLA_WORK.composerTag,
};

console.log("\n--- Dividir combinados ---");
for (const split of FALLA_WORK.splits) runSplit(workDir, split);

console.log("\n--- Recortar portadas ---");
for (const crop of FALLA_WORK.crops) runCrop(workDir, crop);
