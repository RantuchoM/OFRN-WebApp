/**
 * Sync Orquesta particellas (Pärt lullabies + Vater unser + Händel Messiah)
 * → BD + gira 130 Navidad Coral (bloque Repertorio id=100).
 *
 *   node scripts/generate-orquesta-particellas-sync.mjs
 */
...
function isCelloBassCombined(fileName) {
  ...
  return (
    /violoncello y contrabajo/.test(n) ||
    /cellos?\s*-?\s*basses?/.test(n)
  );
}
...
async function discoverDriveFolderId(work) {
  ...
  const needle = foldName(work.targetFolder);
  const hit = files.find((f) => foldName(f.name) === needle);
  return hit?.id || null;
}
...
function inferInstrument(fileName, work) {
  ...
  const hit = (work.renames || []).find((r) => {
    ...
    return canonical === n || new RegExp(r.re, "i").test(n);
  });
...
function buildGiraSql(titles) {
  const titlesSql = titles.map((t) => `    '${sqlEscape(t)}'`).join(",\n");
  return `-- Gira ${GIRA_130.id_programa} "${GIRA_130.nombre_gira}" — Pärt + Messiah
-- Bloque existente "${GIRA_130.bloque}" id=${GIRA_130.id_repertorio}.
-- No agrega Suite Rutter (ya está obra 3196). Idempotente.

DO $$
DECLARE
  _id_programa bigint := ${GIRA_130.id_programa};
  _block_id bigint := ${GIRA_130.id_repertorio};
  _orden int;
  _id_obra bigint;
  _titulo text;
  _titles text[] := ARRAY[
...
async function main() {
  ...
  for (const work of ORQUESTA_PARTICELLAS_WORKS) {
    ...
    let duracion = work.skipYoutubeDuration
      ? null
      : fetched.duracion_segundos ?? null;
  ...
  writeSeed("supabase/seed_orquesta_particellas_sync.sql", insertSql, workData);
  const giraSql = buildGiraSql(workData.map((w) => w.titulo));
  writeFileSync("supabase/seed_gira_130_navidad_coral.sql", giraSql, "utf8");
  console.log("\nSeed: supabase/seed_orquesta_particellas_sync.sql");
  console.log("Gira: supabase/seed_gira_130_navidad_coral.sql");
  if (incompleteUrls) {
    console.log(
      "Nota: re-ejecutar generate tras sync Drive para completar url_archivo.",
    );
  }
}
