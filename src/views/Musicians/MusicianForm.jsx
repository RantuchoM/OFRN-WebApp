...
const CONDICION_OPCIONES = ["Estable", "Contratado", "Refuerzo", "Invitado"]; // NUEVO
...
export default function MusicianForm({ 
    ...
    // ... (El código de historial y efectos se mantiene igual)
        ...
        <div className={`p-4 rounded-xl border shadow-sm animate-in fade-in zoom-in-95 duration-200 ${isNew ? 'bg-indigo-50 border-indigo-200' : 'bg-white ring-2 ring-indigo-500 border-indigo-500 z-10 relative'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="md:col-span-1">
                    ...
                    <select className="w-full border border-slate-300 p-2 rounded focus:ring-2 focus:ring-indigo-500 outline-none bg-white" value={formData.id_instr} onChange={(e) => setFormData({...formData, id_instr: e.target.value})}>
                        <option value="">-- Sin Asignar --</option>{catalogoInstrumentos.map(inst => (<option key={inst.id} value={inst.id}>{inst.instrumento}</option>))}
                    </select>
                </div>

                {/* NUEVO CAMPO: CONDICIÓN */}
                <div className="md:col-span-1">
                ...
                {/* (Aquí irían Fecha Nacimiento, Alta, Baja, Contacto, etc. tal como ya los tenías) */}
