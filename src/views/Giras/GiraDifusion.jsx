...
const formatHeaderDates = (events) => {
  ...
  const dates = events.map(e => new Date(e.fecha + "T12:00:00")).sort((a, b) => a - b);
          ...
          {(timestamp || editorId) && value && (
            <div className="text-[10px] text-slate-400 mt-1 flex items-center gap-1">
              <span>
                ...
                {new Date(timestamp).toLocaleString("es-ES", { day: 'numeric', month: 'short', hour: '2-digit', minute:'2-digit'})}
              </span>
            </div>
          )}
        </div>
...
export default function GiraDifusion({ supabase, gira, onBack }) {
  ...
  const handleUpdateDifusion = async (fieldBaseName, value) => {
    try {
      ...
      const currentEditor = allIntegrantes.find(i => 
        i.id == user.id || 
        (user.email && (i.mail === user.email || i.email_google === user.email))
      );
  ...
  const getCitiesList = () => {
      const cities = [...new Set(filteredEvents.map(e => {
  ...
  const getComposerName = (obra) => {
      ...
      if (obra.compositores) {
          c = Array.isArray(obra.compositores) ? obra.compositores[0] : obra.compositores;
      }
        ...
        <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 mt-4">
                 ...
                 <div className="space-y-4">
                    {getGroupedEvents().length > 0 ? getGroupedEvents().map((group, idx) => (
                        <div key={idx} className="border-l-2 border-indigo-400 pl-3">
                            ...
                            <div className="mt-1">
                                ...
                                {group.localidad && (
                                    <p className="text-xs text-slate-500 uppercase">{group.localidad}</p>
                                )}
                            </div>
                        </div>
                    )) : (
                            ...
                            {rep.repertorio_obras?.filter(o => !o.excluir).map(obraItem => {
                                ...
                                return (
                                    <li key={obraItem.id} className="text-sm border-b border-slate-200 last:border-0 pb-1 flex flex-wrap gap-x-2">
                                            ...
                                            <span className="text-slate-600 italic">{obra.titulo}</span>
                                    </li>
                                );
                            })}
                        </React.Fragment>
                    ))}
                    {(!gira.programas_repertorios || gira.programas_repertorios.length === 0) && (
                        <p className="text-slate-400 italic text-sm">No hay repertorio cargado.</p>
                    )}
                </ul>
            </div>
        </section>
                                        ...
                                        <input 
                                            ...
                                            onBlur={(e) => {
                                                if(e.target.value !== details.link_bio) {
                                                    handleUpdateIntegrante(person.id_integrante, 'link_bio', e.target.value);
                                                }
                                            }}
                                        />
                                        {details.link_bio && (
                                            <a href={details.link_bio} target="_blank" rel="noreferrer" className="p-2 bg-indigo-50 rounded hover:bg-indigo-100 text-indigo-600"><IconLink size={14}/></a>
                                        )}
                                    </div>
                                </div>
                                <div className="space-y-1">
                                        ...
                                        <input 
                                            ...
                                            onBlur={(e) => {
                                                if(e.target.value !== details.link_foto_popup) {
                                                    handleUpdateIntegrante(person.id_integrante, 'link_foto_popup', e.target.value);
                                                }
                                            }}
                                        />
                                         {details.link_foto_popup && (
                                            <a href={details.link_foto_popup} target="_blank" rel="noreferrer" className="p-2 bg-indigo-50 rounded hover:bg-indigo-100 text-indigo-600"><IconLink size={14}/></a>
                                        )}
                                    </div>
                                </div>
                             </div>
