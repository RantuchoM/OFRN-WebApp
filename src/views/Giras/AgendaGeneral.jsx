...
import { 
    ...
    differenceInHours, 
    differenceInDays, 
    isPast, // NUEVO: Para chequear si la fecha límite pasó
} from "date-fns";
...
// --- LÓGICA DE FECHA LÍMITE (DEADLINE) ---
const getDeadlineStatus = (deadlineISO) => {
    // Si no hay fecha límite definida, no aplicamos restricciones de tiempo
    if (!deadlineISO) {
    ...
    if (isPast(deadline)) {
        return { status: 'CLOSED', message: 'Plazo finalizado' };
    }
    ...
    if (diffDays > 0) {
        return { status: 'OPEN', message: `Quedan ${diffDays} día(s) para confirmar` };
    }
    
    // Si quedan menos de 24 horas, mostramos horas
...
    } catch (err) {
      console.error("Error fetching agenda:", err); // Añadido para debugging
    } finally {
      setLoading(false);
    }
  };

  // --- FUNCIÓN PARA CONFIRMAR/CANCELAR COMIDA ---
...
        .from("eventos_asistencia")
        .upsert(payload, { onConflict: "id_evento, id_integrante" });
...
                      const isNonConvokedMeal = isMeal && !evt.is_convoked;
                      ...
                      // Usar el dato real de la BD para la fecha límite
                      const deadlineISO = evt.programas?.fecha_confirmacion_limite;
                      const deadlineStatus = isMeal && evt.is_convoked ? getDeadlineStatus(deadlineISO) : null;
                      
                      // showButtons solo si hay un plazo y está abierto
                      const showButtons = deadlineStatus?.status === 'OPEN';
...
                                    </span>
                                  ) : (
                                      // === LÓGICA DE CONFIRMACIÓN DE COMIDAS ===
                                    <>
...
                                          {/* MOSTRAR BOTONES SI ESTÁ ABIERTO */}
                                          {showButtons && (
                                            <div className="flex flex-col gap-1 mt-2">
                                                <span className="text-[10px] font-medium text-slate-500">{deadlineStatus.message}</span>
                                                <div className="flex gap-1.5">
                                                    <button 
                                                        onClick={() => toggleMealAttendance(evt.id, evt.mi_asistencia === 'P' ? null : 'P')}
                                                        className={`flex items-center gap-1 px-2 py-1 text-[10px] rounded transition-colors font-bold border ${
                                                            evt.mi_asistencia === 'P' ? 'bg-emerald-600 text-white border-emerald-700' : 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'
                                                        }`}
                                                    >
                                                        <IconCheck size={10} /> {evt.mi_asistencia === 'P' ? 'Confirmado' : 'Asisto'}
                                                    </button>
                                                    <button 
                                                        onClick={() => toggleMealAttendance(evt.id, evt.mi_asistencia === 'A' ? null : 'A')}
                                                        className={`flex items-center gap-1 px-2 py-1 text-[10px] rounded transition-colors font-bold border ${
                                                            evt.mi_asistencia === 'A' ? 'bg-red-600 text-white border-red-700' : 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100'
                                                        }`}
                                                    >
                                                        <IconX size={10} /> {evt.mi_asistencia === 'A' ? 'Rechazado' : 'No asisto'}
                                                    </button>
                                                </div>
                                            </div>
                                          )}
                                          {deadlineStatus?.status === 'CLOSED' && evt.mi_asistencia === null && (
                                            <span className="flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-50/50 px-2 py-0.5 rounded border border-red-200 mt-2">
                                                <IconBan size={10} /> Plazo finalizado
                                            </span>
                                          )}
                                    </>
                                  )}
