...
// Helper para parsear convocatoria (sin cambios)
...
export default function MealsAttendance({ supabase, gira, roster }) {
  ...
  const saveDeadline = async (dateVal, timeVal) => {
    if (!dateVal && !timeVal) {
      if (!dateVal && !timeVal) updateDbDeadline(null);
      return;
    }
    if (dateVal && timeVal) {
      ...
      if (!isNaN(dateObj.getTime())) {
        updateDbDeadline(dateObj.toISOString());
      }
    }
  };
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2">
                {/* Aumentamos de w-28 a w-36 para que la fecha entre cómoda */}
                <div className="w-36">
                  <DateInput
                    value={deadlineDate}
                    onChange={(val) => {
                      setDeadlineDate(val);
                      saveDeadline(val, deadlineTime);
                    }}
                    className="bg-white text-xs py-1 w-full" // Asegúrate de tener w-full aquí
                  />
                </div>

                {/* Aumentamos de w-20 a w-28 para la hora */}
                <div className="w-28">
                  <TimeInput
                    value={deadlineTime}
                    onChange={(val) => {
                      setDeadlineTime(val);
                      saveDeadline(deadlineDate, val);
                    }}
                    className="bg-white text-xs py-1 w-full" // Asegúrate de tener w-full aquí
                  />
                </div>
                ...
                <thead className="bg-slate-100 text-slate-600 uppercase font-bold text-[10px] sticky top-0 z-40 shadow-sm">
                  ...
                  <tr>
                    ...
                    <th className="p-0 bg-slate-100 sticky left-0 z-50 border-r border-b border-slate-300 w-[380px] min-w-[380px] max-w-[380px] align-bottom">
                      {
                        <div className="absolute bottom-0 right-0 bg-slate-100 text-[9px] text-slate-400 px-1 border-t border-l border-slate-200">
                          {`${sortedRoster.length} integrantes`}
                        </div>
                      }
                    </th>{" "}
                    {/* Vacío */}
                  ...
                  {/* FILA 2: CONTROLES DE ORDENAMIENTO + EVENTOS INDIVIDUALES */}
                  <tr>
                    {/* CELDA DE CONTROL - Z-INDEX ALTO Y ANCHO FIJO IDENTICO A ARRIBA */}
                    {/* CELDA DE CONTROL - DIVIDIDA EN 4 COLUMNAS INTERNAS */}
                    <th className="p-0 bg-slate-50 sticky left-0 z-50 border-r border-slate-300 align-bottom w-[380px] min-w-[380px] max-w-[380px]">
                      <div className="flex items-center h-full w-full text-[10px] text-slate-500 font-bold uppercase">
                        {/* COL 1: NOMBRE (Flexible, ocupa el resto) */}
                        ...
                        {/* COL 2: L (Localía) - Ancho fijo 40px */}
                        ...
                        {/* COL 3: CIU (Ciudad) - Ancho fijo 80px */}
                        ...
                        {/* COL 4: R (Rol/Instrumento) - Ancho fijo 80px */}
                      ...
                      {/* Contador de PAX pequeño flotando o debajo */}
                    </th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {sortedRoster.map((person) => (
                    <tr key={person.id} className="hover:bg-slate-50 group">
                      {/* CELDA DE INFORMACIÓN DEL INTEGRANTE - FORMATO TABLA REAL */}
                      <td className="p-0 sticky left-0 bg-white group-hover:bg-slate-50 z-40 border-r border-slate-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] w-[380px] min-w-[380px] max-w-[380px]">
                        <div className="flex items-center h-12 w-full text-xs">
                          ...
                          <div className="flex-1 px-3 min-w-0 flex flex-col justify-center h-full">
                            ...
                            {/* Etiqueta de dieta opcional debajo del nombre para no romper la columna */}
                            {person.alimentacion && (
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              ...
                              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 mx-auto border-2 ${
                                status === "P"
                                  ? "bg-emerald-100 border-emerald-400 text-emerald-700 shadow-sm scale-100"
                                  : status === "A"
                                  ? "bg-red-50 border-red-200 text-red-400 opacity-60 scale-95"
                                  : "bg-slate-50 border-slate-200 text-slate-300 hover:border-slate-300 hover:bg-white"
                              }`}
                              title={
                                status === "P"
                                  ? "Presente"
