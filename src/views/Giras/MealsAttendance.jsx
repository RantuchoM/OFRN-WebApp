...
export default function MealsAttendance({ supabase, gira, roster: enrichedRoster }) {
  ...
  const handleSaveDeadline = async () => {
    ...
    try {
      const isoString = new Date(`${deadline}T${deadlineTime}:00`).toISOString();
  ...
  const sortedRoster = useMemo(() => {
    ...
    let data = enrichedRoster.filter((p) => p.estado_gira !== "ausente" && p.estado_gira !== "baja");

    // 2. Filtro por Rol (Multiselect) - AHORA DINÁMICO
    if (selectedRoles.length > 0) {
      data = data.filter(p => selectedRoles.includes((p.rol_gira || "").toLowerCase()));
    }
  ...
  }, [enrichedRoster, searchTerm, sortConfig, filterResponse, events, attendanceMap, selectedRoles, selectedConditions]);
  ...
  const handleAttendanceChange = async (eventId, memberId, currentStatus) => {
    ...
    } catch (e) { toast.error("Error al actualizar asistencia"); }
  ...
  const handleSendMealEmails = async () => {
    ...
    if (!confirm(`¿Enviar mail de aviso a ${recipients.length} integrantes?\n\nSe enviará una COPIA OCULTA (BCC) a todos.`)) return;
    ...
    const fechaLarga = format(deadlineDateObj, "EEEE d 'de' MMMM 'a las' HH:mm 'hs'", { locale: es });
    ...
    try {
      ...
      toast.success(`Aviso enviado a ${bccList.length} personas.`, { id: toastId });
  ...
  const isExpired = deadline && isAfter(new Date(), new Date(`${deadline}T${deadlineTime}:00`));
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-lg p-2 max-w-fit">
              <div className="flex items-center gap-2">
                <div className={`p-1.5 rounded-full ${isExpired ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                  <IconClock size={16} />
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Cierre de Confirmación</span>
                  {isEditingDeadline ? (
                    <div className="flex items-center gap-2 mt-1">
                      <DateInput value={deadline} onChange={setDeadline} className="h-7 text-xs w-32 bg-white border-slate-300 focus:border-indigo-500" />
                      <TimeInput value={deadlineTime} onChange={setDeadlineTime} className="h-7 text-xs w-20 bg-white border-slate-300 focus:border-indigo-500" />
                      <button onClick={handleSaveDeadline} disabled={savingDeadline} className="p-1 bg-indigo-600 text-white rounded hover:bg-indigo-700 disabled:opacity-50">
                        {savingDeadline ? <IconLoader className="animate-spin" size={14} /> : <IconCheck size={14} />}
                      </button>
                      <button onClick={() => setIsEditingDeadline(false)} className="p-1 text-slate-400 hover:text-slate-600"><IconX size={14} /></button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 group cursor-pointer" onClick={() => setIsEditingDeadline(true)}>
                      <span className={`text-sm font-bold ${isExpired ? 'text-red-600' : 'text-slate-700'}`}>
                        {deadline ? (
                          <>
                            {format(parseISO(deadline), "d 'de' MMMM", { locale: es })} • {deadlineTime} hs
                            <span className="text-[10px] font-normal text-slate-400 ml-2 normal-case">
                              ({isExpired ? `Cerró hace ${formatDistanceToNow(new Date(`${deadline}T${deadlineTime}`), { locale: es })}` : `Cierra en ${formatDistanceToNow(new Date(`${deadline}T${deadlineTime}`), { locale: es })}`})
                            </span>
                          </>
                        ) : <span className="text-slate-400 italic">Sin fecha límite definida</span>}
                      </span>
                      <IconEdit size={12} className="text-slate-300 group-hover:text-indigo-500 transition-colors opacity-0 group-hover:opacity-100" />
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-col items-end gap-3">
            <div className="flex items-center gap-2">
              ...
              <div className="relative z-10">
                <IconSearch className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
                <input 
                  type="text" 
                  placeholder="Buscar..." 
                  value={searchTerm} 
            ...
            <div className="flex items-center gap-2">
                <select value={filterResponse} onChange={(e) => setFilterResponse(e.target.value)} className="text-xs border rounded px-2 py-1.5 font-bold outline-none bg-white hover:border-slate-400 transition-colors cursor-pointer h-[34px]">
                ...
                >
                  {sendingMails ? <IconLoader className="animate-spin" size={14} /> : <IconMail size={14} />}
