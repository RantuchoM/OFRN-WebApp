...
import DateInput from "../../components/ui/DateInput"; // Importado para el filtro de fechas
...
export default function GirasView({ supabase }) {
  ...
  // FIX: Multi-select Set para tipos de programa
  const [filterType, setFilterType] = useState(new Set(["Sinfónico", "Camerata Filarmónica", "Ensamble"]));
    ...
    try {
      ...
      if (isPersonal) {
        result = result.filter((gira) => {
          ...
          // PRIORIDAD 1: Override manual (giras_integrantes)
          if (myOverride && myOverride.estado === "ausente") return false; // Override Ausente -> Excluir
          if (myOverride) return true; // Override Presente/Rol -> Incluir

          // PRIORIDAD 2: Lógica dinámica de Inclusión
          ...
          if (isIncluded) {
              // PRIORIDAD 3: Lógica dinámica de Exclusión (EXCL_ENSAMBLE anula FAMILIA)
              const excludedEnsembles = sources
                  .filter(s => s.tipo === 'EXCL_ENSAMBLE')
                  .map(s => s.valor_id);
              
              const isExcludedByEnsemble = excludedEnsembles.some(exclId => myEnsembles.has(exclId));
              
              if (isExcludedByEnsemble) {
                  return false; // Incluido dinámicamente, pero luego excluido por ensamble
              }
              
              return true; // Incluido y no excluido
          }

          // Por defecto: no mostrar si no hay inclusión y no hay override
          return false;
        });
      }
      setGiras(result);
  ...
  const handleCommentNavigation = (type, entityId) => {
    ...
    if (type === "EVENTO") {
      setView({ mode: "AGENDA", data: currentGira });
    } else if (type === "OBRA") {
  ...
  // --- FUNCIONES AUXILIARES SOLICITADAS ---
  ...
  const renderDateInputs = (isMobile = false) => (
      <div className="flex items-center gap-2">
          {/* FIX: Usar DateInput para formato DD/MM/YYYY */}
  ...
  const getPersonnelDisplay = (gira) => {
    ...
    // Filtramos solo por rol y estado confirmado
    ...
    // REQUEST: Director/es pon Dir: (con | separador)
    if (directorNames.length > 0) {
        output.push(<span key="dir" className="font-semibold text-indigo-700">Dir: {directorNames.join(' | ')}</span>);
    }
    
    // REQUEST: Solista/s: | (solo si hay solistas)
    if (soloistNames.length > 0) {
        output.push(<span key="sol" className="font-semibold text-fuchsia-700">Solista/s: {soloistNames.join(' | ')}</span>);
    }
        ...
        <div className="text-xs space-y-1"> 
            <ul className="pl-1 space-y-0.5">
                {concerts.slice(0, 3).map((c, idx) => (
                    <li key={idx} className="text-slate-500 truncate max-w-full flex items-center gap-1">
                        <span className="font-mono text-[10px] mr-1 bg-slate-100 px-1 rounded">{formatDate(c.fecha)} - {c.hora_inicio.slice(0, 5)}</span>
                        {c.locaciones?.nombre}
                    </li>
                ))}
                {concerts.length > 3 && (
  ...
  // --- PREPARACIÓN DE EVENTOS PARA CALENDARIO (Omitted) ---
  const allCalendarEvents = useMemo(() => {
    return giras.flatMap((gira) => {
      ...
      return gira.eventos.map((evento) => {
        ...
        const nomStr = gira.nomenclador || "";
        const labelCombined = nomStr ? `${typeName} - ${nomStr}` : typeName;
              ...
              return (
                ...
                >
                  ...
                  <div className="pl-2 flex flex-col gap-2">
                        ...
                        {personnelDisplay ? (
                             <div className="flex flex-wrap items-center gap-3 text-xs">
                                 {personnelDisplay.map((item, index) => (
                                     <React.Fragment key={index}>
                                         {item}
                                         {index < personnelDisplay.length - 1 && <span className="text-slate-400">|</span>}
                                     </React.Fragment>
                                 ))}
                             </div>
                        ) : (
                            <span className="text-xs text-slate-400 italic">Sin personal clave asignado</span>
                        )}
                    ...
                    {/* LÍNEA 3: fecha desde/hasta | localía */}
