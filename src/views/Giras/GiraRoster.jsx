...
import { IconUsers, IconPlus, IconX, IconCheck, IconTrash, IconLoader, IconSearch, IconAlertCircle, IconFilter } from '../../components/ui/Icons';
...
export default function GiraRoster({ supabase, gira, onBack }) {
    ...
    const fetchDropdownData = async () => {
        ...
        if (inst) setFamiliesList([...new Set(inst.map(i => i.familia).filter(Boolean))]);
    };
    ...
    const loadAllData = async () => {
        ...
        try {
            ...
            const dynamicIds = new Set(); // Para saber quién vino por grupo y quién es "adicional"
    ...
    const addSource = async (tipo, valor) => {
        setLoading(true);
        const payload = { id_gira: gira.id, tipo, valor_id: tipo === 'ENSAMBLE' ? valor : null, valor_texto: tipo === 'FAMILIA' ? valor : null };
        const exists = sources.some(s => s.tipo === payload.tipo && (s.valor_id == payload.valor_id || s.valor_texto == payload.valor_texto));
        if (exists) { setLoading(false); return alert("Ya agregado."); }
        const { error } = await supabase.from('giras_fuentes').insert(payload);
        if (!error) { setAddMode(null); loadAllData(); }
    };
    ...
    const addManualMusician = async (musicianId) => {
        const { error } = await supabase.from('giras_integrantes').insert({ id_gira: gira.id, id_integrante: musicianId, estado: 'confirmado' });
    ...
    const toggleStatus = async (musician) => {
        ...
        const { error } = await supabase.from('giras_integrantes').upsert({ id_gira: gira.id, id_integrante: musician.id, estado: newStatus }, { onConflict: 'id_gira, id_integrante' });
        ...
        <div className="flex flex-col h-full bg-slate-50 animate-in fade-in duration-300">
            ...
            <div className="bg-white p-4 border-b border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between shrink-0 gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                            {gira.nombre_gira} 
                            <span className="text-xs font-normal text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full border">ID: {gira.id}</span>
                        </h2>
                        <div className="flex gap-2 mt-1 flex-wrap">
                            {sources.map(s => (
                                <span key={s.id} className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 uppercase tracking-wide">
                                    {s.tipo === 'ENSAMBLE' ? (ensemblesList.find(e => e.id === s.valor_id)?.ensamble || 'Ensamble') : s.valor_texto}
                                    <button onClick={() => removeSource(s.id)} className="ml-1 hover:text-red-500"><IconX size={12}/></button>
                                </span>
                            ))}
                            {sources.length === 0 && <span className="text-xs text-slate-400 italic">Sin grupos vinculados</span>}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-3 items-center">
                    <div className="text-xs font-bold text-slate-400 uppercase mr-1">Resumen:</div>
                    ...
                    <div className="text-xs text-slate-400 ml-1">Total: <b>{roster.length}</b></div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-hidden flex flex-col p-4 max-w-6xl mx-auto w-full gap-4">
                ...
                <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between z-20 overflow-visible">
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        <span className="text-xs font-bold text-slate-400 uppercase mr-2">Agregar:</span>
                        <div className="flex bg-slate-100 p-1 rounded-lg">
                            <button onClick={() => setAddMode(addMode === 'ensamble' ? null : 'ensamble')} className={`px-3 py-1 rounded text-xs font-medium transition-all ${addMode === 'ensamble' ? 'bg-white shadow text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}>Ensamble</button>
                            <button onClick={() => setAddMode(addMode === 'familia' ? null : 'familia')} className={`px-3 py-1 rounded text-xs font-medium transition-all ${addMode === 'familia' ? 'bg-white shadow text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}>Familia</button>
                            <button onClick={() => setAddMode(addMode === 'individual' ? null : 'individual')} className={`px-3 py-1 rounded text-xs font-medium transition-all ${addMode === 'individual' ? 'bg-white shadow text-indigo-700' : 'text-slate-500 hover:text-slate-700'}`}>Individual</button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 w-full flex justify-end">
                        {addMode === 'ensamble' && (
                            <div className="flex gap-2 animate-in slide-in-from-left-2 w-full max-w-xs">
                                <select className="flex-1 border border-slate-300 rounded px-2 py-1 text-sm outline-none" value={selectedEnsemble} onChange={e => setSelectedEnsemble(e.target.value)}>
                                    <option value="">Seleccionar Ensamble...</option>
                                    {ensemblesList.map(e => <option key={e.id} value={e.id}>{e.ensamble}</option>)}
                                </select>
                                <button onClick={() => addSource('ENSAMBLE', selectedEnsemble)} disabled={!selectedEnsemble} className="bg-indigo-600 text-white px-3 py-1 rounded text-sm hover:bg-indigo-700 disabled:opacity-50"><IconPlus size={16}/></button>
                            </div>
                        )}
                        {addMode === 'familia' && (
                            <div className="flex gap-2 animate-in slide-in-from-left-2 w-full max-w-xs">
                                <select className="flex-1 border border-slate-300 rounded px-2 py-1 text-sm outline-none" value={selectedFamily} onChange={e => setSelectedFamily(e.target.value)}>
                                    <option value="">Seleccionar Familia...</option>
                                    {familiesList.map(f => <option key={f} value={f}>{f}</option>)}
                                </select>
                                <button onClick={() => addSource('FAMILIA', selectedFamily)} disabled={!selectedFamily} className="bg-indigo-600 text-white px-3 py-1 rounded text-sm hover:bg-indigo-700 disabled:opacity-50"><IconPlus size={16}/></button>
                            </div>
                        )}
                        {addMode === 'individual' && (
                            <div className="relative w-full max-w-xs animate-in slide-in-from-left-2">
                                <input type="text" placeholder="Buscar apellido..." className="w-full border border-slate-300 rounded px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                                {searchResults.length > 0 && (
                                    <div className="absolute top-full right-0 w-64 bg-white border border-slate-200 mt-1 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto">
                                        {searchResults.map(m => (
                                            <button key={m.id} onClick={() => addManualMusician(m.id)} className="w-full text-left p-2 hover:bg-slate-50 text-sm border-b border-slate-50 last:border-0 group">
                                                <div className="font-bold text-slate-700 group-hover:text-indigo-700">{m.apellido}, {m.nombre}</div>
                                                <div className="text-xs text-slate-400">{m.instrumentos?.instrumento}</div>
                                            </button>
                                        ))}
                                    </div>
                                )}
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {roster.map((musician) => {
                                ...
                                return (
                                    <tr key={musician.id} className={`hover:bg-slate-50 transition-colors ${isAbsent ? 'bg-red-50/40' : ''}`}>
                                        <td className="p-3 pl-4 font-medium text-slate-700">
                                            {musician.apellido}, {musician.nombre}
                                        </td>
                                        <td className="p-3 text-slate-500">{musician.instrumentos?.instrumento || '-'}</td>
                                        <td className="p-3">
                                            {musician.es_adicional ? (
                                                <span className="text-[10px] bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded border border-amber-100">Adicional</span>
                                            ) : (
                                                <span className="text-[10px] text-slate-400">Grupo</span>
                                            )}
                                        </td>
                                        <td className="p-3 text-center">
                                            <button onClick={() => toggleStatus(musician)} className={`w-24 py-1 rounded-md text-[10px] font-bold border uppercase tracking-wider transition-all ${isAbsent ? 'bg-white text-red-600 border-red-200 hover:bg-red-50' : 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'}`}>
                                                {isAbsent ? 'Ausente' : 'Presente'}
                                            </button>
                                        </td>
                                        <td className="p-3 text-right pr-4">
                                            {musician.es_adicional && (
                                                <button onClick={() => removeMemberManual(musician.id)} className="text-slate-300 hover:text-red-500 p-1" title="Eliminar adicional"><IconTrash size={16}/></button>
                                            )}
                                        </td>
                                    </tr>
                                );
                            })}
                            {roster.length === 0 && !loading && (<tr><td colSpan="5" className="p-12 text-center text-slate-400 italic">Lista vacía.</td></tr>)}
                        </tbody>
                    </table>
                </div>
            </div>
