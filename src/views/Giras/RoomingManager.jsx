...
import {
  ...
  IconBed, IconMusic, IconTrash, IconMapPin, IconEdit, IconChevronDown
...
// --- SUBCOMPONENTE: TARJETA DE MÚSICO ---
  ...
  return (
    <div
      ...
      className={`p-1.5 rounded border text-xs flex justify-between items-center shadow-sm cursor-grab active:cursor-grabbing select-none transition-colors 
            ${isLocalWarning ? "bg-orange-100 border-orange-400 text-orange-900 ring-2 ring-orange-300/50 font-medium" : isLocal ? "bg-slate-100 text-slate-400 border-slate-200 grayscale opacity-80" : "bg-white text-slate-700 border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50"}`}
      title={isLocalWarning ? "¡Atención! Músico local asignado a hotel" : `${m.apellido}, ${m.nombre}`}
    >
...
// --- SUBCOMPONENTE: COLUMNA DE MÚSICOS ---
  ...
  return (
    ...
    >
      <h4 className={`font-bold text-${color}-800 mb-3 flex justify-between`}>
          <span>{title}</span>
          <span className="bg-white px-2 rounded text-xs shadow-sm">{visitors.length}</span>
      </h4>
      
      <div className="space-y-3 flex-1">
        {sortedCities.map(city => (
            <div key={city}>
                <div className={`text-[10px] font-bold uppercase tracking-wider mb-1 flex items-center gap-1 border-b border-${color}-200/50 pb-0.5 text-${color}-700`}>
                    {city}
                </div>
                <div className="space-y-1">
                    {byCity[city].map(m => <MusicianCard key={m.id} m={m} onDragStart={onDragStart} isLocal={false}/>)}
                </div>
            </div>
        ))}
        {visitors.length === 0 && <div className="text-xs text-slate-400 italic text-center py-4 opacity-70">Todos asignados</div>}
      </div>

      {locals.length > 0 && (
        <div className="mt-4 pt-2 border-t border-slate-200/60">
            <button onClick={() => setShowLocals(!showLocals)} className={`w-full flex justify-between items-center px-2 py-1.5 rounded text-xs font-bold transition-all ${showLocals ? "bg-slate-200 text-slate-600" : "bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-600"}`}>
                <span className="flex items-center gap-1"><IconMapPin size={12}/> Locales ({locals.length})</span>
                <IconChevronDown size={14} className={`transition-transform duration-200 ${showLocals ? "rotate-180" : ""}`}/>
            </button>
            {showLocals && (
                <div className="mt-2 space-y-1 animate-in slide-in-from-bottom-2 fade-in bg-slate-50 p-1.5 rounded border border-slate-100">
                    <div className="text-[9px] text-center text-slate-400 mb-1 italic">No requieren hotel</div>
                    {locals.map(m => <MusicianCard key={m.id} m={m} onDragStart={onDragStart} isLocal={true}/>)}
                </div>
            )}
        </div>
      )}
...
// --- SUBCOMPONENTE: TARJETA DE HABITACIÓN ---
const RoomCard = ({ 
    room, index, total, isDragging, onDragStart, onDragOver, onDrop, 
    onDelete, onEdit, onRemoveMusician, onMove, onTransfer, 
    onComment, getCapacityLabel, getRoomColor, supabase // <--- Asegurar que recibe supabase
}) => {
    ...
    >
      <div className="p-2 border-b border-black/5 flex justify-between items-start">
        ...
        <div className="flex items-center gap-0.5">
          ...
          {onTransfer && (
            <button onClick={onTransfer} className="text-slate-400 hover:text-indigo-600 p-0.5 rounded hover:bg-white/50 mr-1" title="Mover a otro hotel">
      ...
      <div className="p-1.5 space-y-1 min-h-[50px] flex-1">
        {room.occupants.map((m) => (
          <MusicianCard key={m.id} m={m} onDragStart={(e) => onDragStart(e, m, room.id)} isInRoom={true} onRemove={(mus) => onRemoveMusician(mus, room.id)} isLocal={m.isLocal} />
        ))}
...
export default function RoomingManager({ supabase, bookings, program, onBack }) {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      // 2. Traer fuentes y overrides para saber quién participa en la gira
      ...
      if (srcIndex !== -1) {
        ...
        if (srcRoom.occupants.length === 0) { 
            ...
            if (srcIndex < targetIndex) { 
                // Ajuste de índice si eliminamos una room anterior
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="flex flex-col gap-3">
                      ...
                      {roomsF.map((r, idx) => (
                        ...
                        supabase={supabase} // Importante pasar supabase
