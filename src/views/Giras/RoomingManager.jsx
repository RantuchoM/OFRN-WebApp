...
const HotelForm = ({ onSubmit, onClose, locationsList, masterHotels, initialData }) => {
    ...
    const handleSubmit = () => {
        if (mode === 'select' && !formData.id_hotel) return alert("Selecciona un hotel");
        if (mode === 'create' && (!formData.nombre || !formData.id_localidad)) return alert("Completa nombre y localidad");
...
const TransferRoomModal = ({ room, masterHotels, bookings, currentBooking, onConfirm, onClose }) => {
    const [targetHotelId, setTargetHotelId] = useState("");
    const currentHotelId = bookings.find(b => b.id === currentBooking)?.id_hotel;
    const sortedHotels = [...masterHotels].sort((a,b) => a.nombre.localeCompare(b.nombre));
...
const RoomForm = ({ onSubmit, onClose, initialData }) => {
  ...
  const handleSubmit = () => { onSubmit({ id: initialData?.id, tipo, es_matrimonial: esMatrimonial, con_cuna: conCuna, notas_internas: notas, configuracion: esMatrimonial ? "Matrimonial" : "Simple" }); onClose(); };
  return (
    <div className="p-4 bg-white border rounded-lg shadow-xl w-64 space-y-3 animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="space-y-2">
        <div><label className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Categoría</label><select value={tipo} onChange={(e) => setTipo(e.target.value)} className="w-full border p-1.5 rounded text-sm outline-none"><option value="Común">Estándar</option><option value="Plus">Plus / Suite</option></select></div>
        <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-slate-50 p-1 rounded"><input type="checkbox" checked={esMatrimonial} onChange={(e) => setEsMatrimonial(e.target.checked)} className="rounded text-indigo-600"/><span>Cama Matrimonial</span></label>
        <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-slate-50 p-1 rounded"><input type="checkbox" checked={conCuna} onChange={(e) => setConCuna(e.target.checked)} className="rounded text-indigo-600"/><span>Agregar Cuna</span></label>
        <textarea placeholder="Notas internas..." value={notas} onChange={(e) => setNotas(e.target.value)} className="w-full border p-1.5 rounded text-xs resize-none h-16 outline-none"/>
      </div>
...
const MusicianCard = ({ m, onDragStart, isInRoom, onRemove, isLocal }) => {
  ...
  return (
    <div draggable onDragStart={(e) => { e.stopPropagation(); onDragStart(e, m); }} className={`p-1.5 rounded border text-xs flex justify-between items-center shadow-sm cursor-grab active:cursor-grabbing select-none transition-colors ${isLocalWarning ? "bg-orange-100 border-orange-400 text-orange-900 ring-2 ring-orange-300/50 font-medium" : isLocal ? "bg-slate-100 text-slate-400 border-slate-200 grayscale opacity-80" : "bg-white text-slate-700 border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50"}`} title={isLocalWarning ? "¡Atención! Músico local asignado a hotel" : `${m.apellido}, ${m.nombre}`}>
      <div className="truncate max-w-[120px]"><b>{m.apellido}</b> {m.nombre}</div>
      <div className="flex items-center gap-1">
        {isLocal && <IconMapPin size={10} className={isLocalWarning ? "text-orange-600" : "text-slate-400"}/>}
        <span className={`text-[9px] px-1 rounded uppercase border ${isLocalWarning ? "bg-orange-200 border-orange-300 text-orange-800" : "text-slate-400 bg-black/5 border-transparent"}`}>{loc.slice(0, 3)}</span>
...
const MusicianListColumn = ({ title, color, musicians, isDragging, onDragStart, onDragOver, onDrop }) => {
  ...
  return (
    <div className={`w-48 flex flex-col bg-${color}-50/50 rounded-xl border border-${color}-100 p-3 overflow-y-auto transition-colors ${isDragging ? `bg-${color}-100/50 border-${color}-300 border-dashed` : ""}`} onDragOver={onDragOver} onDrop={onDrop}>
      <h4 className={`font-bold text-${color}-800 mb-3 flex justify-between`}><span>{title}</span><span className="bg-white px-2 rounded text-xs shadow-sm">{visitors.length}</span></h4>
      <div className="space-y-3 flex-1">{sortedCities.map(city => (<div key={city}><div className={`text-[10px] font-bold uppercase tracking-wider mb-1 flex items-center gap-1 border-b border-${color}-200/50 pb-0.5 text-${color}-700`}>{city}</div><div className="space-y-1">{byCity[city].map(m => <MusicianCard key={m.id} m={m} onDragStart={onDragStart} isLocal={false}/>)}</div></div>))}{visitors.length === 0 && <div className="text-xs text-slate-400 italic text-center py-4 opacity-70">Todos asignados</div>}</div>
      {locals.length > 0 && (<div className="mt-4 pt-2 border-t border-slate-200/60"><button onClick={() => setShowLocals(!showLocals)} className={`w-full flex justify-between items-center px-2 py-1.5 rounded text-xs font-bold transition-all ${showLocals ? "bg-slate-200 text-slate-600" : "bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-600"}`}><span className="flex items-center gap-1"><IconMapPin size={12}/> Locales ({locals.length})</span><IconChevronDown size={14} className={`transition-transform duration-200 ${showLocals ? "rotate-180" : ""}`}/></button>{showLocals && (<div className="mt-2 space-y-1 animate-in slide-in-from-bottom-2 fade-in bg-slate-50 p-1.5 rounded border border-slate-100"><div className="text-[9px] text-center text-slate-400 mb-1 italic">No requieren hotel</div>{locals.map(m => <MusicianCard key={m.id} m={m} onDragStart={onDragStart} isLocal={true}/>)}</div>)}</div>)}
    </div>
  );
};

const RoomCard = ({ room, index, total, isDragging, onDragStart, onDragOver, onDrop, onDelete, onEdit, onRemoveMusician, onMove, onTransfer, onComment, getCapacityLabel, getRoomColor, supabase }) => {
...
export default function RoomingManager({ supabase, program, onBack }) {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (bookingIds.length > 0) {
          const { data } = await supabase.from("hospedaje_habitaciones").select("*").in("id_hospedaje", bookingIds).order("orden", { ascending: true });
          roomsData = data || [];
      }
      ...
      const roomsWithDetails = roomsData.map((room) => {
        ...
        return { ...room, occupants, roomGender: calculateRoomGender(occupants) };
      });
  ...
  const getCapacityLabel = (c) => { if(c===0) return "Vacía"; if(c===1) return "Single"; if(c===2) return "Doble"; if(c===3) return "Triple"; return `Múltiple (${c})`; };
  const getRoomColor = (c) => { if(c===0) return "bg-slate-50 border-slate-200"; if(c===1) return "bg-blue-50 border-blue-200 text-blue-800"; if(c===2) return "bg-emerald-50 border-emerald-200 text-emerald-800"; if(c===3) return "bg-amber-50 border-amber-200 text-amber-800"; return "bg-rose-50 border-rose-200 text-rose-800"; };
                ...
                <div key={bk.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 relative group">
                  <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="text-xs text-slate-500 font-mono mr-2">Habitaciones: {hotelRooms.length} | Pax: {hotelRooms.reduce((acc, r) => acc + r.occupants.length, 0)}</div>
                        <button onClick={() => setHotelToMerge(bk)} className="text-slate-300 hover:text-amber-600 p-1" title="Fusionar / Trasladar todo"><IconRefresh size={16}/></button>
                        <button onClick={() => { setEditingHotelData({ id: bk.id, id_hotel: bk.id_hotel, id_localidad: bk.hoteles.id_localidad }); setShowHotelForm(true); }} className="text-slate-300 hover:text-indigo-600 p-1"><IconEdit size={16}/></button>
                        <button onClick={() => handleDeleteHotel(bk.id)} className="text-slate-300 hover:text-red-600 p-1"><IconTrash size={16}/></button>
                    </div>
                  </div>
