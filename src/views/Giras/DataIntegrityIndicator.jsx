      ...
      <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-red-200 shadow-xl rounded-lg p-0 z-50 hidden group-hover:flex flex-col max-h-60">
        ...
        <div className="overflow-y-auto p-2">
          <ul className="space-y-2">
            {issues.map((issue) => (
              <li key={issue.id} className="flex flex-col border-b border-slate-50 last:border-0 pb-1">
...
export default function GirasTransportesManager({ supabase, gira }) {
  ...
  const [passengersModal, setPassengersModal] = useState({ isOpen: false, transportId: null });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: list } = await supabase
        ...
        .select(`id, detalle, costo, capacidad_maxima, id_transporte, transportes ( nombre )`)
        .eq("id_gira", giraId)
        .order("id");
      ...
      if (list && list.length > 0) {
        ...
        const { data: evts } = await supabase
          .from("eventos")
          .select(`id, fecha, hora_inicio, descripcion, id_tipo_evento, id_gira_transporte, id_locacion, locaciones(nombre, direccion, localidades(localidad))`)
  ...
  const handleAddTransport = async () => {
    ...
    await supabase.from("giras_transportes").insert([{
        ...
        capacidad_maxima: newTransp.capacidad ? parseInt(newTransp.capacidad) : null,
      }]);
  ...
  const handleDeleteEvent = async (eventId) => {
    const rulesAffectingUp = (transportRules || []).filter((r) => String(r.id_evento_subida) === String(eventId));
    const rulesAffectingDown = (transportRules || []).filter((r) => String(r.id_evento_bajada) === String(eventId));
    const totalAffected = rulesAffectingUp.length + rulesAffectingDown.length;

    if (totalAffected > 0) {
      const message = `⚠️ ADVERTENCIA DE INTEGRIDAD \n\n` +
  ...
  const startEditingTransport = (e, t) => {
    ...
    setEditFormData({ detalle: t.detalle || "", capacidad: t.capacidad_maxima || "", costo: t.costo || "" });
  };
  ...
  const saveTransportChanges = async (e) => {
    ...
    setLoading(true);
    try {
      await supabase.from("giras_transportes").update({
          detalle: editFormData.detalle,
          capacidad_maxima: editFormData.capacidad ? parseInt(editFormData.capacidad) : null,
  ...
  return (
    <div className="h-full overflow-y-auto p-4 bg-white rounded-lg shadow-sm border border-slate-200 max-w-6xl mx-auto">
      ...
      <div className="mb-6 grid grid-cols-3 gap-4">
        ...
        {/* MULTI-TRANSPORTE */}
        <div
          onClick={() => setInfoListModal({ isOpen: true, title: "Pasajeros en Múltiples Transportes", list: coverageStats.multiple })}
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs text-amber-600 font-medium underline decoration-dashed underline-offset-2">Multi-transporte (Ver)</div>
          </div>
        </div>

        {/* SIN TRANSPORTE */}
        <div
          onClick={() => setInfoListModal({ isOpen: true, title: "Lista de Espera (Sin Transporte)", list: coverageStats.none })}
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs text-rose-600 font-medium underline decoration-dashed underline-offset-2">Sin transporte (Ver)</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 items-end bg-slate-50 p-3 rounded-lg border border-slate-200">
        ...
        <div className="w-24">
          ...
          <input type="number" min="0" className="w-full text-xs border p-2 rounded" placeholder="Opcional" value={newTransp.capacidad} onChange={(e) => setNewTransp({ ...newTransp, capacidad: e.target.value })} />
        </div>
      ...
      <div className="space-y-4">
        {transports.map((t) => {
          ...
          const tPassengers = passengerList.filter((p) => p.logistics?.transports?.some((tr) => tr.id === t.id));
          ...
          const occupancyColor = isOverbooked ? "text-red-600 bg-red-50 border-red-200" : maxCap > 0 && tPassengerCount === maxCap ? "text-amber-600 bg-amber-50 border-amber-200" : "text-indigo-600";
          ...
          return (
            <div key={t.id} className={`border rounded-lg transition-all ${isExpanded ? "border-indigo-300 shadow-md bg-white" : "border-slate-200 bg-white"}`}>
              ...
              <div className="p-3 flex justify-between items-center cursor-pointer hover:bg-slate-50" onClick={() => !isEditing && setActiveTransportId(isExpanded ? null : t.id)}>
                  ...
                  {isEditing ? (
                    <div className="flex gap-2 items-center flex-1" onClick={(e) => e.stopPropagation()}>
                      <div className="flex flex-col">
                        <label className="text-[9px] font-bold text-slate-400">DETALLE</label>
                        <input className="border border-indigo-300 rounded px-2 py-1 text-sm w-48 outline-none" value={editFormData.detalle} onChange={(e) => setEditFormData({ ...editFormData, detalle: e.target.value })} autoFocus />
                      </div>
                      <div className="flex flex-col w-24">
                        <label className="text-[9px] font-bold text-slate-400">CAPACIDAD</label>
                        <input type="number" className="border border-indigo-300 rounded px-2 py-1 text-sm w-full outline-none" value={editFormData.capacidad} onChange={(e) => setEditFormData({ ...editFormData, capacidad: e.target.value })} />
                      </div>
                      <div className="flex gap-1 mt-4">
                        <button onClick={saveTransportChanges} className="bg-indigo-600 text-white p-1 rounded hover:bg-indigo-700"><IconSave size={16} /></button>
                        <button onClick={cancelEditingTransport} className="bg-slate-200 text-slate-600 p-1 rounded hover:bg-slate-300"><IconX size={16} /></button>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <div className="font-bold text-slate-800 flex items-center gap-2">
                        ...
                        <button onClick={(e) => startEditingTransport(e, t)} className="text-slate-300 hover:text-indigo-600"><IconEdit size={14} /></button>
                      </div>
                      <div className="text-xs text-slate-500 uppercase flex items-center gap-2 mt-1">
                        <span>{t.transportes?.nombre}</span>
                        <span className="text-slate-300">|</span>
                        <span className={`font-bold px-2 py-0.5 rounded border ${occupancyColor} flex items-center gap-1`}>
                          {isOverbooked && <IconAlertTriangle size={12} />}
                          {tPassengerCount} {maxCap > 0 ? `/ ${maxCap}` : "Pasajeros"}
                        </span>
                      </div>
                    </div>
                  )}
                ...
                {!isEditing && (
                  <div className="flex gap-2 items-center">
                    {incompletePax.length > 0 && (
                      <button
                        onClick={(e) => { e.stopPropagation(); setInfoListModal({ isOpen: true, title: `Pasajeros sin parada en ${t.detalle || "Transporte"}`, list: incompletePax }); }}
                      ...
                      >
                        <IconAlertTriangle size={12} /> {incompletePax.length} Sin asignar
                      </button>
                    )}
                    <button onClick={(e) => { e.stopPropagation(); setPassengersModal({ isOpen: true, transportId: t.id }); }} className="flex items-center gap-2 px-3 py-1 bg-indigo-600 text-white rounded text-xs font-bold hover:bg-indigo-700 shadow-sm"><IconUsers size={14} /> Pasajeros</button>
                    <button onClick={(e) => { e.stopPropagation(); setItineraryModal({ isOpen: true, transportId: t.id }); }} className="text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold border border-indigo-100 hover:bg-indigo-100">+ Itinerario</button>
                    <button onClick={(e) => { e.stopPropagation(); setBoardingModal({ isOpen: true, transportId: t.id }); }} className="px-2 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded text-[10px] font-bold hover:bg-amber-100" title="Gestor de Abordaje"><IconUsers size={14} /></button>
                    <button onClick={(e) => { e.stopPropagation(); setRoadmapModal({ isOpen: true, transportId: t.id }); }} className="px-2 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded text-[10px] font-bold hover:bg-blue-100" title="Hoja de Ruta"><IconFileText size={14} /> Hoja Ruta</button>
                    <button onClick={(e) => { e.stopPropagation(); setCnrtModal({ isOpen: true, transportId: t.id }); }} className="px-2 py-1 bg-slate-50 text-slate-600 border border-slate-200 rounded text-[10px] font-bold hover:bg-slate-100" title="Lista CNRT">CNRT</button>
                  ...
                  <table className="w-full min-w-[1000px] text-xs text-left border-t border-slate-200">
                    <thead className="bg-slate-50 text-slate-500 font-semibold border-b">
                      <tr>
                        ...
                        <th className="p-2 w-28 text-center bg-emerald-50 text-emerald-700 border-l border-emerald-100">Suben</th>
                        <th className="p-2 w-28 text-center bg-rose-50 text-rose-700 border-l border-rose-100">Bajan</th>
                      ...
                      {myEvents.map((evt) => {
                        ...
                        return (
                          <tr key={evt.id} className="hover:bg-slate-50 group">
                            <td className="p-1"><DateInput value={evt.fecha} onChange={(v) => handleUpdateEvent(evt.id, "fecha", v)} className="h-8 border-transparent hover:border-slate-300 bg-transparent" /></td>
                            <td className="p-1"><TimeInput value={evt.hora_inicio} onChange={(v) => handleUpdateEvent(evt.id, "hora_inicio", v)} className="h-8 border-transparent hover:border-slate-300 bg-transparent" /></td>
                            <td className="p-1 relative"><SearchableSelect options={locationOptions} value={evt.id_locacion} onChange={(v) => handleUpdateEvent(evt.id, "id_locacion", v)} className="h-8" placeholder="Sin locación..." /></td>
                            <td className="p-1"><input type="text" className="w-full h-8 px-2 border border-transparent hover:border-slate-300 rounded bg-transparent" value={evt.descripcion || ""} onChange={(e) => handleUpdateEvent(evt.id, "descripcion", e.target.value)} placeholder="-" /></td>
                            <td className="p-1"><select className="w-full h-8 border-transparent hover:border-slate-300 rounded bg-transparent text-xs outline-none" value={evt.id_tipo_evento} onChange={(e) => handleUpdateEvent(evt.id, "id_tipo_evento", e.target.value)}><option value="11">Público</option><option value="12">Interno</option></select></td>
                            <td className="p-1 text-center border-l border-emerald-50 align-top"><button onClick={() => setRulesModal({ isOpen: true, event: evt, type: "up", transportId: t.id })} className={`w-full py-1 px-1 h-auto rounded text-[10px] font-bold flex flex-col items-center justify-center gap-1 min-h-[32px] ${upsCount > 0 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}><div className="flex items-center gap-1 border-b border-black/5 pb-0.5 w-full justify-center"><IconUpload size={10} /> <span>{upsCount > 0 ? `${upsCount} Pax` : "+"}</span></div>{upsSummary && upsSummary.length > 0 && (<div className="flex flex-col gap-0.5 w-full text-center">{upsSummary.map((line, idx) => (<span key={idx} className="text-[9px] font-normal leading-tight break-words opacity-90">{line}</span>))}</div>)}</button></td>
                            <td className="p-1 text-center border-l border-rose-50 align-top"><button onClick={() => setRulesModal({ isOpen: true, event: evt, type: "down", transportId: t.id })} className={`w-full py-1 px-1 h-auto rounded text-[10px] font-bold flex flex-col items-center justify-center gap-1 min-h-[32px] ${downsCount > 0 ? "bg-rose-100 text-rose-700" : "bg-slate-100 text-slate-400"}`}><div className="flex items-center gap-1 border-b border-black/5 pb-0.5 w-full justify-center"><IconDownload size={10} /> <span>{downsCount > 0 ? `${downsCount} Pax` : "+"}</span></div>{downsSummary && downsSummary.length > 0 && (<div className="flex flex-col gap-0.5 w-full text-center">{downsSummary.map((line, idx) => (<span key={idx} className="text-[9px] font-normal leading-tight break-words opacity-90">{line}</span>))}</div>)}</button></td>
                            <td className="p-1 text-center align-middle"><button onClick={() => handleDeleteEvent(evt.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><IconTrash size={14} /></button></td>
                          </tr>
                        );
                      })}
                      {editingEventId ? (
                        <tr className="bg-amber-50">
                          ...
                          <td colSpan="4" className="p-1 text-right"><div className="flex gap-1 justify-end"><button onClick={cancelEdit} className="text-[10px] px-2 py-1 bg-slate-200 rounded">Cancelar</button><button onClick={() => handleSaveEvent(t.id)} className="text-[10px] px-3 py-1 bg-amber-500 text-white rounded font-bold">Guardar</button></div></td>
                        </tr>
                      ) : (
                        <tr><td colSpan="9" className="p-2 text-center border-t border-slate-100"><button onClick={() => startEditEvent({ id: null, fecha: "", hora_inicio: "", id_tipo_evento: 11, descripcion: "", id_locacion: null })} className="text-xs text-indigo-500 font-bold hover:text-indigo-700 flex items-center justify-center gap-1 w-full"><IconPlus size={12} /> Agregar Parada Manualmente</button></td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
      ...
      <InfoListModal />
      {passengersModal.isOpen && <TransportPassengersModal isOpen={passengersModal.isOpen} onClose={() => setPassengersModal({ isOpen: false, transportId: null })} transport={transports.find((t) => t.id === passengersModal.transportId)} transportRules={transportRules} roster={roster} regions={regionsList} localities={localitiesList} supabase={supabase} onRefresh={refresh} />}
      {cnrtModal.isOpen && <CnrtExportModal transport={transports.find((t) => t.id === cnrtModal.transportId)} events={transportEvents[cnrtModal.transportId] || []} onClose={() => setCnrtModal({ isOpen: false, transportId: null })} onExport={handleExportCNRT} />}
      {roadmapModal.isOpen && <CnrtExportModal title="Exportar Hoja de Ruta" transport={transports.find((t) => t.id === roadmapModal.transportId)} events={transportEvents[roadmapModal.transportId] || []} onClose={() => setRoadmapModal({ isOpen: false, transportId: null })} onExport={handleExportRoadmap} />}
      {itineraryModal.isOpen && <ItineraryManagerModal supabase={supabase} isOpen={itineraryModal.isOpen} onClose={() => setItineraryModal({ isOpen: false, transportId: null })} locations={locationsList} localities={localitiesList} onApplyItinerary={handleInsertItinerary} />}
      {boardingModal.isOpen && <BoardingManagerModal isOpen={boardingModal.isOpen} onClose={() => setBoardingModal({ isOpen: false, transportId: null })} transportId={boardingModal.transportId} passengers={passengerList} events={transportEvents[boardingModal.transportId] || []} onSaveBoarding={handleSaveBoarding} />}
      {rulesModal.isOpen && <StopRulesManager isOpen={rulesModal.isOpen} onClose={() => setRulesModal({ isOpen: false, event: null, type: null, transportId: null })} event={rulesModal.event} type={rulesModal.type} transportId={rulesModal.transportId} supabase={supabase} giraId={giraId} regions={regionsList} localities={localitiesList} musicians={musiciansList} onRefresh={fetchData} />}
    </div>
  );
}