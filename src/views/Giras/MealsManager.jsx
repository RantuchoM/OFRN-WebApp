        ...
        <div className="absolute top-full left-0 w-[220px] bg-white border border-slate-300 shadow-xl rounded-lg z-[999] mt-1 overflow-hidden flex flex-col">
          <div className="p-2 border-b border-slate-100 bg-slate-50">
            <input
              ref={inputRef}
              type="text"
              className="w-full text-xs p-1 border border-slate-200 rounded focus:outline-none focus:border-indigo-500"
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) => (
                <div
                  ...
                  className={`px-3 py-2 text-xs cursor-pointer border-b border-slate-50 last:border-0 hover:bg-indigo-50 hover:text-indigo-700 ${String(value) === String(opt.id) ? "bg-indigo-50 font-bold text-indigo-700" : "text-slate-600"}`}
                >
                  {opt.label}
                </div>
              ))
...
const MultiGroupSelect = ({
  ...
  return (
    ...
    >
      <div
        onClick={() => !disabled && setIsOpen(!isOpen)}
        className={`min-h-[28px] w-full border rounded px-2 py-1 bg-white cursor-pointer flex flex-wrap gap-1 items-center transition-all ${showAlert ? "border-orange-400 ring-1 ring-orange-200" : "border-slate-300 hover:border-indigo-400"}`}
        title={showAlert ? "Debes definir para quién es esta comida" : ""}
      >
        {!value.length && (
          <span
            className={`text-[10px] ${showAlert ? "text-orange-500 font-bold" : "text-slate-400 italic"}`}
          >
        ...
        {value.map((id) => {
          ...
          if (id.startsWith("GRP:"))
            ...
            label =
              catalogs.localidades.find((l) => `LOC:${l.id}` === id)
                ?.localidad || id;
          else if (id.startsWith("FAM:")) label = id.replace("FAM:", "");
          return (
            <span
              key={id}
              className="text-[9px] bg-indigo-50 text-indigo-700 border border-indigo-100 px-1 rounded truncate max-w-[100px]"
            >
              {label}
            </span>
          );
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-[250px] bg-white border border-slate-300 shadow-xl rounded-lg z-[999] mt-1 max-h-60 overflow-y-auto p-2 space-y-2">
          <div>
            <h4 className="text-[9px] font-bold text-slate-400 uppercase">
              Grupos
            </h4>
            <div className="grid grid-cols-1 gap-1">
              {groups.map((g) => (
                <div
                  ...
                  className={`text-[10px] p-1 rounded cursor-pointer border ${value.includes(g.id) ? "bg-indigo-100 border-indigo-300 text-indigo-800" : "bg-slate-50 border-slate-100"}`}
                >
                  {g.label}
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-1">
              {catalogs.localidades.map((l) => (
                <div
                  ...
                  className={`text-[10px] p-1 rounded cursor-pointer border truncate ${value.includes(`LOC:${l.id}`) ? "bg-purple-100 border-purple-300" : "bg-slate-50 border-slate-100"}`}
                >
                  {l.localidad}
                </div>
              ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-1">
              {catalogs.familias.map((f) => (
                <div
                  key={f}
                  onClick={() => toggleOption(`FAM:${f}`)}
                  className={`text-[10px] p-1 rounded cursor-pointer border truncate ${value.includes(`FAM:${f}`) ? "bg-amber-100 border-amber-300" : "bg-slate-50 border-slate-100"}`}
                >
                  {f}
                </div>
              ))}
  ...
  return (
    <div className="bg-indigo-50 border-b border-indigo-100 p-3 flex items-center justify-between animate-in slide-in-from-top-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-20">
            <TimeInput
              ...
              onChange={(v) => setValues({ ...values, hora_fin: v })}
              placeholder="Fin..."
            />
          </div>
          <div className="w-40">
      ...
      <div className="flex gap-2">
        <button
          onClick={onCancel}
          className="text-slate-500 hover:text-slate-700 text-xs font-bold px-3 py-1.5"
        >
          Cancelar
        </button>
        <button
          onClick={handleApply}
          className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-1.5 rounded flex items-center gap-1 shadow-sm"
        >
...
export default function MealsManager({ supabase, gira, roster }) {
  ...
  const calculateGrid = (mealsData, rulesData) => {
    if (!rulesData.length && !mealsData.length) {
      setGrid([]);
      return;
    }
      ...
      SERVICIOS.forEach((svc) => {
        ...
        if (currentKey >= minKey && currentKey <= maxKey) {
          const existing = normalizedMeals.find(
            (m) => m.fecha === currentDateStr && m.servicio === svc,
          );
          newGrid.push(
            existing
    ...
    setGrid((prev) => {
      ...
      if (field === "convocados") {
        ...
        const isSystemGenerated =
          currentDesc.startsWith(row.servicio) || currentDesc === "";
      ...
      if (field === "id_locacion") {
        ...
        if (descEmpty) {
          const locName = catalogs.locaciones.find(
  ...
  const handleBulkApply = async (changes) => {
      ...
      prev.map((row) => {
        if (selectedIds.includes(row.id)) {
          ...
          if (changes.convocados) {
            ...
            const isSystemGenerated =
              currentDesc.startsWith(newRow.servicio) || currentDesc === "";
            if (isSystemGenerated) newRow.descripcion = autoDesc;
          }
          // Recalcular descripción si cambió locación
          if (changes.id_locacion) {
            const descEmpty =
              !newRow.descripcion || newRow.descripcion.trim() === "";
            if (descEmpty) {
              const locName = catalogs.locaciones.find(
                (l) => String(l.id) === String(changes.id_locacion),
              )?.label;
              if (locName) {
                const simpleName = locName.split("(")[0].trim();
    ...
    const rowsToSave = grid
      ...
      .map((r) => ({ ...r, ...changes })); // Usamos los datos nuevos
    ...
    for (const row of rowsToSave) {
      ...
      let rowToSave = { ...row };
      if (changes.convocados)
        rowToSave.descripcion = generateDescription(
          rowToSave.servicio,
          changes.convocados,
        );
  ...
  const saveRow = async (row) => {
    ...
    try {
      ...
      const savedMeal = {
        ...
        hora_inicio: resultData.hora_inicio
          ? resultData.hora_inicio.slice(0, 5)
          : "",
      ...
      if (row.isTemp) setDbMeals((prev) => [...prev, savedMeal]);
        ...
        setDbMeals((prev) =>
          prev.map((m) => (m.id === row.id ? savedMeal : m)),
        );

      setGrid((prev) =>
        prev.map((r) =>
          r.id === row.id
            ? { ...r, id: savedMeal.id, isTemp: false, dirty: false }
            : r,
        ),
      );
    } catch (error) {
      ...
      <div className="bg-white p-4 border-b border-slate-200 shadow-sm flex justify-between items-center shrink-0 z-10 relative">
        <div className="flex items-center gap-3">
          ...
          <div className="relative group">
            ...
            <div
              className={`absolute z-50 top-full left-0 mt-2 w-72 bg-white border border-slate-200 shadow-xl rounded-lg p-3 text-xs text-slate-600 z-50 transition-all origin-top-left ${showHelp ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
            >
              <h4 className="font-bold text-slate-800 mb-2 uppercase text-[10px]">
                Guía Rápida
              </h4>
              <ul className="space-y-1.5 list-disc pl-3">
                <li>
                  Usa los <strong>checkboxes</strong> de la izquierda para
                  edición masiva.
                </li>
                <li>
                  <strong>Shift + Click</strong> para seleccionar rangos de
                  comidas.
                </li>
                <li>
                  Los eventos grises son <strong>borradores</strong> (no existen
                  en BD).
                </li>
                <li>
                  Los cambios se guardan <strong>automáticamente</strong> al
        ...
        <div className="bg-white border border-slate-300 rounded-lg shadow-sm flex-1 overflow-hidden flex flex-col relative">
          <div className="flex-1 overflow-auto">
            <table className="w-full text-left text-sm min-w-[1100px] border-collapse relative">
              <thead className="bg-slate-100 text-slate-500 uppercase font-bold text-[10px] z-10 sticky top-0 shadow-sm">
                <tr>
                  {/* HEADER CHECKBOX */}
                  <th className="px-2 py-3 w-8 border-b border-r border-slate-200 bg-slate-100 sticky top-0 left-0 z-50 text-center">
                    <input
                      ...
                      onChange={selectAll}
                      checked={
                        selectedRows.size === grid.length && grid.length > 0
                      }
                      className="rounded text-indigo-600 focus:ring-indigo-500"
                    />
                  </th>
                  <th className="px-3 py-3 w-32 border-b border-r border-slate-200 bg-slate-100 sticky top-0 left-8 z-50">
                    Día
                  </th>
                  <th className="px-3 py-3 w-24 border-b border-slate-200 bg-slate-100 sticky top-0 z-40">
                    Servicio
                  </th>
                  <th className="px-3 py-3 w-24 border-b border-slate-200 bg-slate-100 sticky top-0 z-40">
                    Inicio
                  ...
                  <th className="px-3 py-3 w-56 border-b border-slate-200 bg-slate-100 sticky top-0 z-40">
                    Lugar
                  </th>
                  <th className="px-3 py-3 min-w-[200px] border-b border-slate-200 bg-slate-100 sticky top-0 z-40">
                    Descripción
                  </th>
                  <th className="px-3 py-3 min-w-[250px] border-b border-slate-200 bg-slate-100 sticky top-0 z-40">
                    Convocados
                  </th>
                  <th className="px-3 py-3 w-12 text-center border-b border-slate-200 bg-slate-100 sticky top-0 z-40">
                    Vis.
                  </th>
              ...
              <tbody className="divide-y divide-slate-100">
                    ...
                    return rows.map((row, i) => {
                      ...
                      return (
                        <tr key={row.id} className={`group ${rowClass}`}>
                          {/* CHECKBOX CELL */}
                          <td
                            className={`px-2 py-1 text-center border-b border-r border-slate-200 sticky left-0 z-30 ${isSelected ? "bg-indigo-50" : "bg-white"}`}
                          >
                            <input
                              type="checkbox"
                              checked={isSelected}
                              onChange={(e) =>
                                toggleSelectRow(row.id, e.nativeEvent)
                              }
                          ...
                          {i === 0 && (
                            <td
                              rowSpan={rows.length}
                              className="px-3 py-3 font-bold text-slate-700 border-r border-slate-200 bg-white sticky left-8 align-top z-30 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]"
                            >
                              <div className="flex items-center gap-2">
                                <IconCalendar
                                  size={14}
                          ...
                          <td className="px-3 py-2">
                            <span
                              className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${isTemp ? "bg-slate-200 text-slate-500" : row.servicio === "Almuerzo" ? "bg-amber-100 text-amber-700" : row.servicio === "Cena" ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}
                            >
                              {row.servicio}
                            </span>
                          </td>
                          ...
                          <td className="px-1 py-1">
                            <input
                              ...
                              disabled={disableInputs}
                              className={`w-full text-xs border rounded p-1 focus:border-indigo-500 focus:outline-none ${isTemp ? "bg-transparent" : "bg-white"}`}
                            />
                          </td>
                          <td className="px-1 py-1">
                            <MultiGroupSelect
                ...
                {!grid.length && !loading && (
                  <tr>
                    ...
                    >
                      No hay rango definido. Configura las fechas en la pestaña
                      Logística.
                    </td>
                  </tr>
                )}
