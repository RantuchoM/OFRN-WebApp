  ...
  return (
    <div className={`relative w-full ${disabled ? 'opacity-50 pointer-events-none' : ''}`} ref={containerRef}>
      <div onClick={() => !disabled && setIsOpen(!isOpen)} className={`min-h-[28px] w-full border rounded px-2 py-1 bg-white cursor-pointer flex flex-wrap gap-1 items-center transition-all ${showAlert ? "border-orange-400 ring-1 ring-orange-200" : "border-slate-300 hover:border-indigo-400"}`} title={showAlert ? "Debes definir para quién es esta comida" : ""}>
        {!value.length && <span className={`text-[10px] ${showAlert ? "text-orange-500 font-bold" : "text-slate-400"}`}>{showAlert ? "⚠️ Definir..." : "Seleccionar..."}</span>}
        {value.map((id) => {
          let label = id;
...
export default function MealsManager({ supabase, gira, roster }) {
  ...
  const [showHelp, setShowHelp] = useState(false); // Estado para el tooltip de ayuda
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
          ...
          <div className="relative group">
              ...
              <div className={`absolute top-full left-0 mt-2 w-72 bg-white border border-slate-200 shadow-xl rounded-lg p-3 text-xs text-slate-600 z-50 transition-all origin-top-left ${showHelp ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
                  ...
                  <ul className="space-y-1.5 list-disc pl-3">
                      <li>Los eventos grises son <strong>borradores</strong> (no existen en la base de datos hasta que los editas).</li>
                      <li><strong>Hora y Lugar:</strong> Define dónde y cuándo se come.</li>
                      <li><strong>Convocados:</strong> Importante definir quiénes asisten. Si eliges grupos (ej. "Tutti"), la descripción se sugiere sola.</li>
                      <li><strong>Descripción:</strong> Ahora se completa al elegir a los convocados. Pero permite editar igualmente.</li>

                      <li><strong>Visibilidad (Ojo):</strong> Si está tachado, el evento no aparece en la Agenda de los músicos.</li>
                      <li>Los cambios se guardan <strong>automáticamente</strong> al dejar de escribir.</li>
                  </ul>
                  <div className="mt-2 pt-2 border-t border-slate-100 text-[10px] text-slate-400 text-right italic">
                      Clic afuera para cerrar
                  </div>
              </div>
              {/* Overlay transparente para cerrar al hacer clic afuera si está abierto */}
      ...
      {/* REESTRUCTURACIÓN DEL CONTENEDOR PARA STICKY HEADER */}
      <div className="flex-1 p-4 overflow-hidden flex flex-col">
        <div className="bg-white border border-slate-300 rounded-lg shadow-sm flex-1 overflow-hidden flex flex-col relative">
          ...
          {/* Este div maneja el scroll, permitiendo que el thead dentro de él sea sticky */}
          <div className="flex-1 overflow-auto">
            <table className="w-full text-left text-sm min-w-[1100px] border-collapse relative">
              <thead className="bg-slate-100 text-slate-500 uppercase font-bold text-[10px] z-40 sticky top-0 shadow-sm">
                <tr>
                  <th className="px-3 py-3 w-32 border-b border-r border-slate-200 bg-slate-100 sticky top-0 left-0 z-50">Día</th>
              ...
              <tbody className="divide-y divide-slate-100">
                    ...
                    return rows.map((row, i) => {
                      ...
                      return (
                        <tr key={row.id} className={`group transition-all duration-300 ${rowClass}`}>
                          {i === 0 && (
                            <td rowSpan={rows.length} className="px-3 py-3 font-bold text-slate-700 border-r border-slate-200 bg-white sticky left-0 align-top z-30 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                              <div className="flex items-center gap-2"><IconCalendar size={14} className="text-slate-300"/>{formattedDate}</div>
                            </td>
                          )}
                          <td className="px-3 py-2">
                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${row.servicio === "Almuerzo" ? "bg-amber-100 text-amber-700" : row.servicio === "Cena" ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}>{row.servicio}</span>
                          </td>
                          ...
                          <td className="px-1 py-1"><input type="text" placeholder="Descripción..." value={row.descripcion || ""} onChange={(e) => handleGridChange(row.originalIndex, "descripcion", e.target.value)} disabled={disableInputs} className={`w-full text-xs border rounded p-1 focus:border-indigo-500 focus:outline-none ${isTemp ? 'bg-transparent' : ''}`}/></td>
                ...
                {!grid.length && !loading && <tr><td colSpan="9" className="p-8 text-center text-slate-400 italic">No hay rango definido. Configura las fechas en la pestaña Logística.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
