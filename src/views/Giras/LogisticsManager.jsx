...
const getProviderColorClass = (provider) => {
  switch (provider) {
    ...
    case "No lleva":
      return "bg-slate-100 text-slate-400 border-slate-200 line-through decoration-slate-400 opacity-70";
    case "-":
...
const getSourceBadge = (type) => {
  ...
  return (
    <span
      className={`text-[9px] font-bold px-1 rounded border cursor-help ${config.color}`}
      title={config.title}
    >
      {type}
    </span>
...
const MultiSelectCell = ({
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (
        ...
        !e.target.closest(".ms-portal-dropdown")
      ) {
        setIsOpen(false);
      }
    };
  ...
  const toggleSelection = (val) => {
    ...
    const newSelection = current.includes(val)
  ...
  const dropdownContent = (
    <div
      className="ms-portal-dropdown fixed bg-white border border-slate-300 shadow-2xl rounded-lg flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-100"
      style={dropdownStyle}
    >
      ...
      <div className="overflow-y-auto max-h-60 p-1 space-y-0.5">
        {filteredOptions.map((opt) => {
          ...
          return (
            <div
              key={opt.val}
              onClick={() => toggleSelection(opt.val)}
              className={`px-3 py-2 text-xs cursor-pointer rounded flex items-center gap-2 ${
                isSelected
  ...
  return (
    <div className="relative w-full h-full min-h-[32px]" ref={containerRef}>
      <div
        ...
        className="w-full h-full min-h-[32px] px-2 py-1 bg-white cursor-pointer hover:bg-slate-50 flex flex-col justify-center gap-0.5 text-xs"
      >
        ...
        {selectedOptions.map((opt) => (
          <div
            key={opt.val}
            className="flex items-center justify-between bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded text-[10px] text-indigo-700 leading-tight"
          >
...
export default function LogisticsManager({ supabase, gira, onBack }) {
    ...
    try {
      ...
      const normalizedRules = (rulesData || []).map((r) => {
        ...
        const transportsList = (r.giras_logistica_reglas_transportes || [])
          ...
          .map((t) => {
            const nombre =
              t.giras_transportes?.transportes?.nombre || "Vehículo";
  ...
  const fetchRosterForLogistics = async (currentTourLocIds) => {
    ...
    const inclEnsembles =
      ...
      [];
    const inclFamilies =
      fuentes?.filter((f) => f.tipo === "FAMILIA").map((f) => f.valor_texto) ||
      [];
    const exclEnsembles =
      fuentes
    ...
    if (inclFamilies.length > 0) {
      const { data } = await supabase
        ...
        .in("instrumentos.familia", inclFamilies);
      data?.forEach((m) => ids.add(m.id));
    }
    overrides?.forEach((o) => {
      if (o.estado !== "ausente") ids.add(o.id_integrante);
    });
    ...
    if (ids.size > 0) {
      const { data } = await supabase
        .from("integrantes")
        .select(
          "id, nombre, apellido, id_localidad, instrumentos(instrumento, familia), localidades(id, id_region)"
        )
    ...
    if (exclEnsembles.length > 0) {
      const { data } = await supabase
        .from("integrantes_ensambles")
        .select("id_integrante")
        .in("id_ensamble", exclEnsembles);
      data?.forEach((r) => excludedIds.add(r.id_integrante));
    }

    const finalRoster = rosterData
      .filter((m) => {
      ...
      .map((m) => {
        ...
        if (!ov && m.instrumentos?.familia?.includes("Prod"))
          role = "produccion";
        return {
          ...m,
          rol_gira: role,
          is_local: currentTourLocIds.has(m.id_localidad),
        };
      })
  ...
  const addEmptyRow = async () => {
    ...
    const newRowPayload = {
      ...
      fecha_checkout: null,
      hora_checkout: null,
      comida_inicio_fecha: null,
      comida_inicio_servicio: null,
      comida_fin_fecha: null,
      comida_fin_servicio: null,
      prov_desayuno: null,
      prov_almuerzo: null,
      prov_merienda: null,
      prov_cena: null,
  ...
  const renderSelectionCell = (row, index) => {
    if (row.alcance === "General")
      return (
        <div className="text-xs text-slate-400 italic px-2 py-2 flex items-center">
          Aplica a todos
        </div>
      );
    let options = [];
    if (row.alcance === "Persona")
      options = catalogs.musicians.map((m) => ({
        val: m.id,
        label: `${m.apellido}, ${m.nombre}`,
      }));
    else if (row.alcance === "Localidad")
      options = catalogs.locations.map((l) => ({
        val: l.id,
        label: l.localidad,
      }));
    else if (row.alcance === "Region")
      options = catalogs.regions.map((r) => ({ val: r.id, label: r.region }));
    else if (row.alcance === "Categoria") options = CATEGORIA_OPTIONS;
    return (
      <MultiSelectCell
        options={options}
        selectedIds={row.ref_values}
        onChange={(newIds) => handleRowChange(index, "ref_values", newIds)}
      />
    );
  };
  ...
  const ServiceSelect = ({ value, onChange }) => (
    <select
      className="w-full text-[10px] bg-transparent outline-none border-none p-0.5 cursor-pointer hover:bg-slate-100 rounded"
      value={value || ""}
      onChange={(e) => onChange(e.target.value)}
    >
      <option value="">--</option>
      {SERVICIOS_COMIDA.map((s) => (
  ...
  const ProviderSelect = ({ value, onChange }) => (
    <select
      className={`w-full text-[10px] bg-transparent outline-none border-none p-1 text-center cursor-pointer rounded ${getProviderColorClass(
        value
      )}`}
      value={value || ""}
      onChange={(e) => onChange(e.target.value)}
  ...
  const EmptyCellAlert = ({ val, children, className = "" }) =>
    !val ? (
      <div
        className={`w-full h-full bg-orange-50 border border-orange-100 flex items-center justify-center text-orange-300 ${className}`}
      >
        -
      </div>
    ) : (
          ...
          <div className="bg-white border border-slate-300 rounded-lg shadow-sm overflow-x-auto pb-2">
            <table className="w-full text-left text-sm border-collapse min-w-[1700px]">
              <thead className="bg-slate-100 text-slate-600 uppercase font-bold text-[10px] border-b border-slate-300 sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="border-r border-slate-300 px-2 py-2 w-32 shrink-0 bg-slate-100 sticky left-0 z-20">
                    Criterio
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-64 shrink-0 bg-slate-100 sticky left-32 z-20 shadow-r">
                    Selección
                  </th>

                  {/* COLUMNA TRANSPORTE */}
                  <th className="border-r border-slate-300 px-2 py-2 w-48 bg-slate-200 text-slate-700 text-center">
                    Transporte
                  </th>

                  <th className="border-r border-slate-300 px-2 py-2 w-32 bg-blue-50/50 text-blue-800 border-b-2 border-b-blue-200">
                    Check-In Día
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-24 bg-blue-50/50 text-blue-800 border-b-2 border-b-blue-200">
                    Hora
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-32 bg-amber-50/50 text-amber-800 border-b-2 border-b-amber-200">
                    Check-Out Día
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-24 bg-amber-50/50 text-amber-800 border-b-2 border-b-amber-200">
                    Hora
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-48 bg-emerald-50/50 text-emerald-800 border-b-2 border-b-emerald-200">
                    Inicio Comidas
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-48 bg-red-50/50 text-red-800 border-b-2 border-b-red-200">
                    Fin Comidas
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-28 bg-slate-50/50 text-center">
                    Desayuno
                  </th>
                  <th className="border-r border-slate-300 px-2 py-2 w-28 bg-slate-50/50 text-center">
                    Almuerzo
                ...
                {rules.map((row, index) => (
                  <tr key={row.id} className="hover:bg-slate-50 group">
                    <td className="border-r border-slate-200 p-0 bg-white sticky left-0 z-10">
                      <select
                        className="w-full h-full bg-transparent outline-none text-xs font-bold text-indigo-700 px-2 py-3 cursor-pointer"
                        value={row.alcance}
                        onChange={(e) =>
                          handleRowChange(index, "alcance", e.target.value)
                        }
                      >
                    ...
                    <td className="border-r border-slate-200 p-1 text-center bg-slate-50/30 align-top">
                      ...
                      >
                        {row.transport_names &&
                        row.transport_names.length > 0 ? (
                          row.transport_names.map((name, i) => (
                            <span
                              key={i}
                              className="text-[9px] bg-white border border-slate-200 px-1.5 py-0.5 rounded shadow-sm w-full text-left truncate font-medium text-slate-700 flex items-center gap-1"
                            >
                              <IconBus size={8} className="text-slate-400" />{" "}
                              {name}
                            </span>
                          ))
                        ) : (
                          <span className="text-[9px] text-slate-400 italic w-full text-center">
                            + Configurar
                          </span>
                        )}
                      </div>
                    ...
                    <td className="border-r border-slate-200 p-1">
                      <DateInput
                        value={row.fecha_checkin}
                        onChange={(v) =>
                          handleRowChange(index, "fecha_checkin", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <TimeInput
                        value={row.hora_checkin}
                        onChange={(v) =>
                          handleRowChange(index, "hora_checkin", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <DateInput
                        value={row.fecha_checkout}
                        onChange={(v) =>
                          handleRowChange(index, "fecha_checkout", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <TimeInput
                        value={row.hora_checkout}
                        onChange={(v) =>
                          handleRowChange(index, "hora_checkout", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <div className="flex items-center gap-1 w-full">
                        <div className="w-24 shrink-0">
                          <DateInput
                            value={row.comida_inicio_fecha}
                            onChange={(v) =>
                              handleRowChange(index, "comida_inicio_fecha", v)
                            }
                          />
                        </div>
                        <div className="flex-1 min-w-[80px]">
                          <ServiceSelect
                            value={row.comida_inicio_servicio}
                            onChange={(v) =>
                              handleRowChange(
                                index,
                    ...
                    <td className="border-r border-slate-200 p-1">
                      <div className="flex items-center gap-1 w-full">
                        <div className="w-24 shrink-0">
                          <DateInput
                            value={row.comida_fin_fecha}
                            onChange={(v) =>
                              handleRowChange(index, "comida_fin_fecha", v)
                            }
                          />
                        </div>
                        <div className="flex-1 min-w-[80px]">
                          <ServiceSelect
                            value={row.comida_fin_servicio}
                            onChange={(v) =>
                              handleRowChange(index, "comida_fin_servicio", v)
                            }
                          />
                        </div>
                      </div>
                    ...
                    <td className="border-r border-slate-200 p-1">
                      <ProviderSelect
                        value={row.prov_desayuno}
                        onChange={(v) =>
                          handleRowChange(index, "prov_desayuno", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <ProviderSelect
                        value={row.prov_almuerzo}
                        onChange={(v) =>
                          handleRowChange(index, "prov_almuerzo", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <ProviderSelect
                        value={row.prov_merienda}
                        onChange={(v) =>
                          handleRowChange(index, "prov_merienda", v)
                        }
                      />
                    </td>
                    <td className="border-r border-slate-200 p-1">
                      <ProviderSelect
                        value={row.prov_cena}
                        onChange={(v) => handleRowChange(index, "prov_cena", v)}
                      />
                    </td>
            ...
            <button
              onClick={addEmptyRow}
              className="w-full py-3 bg-slate-50 hover:bg-slate-100 text-slate-500 text-xs font-bold uppercase tracking-wide flex items-center justify-center gap-2 border-t border-slate-200 transition-colors sticky left-0"
            >
        ...
        <div className="pb-10">
          <div className="flex justify-between items-end mb-2">
            <h3 className="text-sm font-bold text-emerald-800 flex items-center gap-2">
              2. Resumen Calculado por Persona
            </h3>
          ...
          <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden overflow-x-auto">
            <table className="w-full text-left text-xs min-w-[1400px]">
              {" "}
              {/* Aumentado min-w para columna extra */}
              <thead className="bg-slate-50 text-slate-500 uppercase font-bold border-b border-slate-200">
                <tr>
                  <th className="p-3 w-64 sticky left-0 bg-slate-50 z-10 border-r border-slate-200">
                    Apellido, Nombre
                  </th>

                  {/* NUEVA COLUMNA TRANSPORTE */}
                  <th className="p-3 text-center border-r border-slate-100 w-48">
                    Transporte Asignado
                  </th>

                  <th className="p-3 text-center bg-blue-50/30 border-r border-slate-100">
                    Check-In
                  </th>
                  <th className="p-3 text-center bg-amber-50/30 border-r border-slate-200 border-r-2">
                    Check-Out
                  </th>
                  <th className="p-3 text-center bg-emerald-50/30 border-r border-slate-100 w-40">
                    Inicio Comidas
                  </th>
                  <th className="p-3 text-center bg-red-50/30 border-r border-slate-200 border-r-2 w-40">
                    Fin Comidas
                  </th>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50">
                    <td className="p-3 sticky left-0 bg-white z-10 border-r border-slate-100">
                      ...
                      <div className="flex gap-1 mt-0.5">
                        {item.is_local && (
                          <span className="text-[9px] text-orange-600 bg-orange-100 px-1 rounded border border-orange-200">
                            LOCAL
                          </span>
                        )}
                        <span className="text-[9px] text-slate-400 bg-slate-100 px-1 rounded">
                          {item.rol_gira !== "musico"
                            ? item.rol_gira.toUpperCase()
                    ...
                    <td className="p-3 align-top border-r border-slate-100 bg-slate-50/10">
                      <div className="flex flex-col gap-1 items-start min-h-[20px]">
                        {item.logistics.transports &&
                        item.logistics.transports.length > 0 ? (
                          item.logistics.transports.map((t, idx) => (
                            <div
                              key={idx}
                              className="flex items-center gap-1.5 text-[10px] w-full"
                            >
                          ...
                          <span className="text-slate-300 text-[9px] italic mx-auto">
                            -
                          </span>
                        )}
                      ...
                      <EmptyCellAlert val={item.logistics.checkin}>
                        <div className="flex items-center justify-center gap-2">
                          <span className="text-blue-700 font-bold">
                            {formatDate(item.logistics.checkin)}
                          </span>
                          {getSourceBadge(item.logistics.checkin_src)}
                          <span className="text-slate-400 font-normal ml-1 border-l border-slate-300 pl-2">
                            {item.logistics.checkin_time?.slice(0, 5)}
                          </span>
                        </div>
                      </EmptyCellAlert>
                    </td>
                      ...
                      <EmptyCellAlert val={item.logistics.checkout}>
                        <div className="flex items-center justify-center gap-2">
                          <span className="text-amber-700 font-bold">
                            {formatDate(item.logistics.checkout)}
                          </span>
                          {getSourceBadge(item.logistics.checkout_src)}
                          <span className="text-slate-400 font-normal ml-1 border-l border-slate-300 pl-2">
                            {item.logistics.checkout_time?.slice(0, 5)}
                          </span>
                        </div>
                      </EmptyCellAlert>
                    </td>
                        ...
                        <div className="flex flex-col items-center leading-tight">
                          <div className="flex gap-1 items-center">
                            <span className="font-bold text-emerald-800">
                              {formatDate(item.logistics.comida_inicio)}
                            </span>
                            {getSourceBadge(item.logistics.comida_inicio_src)}
                          </div>
                          <span className="text-[9px] uppercase text-emerald-600 font-bold">
                            {item.logistics.comida_inicio_svc}
                          </span>
                        </div>
                      </EmptyCellAlert>
                    </td>
                    ...
                    <td className="p-2 text-[9px] align-middle">
                      <div className="grid grid-cols-4 gap-1">
                        <div
                          className={`text-center border p-1 rounded ${getProviderColorClass(
                            item.logistics.prov_des
                          )}`}
                        >
                          <div className="font-bold opacity-50 text-[7px] mb-0.5">
                            DES
                          </div>
                          {item.logistics.prov_des || "-"}
                        </div>
