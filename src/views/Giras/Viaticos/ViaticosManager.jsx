...
import {
  ...
  IconBus, IconInfo, IconCloudUpload, IconLoader, IconRefresh
...
export default function ViaticosManager({ supabase, giraId }) {
  ...
  const [config, setConfig] = useState({ valor_diario_base: 0, factor_temporada: 0, motivo: "", lugar_comision: "", link_drive: "" });
  ...
  const fetchViaticosData = async () => {
    ...
    try {
      ...
      let { data: detalles, error } = await supabase.from("giras_viaticos_detalle").select(`*, integrantes:id_integrante(id, nombre, apellido, dni, firma, id_instr, documentacion, docred)`).eq("id_gira", giraId).order("id");
      if(error || !detalles) {
    ...
    } catch (error) { console.error("Critical error fetching viaticos:", error); } finally { setLoading(false); }
  };
  ...
  const logisticsMap = useMemo(() => {
    ...
    summary.forEach(person => {
        ...
        transports.forEach(t => {
            ...
            if (t.subidaData) {
                ...
                if (!minSalida || dateObj < minSalida.dt) {
                    minSalida = { dt: dateObj, fecha: t.subidaData.fecha, hora: t.subidaData.hora ? t.subidaData.hora.slice(0, 5) : "00:00", lugar: t.subidaData.nombre_localidad || "Origen", transporte: nombreFinal };
                }
            }
            if (t.bajadaData) {
        ...
        if (minSalida || maxLlegada) {
            map[person.id] = {
                fecha_salida: minSalida?.fecha, hora_salida: minSalida?.hora, transporte_salida: minSalida?.transporte, lugar_salida: minSalida?.lugar,
                fecha_llegada: maxLlegada?.fecha, hora_llegada: maxLlegada?.hora, transporte_llegada: maxLlegada?.transporte, lugar_llegada: maxLlegada?.lugar
            };
        }
    });
    return map;
  ...
  const activeRows = useMemo(() => {
    return viaticosRows.map((row) => {
        ...
        const { valorDiarioCalc, subtotal, totalFinal } = calculateRow(rowWithLogistics, config);
  ...
  const candidateOptions = useMemo(() => {
    ...
    roster.forEach((p) => {
      ...
      if (!esPerfilMasivo(p) && p.estado_gira !== 'ausente' && !existingIds.has(idStr) && !seen.has(idStr)) {
        seen.add(idStr);
  ...
  const updateRow = async (id, field, value) => { const fieldKey = `${id}-${field}`; setUpdatingFields((prev) => new Set(prev).add(fieldKey)); if (successFields.has(fieldKey)) setSuccessFields((prev) => { const next = new Set(prev); next.delete(fieldKey); return next; }); const currentRow = viaticosRows.find((r) => r.id === id); if (!currentRow) return; const updatedRow = { ...currentRow, [field]: value }; setViaticosRows((prev) => prev.map((r) => (r.id === id ? updatedRow : r))); try { const payload = { [field]: value }; await supabase.from("giras_viaticos_detalle").update(payload).eq("id", id); setSuccessFields((prev) => new Set(prev).add(fieldKey)); setTimeout(() => { setSuccessFields((prev) => { const next = new Set(prev); next.delete(fieldKey); return next; }); }, 2000); } catch (err) { console.error(err); alert("Error al guardar cambio."); fetchViaticosData(); } finally { setUpdatingFields((prev) => { const next = new Set(prev); next.delete(fieldKey); return next; }); } };
  const handleDeleteRow = async (id) => { if (!confirm(`¿Eliminar de la lista de viáticos?`)) return; setDeletingRows(prev => new Set(prev).add(id)); try { await supabase.from('giras_viaticos_detalle').delete().eq('id', id); setViaticosRows(prev => prev.filter(r => r.id !== id)); } catch (err) { console.error(err); } finally { setDeletingRows(prev => { const next = new Set(prev); next.delete(id); return next; }); } };
  const handleApplyBatch = async () => { setLoading(true); try { const updates = {}; Object.keys(batchValues).forEach((key) => { if (batchValues[key] !== "" && batchValues[key] !== null && batchValues[key] !== false) updates[key] = batchValues[key]; }); if (Object.keys(updates).length === 0) { alert("No has ingresado ningún valor."); setLoading(false); return; } const selectedIds = Array.from(selection); const promises = selectedIds.map(async (integranteId) => { const row = viaticosRows.find(r => r.id_integrante === integranteId); if (!row) return; await supabase.from("giras_viaticos_detalle").update(updates).eq("id", row.id); }); await Promise.all(promises); await fetchViaticosData(); setSelection(new Set()); alert("Aplicado."); } catch (err) { console.error(err); alert("Error: " + err.message); } finally { setLoading(false); } };
  const handleAddPerson = async () => { if (!selectedToAdd) return; setLoading(true); const persona = roster.find((p) => p.id === selectedToAdd); if (!persona) { alert("Error: No se encontró la persona."); setLoading(false); return; } const { cargo, jornada } = getAutoDatosLaborales(persona); try { const { data, error } = await supabase.from("giras_viaticos_detalle").insert([{ id_gira: giraId, id_integrante: selectedToAdd, dias_computables: 0, porcentaje: 100, cargo, jornada_laboral: jornada }]).select(); if (error) throw error; setViaticosRows((prev) => [...prev, ...data]); setSelectedToAdd(null); } catch (err) { alert("Error: " + err.message); } finally { setLoading(false); } };
  ...
  const handleAddIndividuals = async () => { 
      ...
      try { 
          ...
          if (toAdd.length === 0) { alert("No se encontraron integrantes individuales pendientes."); setLoading(false); return; } 
  ...
  const updateConfig = (key, val) => { 
      ...
      saveTimeoutRef.current = setTimeout(async () => { 
          ...
          try { await supabase.from("giras_viaticos_config").update(changesToSave).eq("id_gira", giraId); } 
  ...
  const fetchPdfFromDrive = async (driveUrl) => {
      try {
          ...
          if (error) throw error;
          if (!data || !data.success) throw new Error(data?.error || "Error descargando archivo de Drive");
  ...
  const processExportList = async (dataList, folderId, options, involvedLocationIds = []) => {
      ...
      const updates = dataList.filter(r => r.id).map(row => {
          return supabase.from("giras_viaticos_detalle").update({
      ...
      if (!options.unifyFiles) {
          ...
          for (const personData of dataList) {
              ...
              if (totalLocs > 0) {
                  ...
                  const locIndex = involvedLocationIds.findIndex(id => String(id) === String(currentLocId));
                  if (locIndex !== -1) {
              ...
              if (options.viatico) {
                  ...
                  const pdfBytes = await exportViaticosToPDFForm(giraData, singleDataArray, config, "viatico");
              ...
              if (options.rendicion) {
                  ...
                  const pdfBytes = await exportViaticosToPDFForm(giraData, singleDataArray, config, "rendicion");
                  await uploadPdfToDrive(pdfBytes, `${nameSafe} - Rendición.pdf`, folderId);
              }
              if (options.destaque) {
                  setExportStatus(`${prefix} Subiendo Destaque: ${nameSafe}...`);
                  const pdfBytes = await exportViaticosToPDFForm(giraData, singleDataArray, config, "destaque");
              ...
              if (options.docComun && personData.link_documentacion) {
                  ...
                  await supabase.functions.invoke('manage-drive', {
                      body: { action: 'copy_file', sourceUrl: personData.link_documentacion, targetParentId: folderId, newName: `${nameSafe} - Documentación` }
                  });
              }
              if (options.docReducida && personData.link_docred) {
                  setExportStatus(`${prefix} Copiando DocRed: ${nameSafe}...`);
                  await supabase.functions.invoke('manage-drive', {
                      body: { action: 'copy_file', sourceUrl: personData.link_docred, targetParentId: folderId, newName: `${nameSafe} - Doc. Reducida` }
                  });
              }
          }
      } else {
          ...
          for (const personData of dataList) {
              ...
              const mergePdfBytes = async (bytes) => { try { const srcDoc = await PDFDocument.load(bytes); const copiedPages = await masterDoc.copyPages(srcDoc, srcDoc.getPageIndices()); copiedPages.forEach(page => masterDoc.addPage(page)); pagesAdded++; } catch (e) { console.error("Error merging PDF:", e); } };

              if (options.viatico) await mergePdfBytes(await exportViaticosToPDFForm(giraData, singleData, config, "viatico"));
              if (options.destaque) await mergePdfBytes(await exportViaticosToPDFForm(giraData, singleData, config, "destaque"));
              if (options.rendicion) await mergePdfBytes(await exportViaticosToPDFForm(giraData, singleData, config, "rendicion"));
              if (options.docComun && personData.link_documentacion) {
                  setExportStatus(`${prefix} Descargando Doc de ${personData.apellido}...`);
                  const bytes = await fetchPdfFromDrive(personData.link_documentacion);
                  if (bytes) await mergePdfBytes(bytes);
              }
              if (options.docReducida && personData.link_docred) {
                  setExportStatus(`${prefix} Descargando DocRed de ${personData.apellido}...`);
          ...
          if (pagesAdded > 0) {
              ...
              await uploadPdfToDrive(masterBytes, `Exportación Lote - ${giraName} - ${dateStr}.pdf`, folderId);
          } else {
  ...
  const handleExportToDrive = async (options) => {
    ...
    const hasSelection = options.viatico || options.rendicion || options.destaque || options.docComun || options.docReducida;
    if (!hasSelection) return alert("Selecciona al menos un tipo de documento.");

    const driveFolderId = config?.link_drive;
    if (!driveFolderId) {
        return alert("Error: No hay carpeta de Drive configurada. Asigna una en la cabecera.");
    }
  ...
  const handleExportLocationBatch = async (peopleArray, folderId, options, locationIdOrIds) => {
      if (!peopleArray || peopleArray.length === 0) return alert("No hay personas en esta localidad.");
      ...
      try {
          const locationIds = Array.isArray(locationIdOrIds) ? locationIdOrIds : (locationIdOrIds ? [locationIdOrIds] : []);
          const officialRulesByLoc = {};

          for (const locId of locationIds) {
             const locRule = routeRules?.find(r => r.alcance === 'Localidad' && String(r.id_localidad) === String(locId) && r.evento_subida);
             let fechaSal = null;
             let horaSal = null;
             let fechaLleg = null;
             let horaLleg = null;
             ...
             const referencePerson = peopleArray.find(p => String(p.id_localidad || 'unknown') === String(locId));
             ...
             const diasCalc = calculateDaysDiff(fechaSal, horaSal, fechaLleg, horaLleg);

             const payload = {
                 id_gira: giraId, id_localidad: locId, fecha_ultima_exportacion: new Date().toISOString(),
                 backup_fecha_salida: fechaSal, backup_hora_salida: horaSal,
                 backup_fecha_llegada: fechaLleg, backup_hora_llegada: horaLleg, backup_dias_computables: diasCalc
             };
             const existingConfig = Object.values(destaquesConfigs).find(c => String(c.id_localidad) === String(locId));
             if (existingConfig) await supabase.from("giras_destaques_config").update(payload).eq("id", existingConfig.id);
          ...
          const richData = peopleArray.map(p => {
              ...
              const diasOficiales = calculateDaysDiff(finalFechaSal, finalHoraSal, finalFechaLleg, finalHoraLleg);
              ...
              return {
                  ...p,
                  nombre: p.nombre, apellido: p.apellido, dni: p.dni, legajo: p.legajo,
                  cargo: "Agente administrativo", 
                  jornada_laboral: "Horas Cátedra",
                  link_documentacion: p.documentacion, link_docred: p.docred,
                  fecha_salida: finalFechaSal,
                  hora_salida: finalHoraSal,
                  fecha_llegada: finalFechaLleg,
                  hora_llegada: finalHoraLleg,
                  dias_computables: diasOficiales,
                  valorDiarioCalc: 0, subtotal: 0, totalFinal: 0
              };
          });
                ...
                <div className="px-6 pb-4 flex flex-col gap-4">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                        <div className="flex gap-2 flex-wrap">
                            ...
                            <button onClick={handleAddIndividuals} disabled={loading || rosterLoading} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors shadow-sm disabled:opacity-50 relative">
                    ...
                    <div className="flex items-center gap-4 text-sm bg-slate-50 p-2 rounded-lg border border-slate-100">
                      <div className="flex items-center gap-2 border-r border-slate-200 pr-4">
                        ...
                        {latestGlobalValue > config.valor_diario_base && (<button onClick={() => updateConfig('valor_diario_base', latestGlobalValue)} className="text-indigo-400 hover:text-indigo-600 ml-1" title={`Actualizar a último valor conocido: $${latestGlobalValue}`}><IconRefresh size={14} /></button>)}
                        </div>
                        <div className="flex items-center gap-1 px-2 py-1 rounded border border-amber-100 bg-white shadow-sm cursor-pointer hover:bg-amber-50" onClick={() => updateConfig("factor_temporada", config.factor_temporada === 0.3 ? 0 : 0.3)}>
                            <div className={`w-3 h-3 rounded-sm border flex items-center justify-center ${config.factor_temporada === 0.3 ? 'bg-amber-500 border-amber-600' : 'border-slate-300'}`}>
                                {config.factor_temporada === 0.3 && <IconCheck size={10} className="text-white" strokeWidth={4} />}
                            </div>
                            <span className={`text-xs font-bold ${config.factor_temporada === 0.3 ? 'text-amber-700' : 'text-slate-400'}`}>TEMP (30%)</span>
                        </div>
                      </div>
      ...
      {notification && (
        <div className="fixed bottom-6 right-6 z-[100] animate-in slide-in-from-bottom-4 fade-in duration-300">
          <div className="bg-slate-800 text-white px-4 py-3 rounded-lg shadow-xl flex items-center gap-3">
            <div className="bg-green-500 rounded-full p-1 text-slate-900"><IconCheck size={14} strokeWidth={3} /></div>
