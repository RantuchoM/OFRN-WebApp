...
const CurrencyInput = ({
  ...
  const handleFocus = (e) => {
    ...
    const rawVal =
      value === 0 || value === "0" || value === null ? "" : String(value);
...
export default function ViaticosTable({
  ...
  const getInputClass = (
    rowId,
    fieldName,
    defaultBgClass = "bg-transparent",
    defaultTextClass = "text-slate-700",
  ) => {
          ...
          <table className="w-full text-sm text-left border-separate border-spacing-0">
            <thead className="text-slate-500 font-bold uppercase text-[10px]">
              <tr>
                ...
                <th className="px-2 py-3 text-center min-w-[120px] sticky top-0 z-30 bg-indigo-50/5 text-slate-600 border-b border-r border-slate-200">
                  Salida
                </th>
                <th className="px-2 py-3 text-center min-w-[120px] sticky top-0 z-30 bg-indigo-50/5 text-slate-600 border-b border-r border-slate-200">
                  Llegada
                </th>
                <th className="px-1 py-3 text-center w-10 sticky top-0 z-30 bg-indigo-50/5 text-slate-600 border-b border-r border-slate-200">
                  Días
                </th>
                ...
                {showBackup && (
                  <>
                    ...
                    <th className="px-2 py-3 text-center w-10 sticky top-0 z-30 bg-amber-50 text-amber-800 border-b border-r border-amber-100">
                      Días
                    </th>
                  </>
                )}
                ...
                {showTransport && (
                  <>
                    <th className="px-2 py-3 text-center text-blue-700 w-24 sticky top-0 z-30 bg-blue-50 border-b border-blue-100">
                      Medios
                    </th>
                ...
                {(showExpenses || showRendiciones) &&
                  FINANCIAL_COLS.map((col, idx) => (
                    <th
                      key={idx}
                      className={`px-2 py-3 text-right min-w-[100px] sticky top-0 z-30 border-b 
                        ${
                          showExpenses && showRendiciones
              ...
              {rows.map((row) => {
                ...
                return (
                  ...
                  >
                    {/* CHECKBOX */}
                    <td
                      className={`px-3 py-2 text-center border-b border-r border-slate-100 sticky left-0 z-20 ${stickyBgClass}`}
                    >
                      <input
                        type="checkbox"
                        checked={isSelected}
                      ...
                      <>
                        ...
                        <td className={cellClass}>
                          <input
                            ...
                            onBlur={(e) => {
                              if (
                                e.target.value !== (row.jornada_laboral || "")
                              )
                                onUpdateRow(
                                  row.id,
                                  "jornada_laboral",
                        ...
                        <td className="px-2 py-2 border-b border-r border-slate-200">
                          <div className="flex flex-col gap-1 text-[9px]">
                            <label className="text-slate-500">
                              <input
                                type="checkbox"
                                checked={row.check_patente_particular}
                                onChange={(e) =>
                                  onUpdateRow(
                                    row.id,
