...
const CreateParticellaModal = ({
  ...
  useEffect(() => {
    const instrName = instrumentList.find(
      (i) => i.id === selectedInstr
    )?.instrumento;
...
const ParticellaSelect = ({
  ...
  if (disabled)
    return (
      <div
        className="w-full h-full min-h-[24px] px-1 text-[10px] border border-transparent flex items-center justify-center text-slate-600 bg-transparent truncate cursor-default"
        title={selectedOption?.nombre_archivo}
      >
        {selectedOption ? (
          <span>
            {selectedOption.nombre_archivo ||
              selectedOption.instrumentos?.instrumento}
            {showCountLabel && (
  ...
  const filteredOptions = options.filter(
    (o) =>
      ...
      (o.instrumentos?.instrumento || "")
        .toLowerCase()
        .includes(search.toLowerCase())
  );

  const recommendedOptions = filteredOptions.filter(
    (o) => o.id_instrumento === preferredInstrumentId
  );
  ...
  const renderOption = (opt) => {
    ...
    return (
      <button
        ...
        className={`w-full text-left px-2 py-1 text-[10px] rounded hover:bg-indigo-50 flex items-center justify-between group ${
          value === opt.id
      ...
      >
        ...
        {value === opt.id && (
          <IconCheck size={10} className="text-indigo-600 shrink-0 ml-1" />
        )}
      </button>
    );
  };
  ...
  return (
    <div className="relative w-full h-full" ref={containerRef}>
      <button
        ...
        className={`w-full h-full min-h-[24px] text-left px-1 text-[10px] border rounded transition-colors flex items-center justify-between gap-0.5 ${
          value
          ...
          {selectedOption ? (
            <>
              {selectedOption.nombre_archivo ||
                selectedOption.instrumentos?.instrumento}
              {showCountLabel && (
                <span className="text-indigo-900 ml-1 font-extrabold">
                  [x{currentAssignedCount}]
                </span>
              )}
            </>
        ...
        <div className="absolute top-full left-0 z-50 w-56 bg-white border border-slate-200 rounded shadow-xl mt-0.5 overflow-hidden flex flex-col max-h-60 animate-in fade-in zoom-in-95">
          <div className="p-1 border-b border-slate-50 bg-slate-50 sticky top-0 flex flex-col gap-1">
            <input
              type="text"
              autoFocus
              className="w-full px-2 py-1 text-[10px] border border-slate-200 rounded outline-none focus:border-indigo-400"
              placeholder="Buscar..."
              value={search}
            ...
            <button
              ...
              className="w-full text-left px-2 py-1.5 text-[10px] bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded border border-indigo-100 flex items-center gap-2 font-bold transition-colors"
            >
          ...
          <div className="overflow-y-auto flex-1 p-1">
            <button
              ...
              className="w-full text-left px-2 py-1 text-[10px] text-slate-400 hover:bg-red-50 hover:text-red-600 rounded flex items-center gap-2 mb-1"
            >
              <IconX size={8} /> Quitar Asignación
            </button>
            {filteredOptions.length === 0 && (
              <div className="text-[10px] text-slate-400 p-2 text-center italic">
                No hay particellas
              </div>
            )}
            {recommendedOptions.length > 0 && (
              <>
                <div className="text-[8px] font-bold text-slate-400 uppercase tracking-wider px-2 py-1 bg-slate-50 mt-1 mb-0.5 rounded">
                  Sugeridos
                </div>
            ...
            {otherOptions.length > 0 && recommendedOptions.length > 0 && (
              <div className="text-[8px] font-bold text-slate-400 uppercase tracking-wider px-2 py-1 mt-1 mb-0.5">
                Otros
              </div>
            )}
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl h-[80vh] flex flex-col border border-slate-200">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-lg">
          <h3 className="font-bold text-slate-800 flex items-center gap-2 text-lg">
            <IconHistory className="text-indigo-600" /> Historial de Seating
            (Cuerdas)
          </h3>
          <button
            onClick={onClose}
                  ...
                  <tr
                    ...
                    className={
                      idx % 2 === 0
                        ...
                        : "bg-slate-50/30 hover:bg-indigo-50/30"
                    }
                  >
...
// --- GESTOR DE CUERDAS CON DRAG AND DROP AVANZADO ---
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-4 border border-slate-200">
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-[10px] text-slate-400 italic">
            * Solo se importarán los músicos que estén convocados en la gira
            actual.
          </p>
...
const GlobalStringsManager = ({
  ...
  // 1. Filtrado de seguridad: Solo músicos que siguen estando en el roster actual
  const validMusicianIds = useMemo(
    () => new Set(roster.map((m) => m.id)),
    [roster]
  );

  // 2. Preparar datos procesados (items filtrados para evitar "fantasmas")
  const displayContainers = useMemo(() => {
    return containers.map((c) => ({
      ...c,
      validItems:
        c.items?.filter((i) => validMusicianIds.has(i.id_musico)) || [],
    }));
  }, [containers, validMusicianIds]);

  // 3. Calcular disponibles
  const stringMusicians = useMemo(
    () => roster.filter((m) => ["01", "02", "03", "04"].includes(m.id_instr)),
    [roster]
  );
  const assignedIds = new Set();
  displayContainers.forEach((c) =>
    c.validItems.forEach((i) => assignedIds.add(i.id_musico))
  );
  ...
  const deleteContainer = async (id) => {
    if (readOnly) return;
    if (!confirm("¿Eliminar este grupo y liberar sus músicos?")) return;
  ...
  const saveEditing = async (id) => {
    // Intenta guardar nombre y capacidad (si la columna existe en BD)
    await supabase
      .from("seating_contenedores")
      .update({
        nombre: editName,
        capacidad: editCap ? parseInt(editCap) : null,
      })
      .eq("id", id);
    setEditingId(null);
    onUpdate();
  ...
  const addMusician = async (containerId, musicianId, targetIndex = -1) => {
    if (readOnly) return;
    const isAlreadyAssigned = containers.some((c) =>
      c.items.some((i) => i.id_musico === musicianId)
    );
    ...
    // Reordenar localmente y guardar
    ...
    await updateOrderInDB(newItems);
    onUpdate();
  };
  ...
  const handleReorder = async (
    ...
    const sourceContainer = displayContainers.find(
    ...
    if (sourceContainerId != targetContainerId) {
      await supabase
        ...
        .eq("id", itemId);
      itemToMove.id_contenedor = targetContainerId;
    }
    ...
    if (sourceContainerId == targetContainerId) {
      targetItems = targetItems.filter((i) => i.id != itemId);
    }
    ...
    await updateOrderInDB(targetItems);
    onUpdate();
  };

  const removeMusician = async (itemId) => {
    if (readOnly) return;
    await supabase.from("seating_contenedores_items").delete().eq("id", itemId);
    onUpdate();
  };
  ...
  const handleImportSeating = async (sourceProgramId, replaceCurrent) => {
    ...
    try {
      ...
      for (const srcCont of sourceContainers) {
        ...
        const itemsInThisContainer = sourceItems.filter(
          (i) => i.id_contenedor === srcCont.id
        );
        const itemsToInsert = itemsInThisContainer
          .filter((item) => validMusicianIdsSet.has(item.id_musico))
          .map((item, idx) => ({
  ...
  const handleDragOver = (e, containerId, itemId = null) => {
    if (readOnly) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleDrop = async (e, targetContainerId, targetItemId = null) => {
    ...
    if (targetContainer && targetItemId) {
      targetIndex = targetContainer.validItems.findIndex(
        (i) => i.id === targetItemId
      );
    }

    if (type === "NEW") {
      await addMusician(targetContainerId, id, targetIndex);
    } else if (type === "MOVE") {
      await handleReorder(
        id,
        sourceContainerId,
  ...
  return (
    <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 mb-4 animate-in fade-in shrink-0">
        ...
        {!readOnly && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-slate-400 italic mr-2 hidden sm:inline">
              Arrastra para reordenar o asignar
            </span>
            <button
      ...
      <div className="grid grid-cols-12 gap-4 h-[350px]">
        ...
        <div
          className={`${
            readOnly ? "col-span-12" : "col-span-9"
          } grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto content-start pr-1`}
        >
          {displayContainers.map((c) => (
            <div
              key={c.id}
              onDragOver={(e) => handleDragOver(e, c.id, null)}
              onDrop={(e) => handleDrop(e, c.id, null)}
              className={`bg-white border rounded-lg shadow-sm flex flex-col h-fit transition-all duration-200 ${
                dragOverContainerId === c.id && dragOverItemId === null
              ...
              <div className="p-1.5 border-b border-slate-100 flex justify-between items-center bg-indigo-50/30 rounded-t-lg min-h-[32px]">
                {editingId === c.id ? (
                  <div className="flex items-center gap-1 w-full">
                    <input
                      className="w-full text-[10px] border border-indigo-300 rounded px-1 py-0.5 focus:outline-none focus:border-indigo-500"
                      value={editName}
                    ...
                    <input
                      type="number"
                      className="w-10 text-[10px] border border-indigo-300 rounded px-1 py-0.5 text-center"
                      value={editCap}
                  ...
                  <>
                    <div className="flex items-center gap-1 overflow-hidden">
                      ...
                      {c.capacidad && (
                        <span
                          className="text-[9px] text-slate-400 bg-slate-100 px-1 rounded-full border border-slate-200"
                          title="Capacidad / Ocupación"
                        >
                          {c.validItems.length}/{c.capacidad}
                        </span>
                      )}
                      <button
                        onClick={() => startEditing(c)}
                        // CAMBIOS AQUÍ: Quitamos opacity-0 y group-hover... y oscurecemos el color
                        className="text-slate-400 hover:text-indigo-600 transition-colors ml-1 p-1"
                        title="Editar nombre y cupo"
                      >
                        <IconEdit size={12} />{" "}
                        {/* Aumenté un poquito el tamaño también */}
                      </button>
                    </div>
                    {!readOnly && (
                      <button
              ...
              <div className="p-1 space-y-0.5 min-h-[40px]">
                {c.validItems.map((item) => (
                  <div
                    ...
                    className={`flex items-center gap-1.5 p-1 border rounded text-[10px] group transition-colors cursor-grab active:cursor-grabbing ${
                      dragOverItemId === item.id
                        ? "border-t-2 border-t-indigo-500 bg-indigo-50 mt-1"
    ...
    return repertoireBlocks.flatMap((block) =>
      block.repertorio_obras.map((ro) => {
        ...
        return {
          id: ro.id,
          obra_id: ro.obras.id,
          link: ro.obras.google_drive_folder_id,
          title: title,
          composer: compName,
                    ...
                    >
                      <div className="flex flex-col gap-0.5 items-center w-full pb-1">
                        <div className="flex items-center gap-1 opacity-70 hover:opacity-100 transition-opacity">
                          ...
                          {obra.link && (
                            <a
                              href={`https://drive.google.com/drive/folders/${obra.link}`}
                              target="_blank"
                      ...
                      <td className="p-2 sticky left-0 bg-white group-hover:bg-indigo-50/30 border-r border-slate-200 z-20 pl-4 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                        <div className="flex flex-col">
                          <span className="font-bold text-indigo-900 text-sm flex items-center gap-2">
                            <IconLayers size={14} /> {c.nombre}
                          </span>
                          <div
                            className="text-[9px] text-slate-500 mt-0.5 truncate max-w-[200px]"
                            title={c.items
                              .map((i) => i.integrantes?.apellido)
                              .join(", ")}
                          >
                            {c.items.length} músicos...
                          </div>
                        </div>
                      </td>
                      {obras.map((obra) => {
                        ...
                        return (
                          <td
                            key={`${c.id}-${obra.id}`}
                            className="p-1 border-l border-slate-100 relative min-w-[140px] bg-slate-50/30"
                          >
                            <ParticellaSelect
                      ...
                      {obras.map((obra) => {
                        ...
                        return (
                          <td
                            ...
                            className="p-1 border-l border-slate-100 relative min-w-[140px]"
                          >
                            <ParticellaSelect
                              options={availableParts}
                              value={currentVal}
