...
const AdvancedImportModal = ({ supabase, currentGiraId, onClose, onImport }) => {
  ...
  const handleSelectProgram = async (prog) => {
    ...
    try {
      ...
      const validBlocks = (data || []).filter(b => b.repertorio_obras.length > 0);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl border border-slate-200 h-[80vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-50 rounded-full">
            <IconX size={24} />
          </button>
        </div>
          ...
          {step === 1 && (
            <div className="h-full flex flex-col p-6 max-w-2xl mx-auto">
              ...
              <div className="flex-1 overflow-y-auto bg-white rounded-lg border border-slate-200 shadow-sm">
                {loading ? (
                  <div className="p-8 text-center text-slate-400"><IconLoader className="animate-spin inline mr-2"/> Cargando giras...</div>
                ) : filteredPrograms.length === 0 ? (
                  <div className="p-8 text-center text-slate-400 italic">No se encontraron giras.</div>
                ) : (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xs text-slate-500 flex gap-2">
                            <span className="bg-slate-100 px-1.5 rounded text-slate-600 font-mono">{prog.nomenclador}</span>
                            <span>{prog.mes_letra}</span>
                          </div>
                        </div>
                        <IconChevronRight size={18} className="text-slate-300 group-hover:text-indigo-400" />
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div className="h-full flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-slate-200">
              ...
              <div className="w-full md:w-1/3 bg-white flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                  {loadingBlocks ? (
                    <div className="p-4 text-center"><IconLoader className="animate-spin inline text-indigo-500"/></div>
                  ) : blocks.length === 0 ? (
                    <div className="p-4 text-center text-slate-400 text-xs italic">Esta gira no tiene obras cargadas.</div>
                  ) : (
                    blocks.map(block => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isSelected && <IconArrowRight size={16} className="text-indigo-500" />}
                        </button>
                      )
                    })
                  )}
              ...
              <div className="flex-1 bg-slate-50 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  {!selectedBlockId ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-60">
                      ...
                      <p className="mt-2 text-sm">Selecciona un bloque para ver sus obras</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {selectedBlockObj?.repertorio_obras?.map((item, idx) => (
                        <div key={item.id} className="bg-white p-3 rounded border border-slate-200 flex gap-3 items-center">
                          <div className="font-mono text-slate-300 text-xs w-6 text-center">{idx + 1}</div>
                          <div>
                ...
                <div className="p-4 bg-white border-t border-slate-200 flex justify-end gap-3">
                  ...
                  >
                    {importing ? <IconLoader className="animate-spin" size={16} /> : <IconCheck size={16} />}
                    Importar Bloque
                  </button>
                </div>
              </div>
...
export default function ProgramRepertoire({ supabase, program, onBack }) {
      ...
      if (program?.tipo === "Ensamble") {
        ...
        if (!error && coordData) {
          ...
          const isCoordinator = fuentes.some(
            (f) => f.tipo === "ENSAMBLE" && myCoordinatedEnsembles.has(f.valor_id)
          );
    ...
    try {
      ...
      if (!data || data.length === 0) {
         ...
         const { data: newBlock, error: createError } = await supabase
           .from("programas_repertorios")
           .insert([{ id_programa: program.id, nombre: "Repertorio", orden: 0 }])
         ...
         const sortedData = data.map(bloque => ({
            ...bloque,
            repertorio_obras: (bloque.repertorio_obras || []).sort((a,b) => a.orden - b.orden)
         }));
    ...
    try {
      ...
      if (!targetRepoId) {
         const { data: newB } = await supabase
            .from("programas_repertorios")
            .insert([{ id_programa: program.id, nombre: sourceBlock.nombre || "Repertorio" }])
            .select()
            .single();
      ...
      const currentMaxOrder = repertorios[0]?.repertorio_obras?.length 
        ? Math.max(...repertorios[0].repertorio_obras.map(o => o.orden)) 
        : 0;

      // 3. Preparar payload
      const sourceWorks = sourceBlock.repertorio_obras.sort((a,b) => a.orden - b.orden);
      ...
      // 4. Insertar
      const { error } = await supabase.from("repertorio_obras").insert(newRows);
      if (error) throw error;
  ...
  if (!program) return <div className="p-10 text-center"><IconLoader className="animate-spin"/></div>;
    ...
    <div className="flex flex-col h-full bg-slate-100 animate-in fade-in">
      ...
      <div className="bg-white p-4 border-b border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button onClick={handleBack} className="text-slate-400 hover:text-indigo-600 text-sm font-bold flex items-center gap-1 shrink-0">
            <IconArrowLeft size={16} />
            {activeTab !== "repertoire" ? "Volver al Repertorio" : "Volver a Programas"}
          </button>
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg items-center">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === "my_parts" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
          >
          ...
          {program.google_drive_folder_id && (
            <>
              ...
              <a href={`https://drive.google.com/drive/folders/${program.google_drive_folder_id}`} target="_blank" rel="noreferrer" className="px-2 py-1.5 rounded-md text-slate-500 hover:text-green-700 hover:bg-white transition-all">
      ...
      <div className="flex-1 overflow-hidden relative">
        {activeTab === "repertoire" && (
          <div className="h-full overflow-y-auto p-4">
            {loadingRepo ? (
               <div className="flex justify-center p-10"><IconLoader className="animate-spin text-indigo-500"/></div>
            ) : (
              <>
