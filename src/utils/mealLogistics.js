...
/** Tipos canÃÂÃÂÃÂÃÂ³nicos (agrupan todos los eventos de comida). */
export const MEAL_SERVICES = ["Desayuno", "Almuerzo", "Merienda", "Cena"];

/**
 * Sufijos automÃÂÃÂÃÂÃÂ¡ticos frecuentes al final de la descripciÃÂÃÂÃÂÃÂ³n de una comida
 * (convocados / placeholders). Usados para extraer el detalle de subcategorÃÂÃÂÃÂÃÂ­a.
 */
...
/**
 * BebÃÂÃÂÃÂÃÂ© / menor en cuna (`ocupa_cama: false` en rooming): no consume, no cuenta en comidas.
 */
...
/** ÃÂÃÂÃÂÃÂ¿El integrante estÃÂÃÂÃÂÃÂ¡ en cuna segÃÂÃÂÃÂÃÂºn `asignaciones_config` de las habitaciones? */
...
/** CategorÃÂÃÂÃÂÃÂ­a de tipos de evento de comida en `tipos_evento` / `categorias_tipos_eventos`. */
export const MEAL_CATEGORY_ID = 4;

/**
 * CategorÃÂÃÂÃÂÃÂ­a Catering (hermana de Comidas). Seed tÃÂÃÂÃÂÃÂ­pico id 9; preferir nombre.
...
/** Slot sintÃÂÃÂÃÂÃÂ©tico de matriz/filtro para eventos de categorÃÂÃÂÃÂÃÂ­a Catering. */
export const CATERING_SERVICE = "Catering";

/** IDs canÃÂÃÂÃÂÃÂ³nicos fijos (compatibilidad / cobertura de reglas). */
...
/**
 * Colores de servicio de comida (fuente ÃÂÃÂÃÂÃÂºnica para matriz, reporte, logÃÂÃÂÃÂÃÂ­stica e impresiÃÂÃÂÃÂÃÂ³n).
 * - `tag` / `card`: UI interactiva (texto coloreado sobre fondo suave).
 * - `reportTag`: PDF/reporte (texto negro sobre fondo resaltado).
 * - `rowHover` / `date` / `icon`: filas de eventos en logÃÂÃÂÃÂÃÂ­stica.
 * - `print`: hex para el CSS crÃÂÃÂÃÂÃÂ­tico de PrintWrapper (sin Tailwind en la ventana de impresiÃÂÃÂÃÂÃÂ³n).
 */
...
/** ÃÂÃÂÃÂÃÂ¿El id es un tipo canÃÂÃÂÃÂÃÂ³nico puro (sin detalle)? */
...
export function mealBaseFromTypeName(nombre) {
  ...
  if (!raw) return null;
  // Primera palabra (corta en espacio o parÃÂÃÂÃÂÃÂ©ntesis sin espacio)
...
/**
 * ÃÂÃÂÃÂÃÂ¿CategorÃÂÃÂÃÂÃÂ­a Catering (no Comidas / id 4)?
...
/**
 * ÃÂÃÂÃÂÃÂ¿Este evento (o fila con tipos_evento) es de comida (categorÃÂÃÂÃÂÃÂ­a Comidas)?
 * Prioriza id_categoria = 4; fallback a ids 7ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ10 o nombre agrupable.
 * Catering NO cuenta aquÃÂÃÂÃÂÃÂ­ (usar `isCateringEvent` / `isMealRelatedEvent`).
 */
...
/** Comida (cat 4 / D-A-M-C) o Catering ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ lo que entra al gestor de comidas. */
...
/** Comidas sin tags FIMBA y con audiencia OFRN (orquesta come; no ÃÂ«NadieÃÂ»). */
 ...
 * Destildar siempre estÃÂ¡ permitido (incluye Tutti / Nadie) y la selecciÃÂ³n puede
 * quedar vacÃÂ­a: no hay mÃÂ­nimo de un chip. Encender Tutti o Nadie sigue siendo
...
/** ÃÂ¿La fila tiene al menos un tag de artista FIMBA? */
export function mealRowHasArtistTags(row) {
  return filterFimbaPropuestasForMeals(row?.propuestas || []).some(
    (p) => p?.id != null,
  );
}
...
/**
 * ÃÂ¿La fila pasa el multi-select de Artista (ids + Solo orquesta / Sin artistas)?
 * VacÃÂ­o = no filtra.
 */
export function mealRowMatchesArtistFilter(row, artistIds) {
  ...
  const ids = filterFimbaPropuestasForMeals(row?.propuestas || [])
...
/** Opciones del filtro Artista: Â«Solo orquestaÂ» siempre primero + tags presentes. */
 ...
 * Orden: fecha ASC ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ servicio (D/A/M/C/Catering = MEAL_SERVICE_ORDER) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ
 * hora_inicio ASC ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ id. Misma lÃÂÃÂÃÂÃÂ³gica que el walk diario + Catering intercalado
 ...
 * Ej: ("Merienda", "a bordo") ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "Merienda a bordo"
 *     ("Almuerzo", "(Vianda)") ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "Almuerzo (Vianda)"
 */
...
export function stripMealAutoDescriptionSuffix(plain, extraLabels = []) {
  ...
  // Quitar una o mÃÂÃÂÃÂÃÂ¡s etiquetas conocidas al final (con + / espacios).
  let guard = 0;
  while (text && guard < 24) {
    guard += 1;
    let matched = false;
...
/**
 * Extrae el "otros detalles" del servicio a partir de la descripciÃÂÃÂÃÂÃÂ³n.
 * "Merienda a bordo Solo alojados" + base Merienda ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "a bordo"
 * "Almuerzo (Vianda)" ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "(Vianda)"
 */
...
/** Etiqueta completa: nombre real del tipo de evento (o legacy desde descripciÃÂÃÂÃÂÃÂ³n). */
...
/**
 * Clave ordenable fecha+servicio (misma lÃÂÃÂÃÂÃÂ³gica que la matriz de comidas).
...
/** LÃÂÃÂÃÂÃÂ­mites de cobertura calculados en logÃÂÃÂÃÂÃÂ­stica por persona. */
...
/**
 * ÃÂÃÂÃÂÃÂ¿La persona tiene cobertura de comida para este slot (fecha + servicio)?
 * Usa comida_inicio / comida_fin de su regla logÃÂÃÂÃÂÃÂ­stica (por persona, categorÃÂÃÂÃÂÃÂ­a, etc.).
 */
export function isPersonEligibleForMealSlot(
  ...
  // BebÃÂÃÂÃÂÃÂ© en cuna: no consume, fuera de cualquier criterio de convocados.
  ...
  // Convocatoria explÃÂÃÂÃÂÃÂ­cita por residencia (LOC:): cuenta a quienes viven
  ...
  // Catering usa slot sintÃÂÃÂÃÂÃÂ©tico 4 (despuÃÂÃÂÃÂÃÂ©s de Cena). La cobertura logÃÂÃÂÃÂÃÂ­stica
  // solo define DÃÂÃÂ¢ÃÂÃÂÃÂÃÂC; comparar por slot dejarÃÂÃÂÃÂÃÂ­a fuera a todos los viajeros.
  // Criterio: dÃÂÃÂÃÂÃÂ­a calendario dentro de comida_inicio..comida_fin (inclusive).
 ...
 * Si `selectedGrupos` es un array (aunque vacÃÂÃÂÃÂÃÂ­o), es la fuente de verdad del Manager
 * ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ asÃÂÃÂÃÂÃÂ­ vaciar grupos no queda ÃÂÃÂ¢ÃÂÃÂÃÂÃÂpegadoÃÂÃÂ¢ÃÂÃÂÃÂÃÂ al embed `eventos_grupos` hasta el save.
 * @param {object} row
 * @returns {number[]}
 */
...
/**
 * Campos de fila cuya ediciÃÂÃÂÃÂÃÂ³n cambia elegibilidad / pax / deducciÃÂÃÂÃÂÃÂ³n / aviso de turno.
...
/**
 * Clave locaciÃÂÃÂÃÂÃÂ³n-aware: fecha + servicio + id_locacion.
 * Conservada por compatibilidad; la **deducciÃÂÃÂÃÂÃÂ³n** orquestaÃÂÃÂ¢ÃÂÃÂÃÂÃÂgrupo usa `mealTurnoKey`
 ...
 * **sin** locaciÃÂÃÂÃÂÃÂ³n. Usado para:
 * - deducciÃÂÃÂÃÂÃÂ³n orquestaÃÂÃÂ¢ÃÂÃÂÃÂÃÂgrupo (grupo tiene prioridad; orquesta resta esos IDs)
 * - sobre-inclusiÃÂÃÂÃÂÃÂ³n (persona en ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¥2 comidas del turno)
 */
 ...
 *
 * @param {Array} rows ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ filas de comida (ignora isTemp)
 * @param {(row: object) => Array} getEligiblePeople ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ elegibles OFRN por fila
 * @returns {{
 ...
 *
 * @param {Array} events ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ eventos meal ya filtrados (vista)
 * @returns {Array<{
 *   turnoKey: string,
 *   fecha: string,
 *   servicio: string,
 ...
 * Si hay varios registros (sobre-inclusiÃÂÃÂÃÂÃÂ³n residual), prioriza P > A > vacÃÂÃÂÃÂÃÂ­o.
 *
...
/**
 * Default true (histÃÂ³rico): solo `requiere_comidas === false` excluye
 * (ÃÂ«no se les provee comidaÃÂ» en HotelerÃÂ­a / Artistas / Rider).
 */
...
/** CatÃÂ¡logo / tags visibles en superficies Comidas (excluye sin comida). */
 ...
 * Total = ÃÂÃÂÃÂÃÂ£ dietas + residualArt (= `fimbaArtistMealPax` cuando hay cupo planificado).
 *
...
/**
 * Detalle libre del nombre de tipo (todo despuÃÂÃÂÃÂÃÂ©s del tipo base).
 * "Merienda a bordo" ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "a bordo"; "Almuerzo" ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ "".
 */
...
/** Tipos de categorÃÂÃÂÃÂÃÂ­a Comidas (id 4) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ editor de tipos D/A/M/C. */
 ...
 * Comidas/Catering: nunca inventar fin desde inicio ÃÂ¢ÃÂÃÂ vacÃÂÃÂ­o ÃÂ¢ÃÂÃÂ null.
 * Otros tipos: conserva el fallback histÃÂÃÂ³rico (fin = inicio si falta).
 */
 ...
 * No exige servicios anteriores al primer tagged ni posteriores al ÃÂÃÂºltimo.
 *
...
function isFimbaPropuestaCancelled(p) {
  const estado = String(p?.estado || "").toLowerCase();
  return estado === "cancelado" || estado === "rechazado";
}

/**
 * Check-in/out efectivos de una propuesta (evento vinculado o espejo).
 * @returns {{ checkin: string|null, checkout: string|null, early: boolean, late: boolean }}
 */
export function resolveFimbaPropuestaStayForCoverage(propuesta) {
  if (!propuesta) {
    return { checkin: null, checkout: null, early: false, late: false };
  }
  const checkin = stayDateFromEventOrMirror(propuesta, "checkin");
  const checkout = stayDateFromEventOrMirror(propuesta, "checkout");
  return {
    checkin,
    checkout,
    early:
      propuesta.checkin_early === true || propuesta.checkin_early === "true",
    late:
      propuesta.checkout_late === true || propuesta.checkout_late === "true",
  };
}
 ...
 *
 * Ventana preferida = estadía Hotelería (check-in → check-out vía
 * `id_evento_checkin`/`checkout` o espejo `checkin_at`/`checkout_at`), con
 ...
 *   resolveServicio?: (row: object) => string|null,
 *   propuestas?: Array, // catálogo edición (con stay); incluye artistas sin tags
 * }} [opts]
 * @returns {{ artistaId: string, artistaNombre: string, first: object|null, last: object|null, presentCount: number, missing: Array<{ fecha: string, servicio: string }>, ok: boolean, windowSource?: 'stay'|'tagged'|'none', checkinAt?: string|null, checkoutAt?: string|null, note?: string|null, skipped?: boolean }[]}
 */
export function findFimbaArtistMealCoverageGaps(mealRows = [], opts = {}) {
  ...
  const seedArtist = (p) => {
    if (!p?.id) return null;
    if (!fimbaPropuestaRequiereComidas(p)) return null;
    if (isFimbaPropuestaCancelled(p)) return null;
    ...
    if (!byArtist.has(id)) {
      ...
      const entry = byArtist.get(id);
      if (p.nombre) entry.nombre = p.nombre;
      const cur = entry.propuesta;
      const catalogRicher =
        (p.checkin_at != null ||
          p.checkout_at != null ||
          p.id_evento_checkin != null ||
          p.evento_checkin != null) &&
        !(
          cur?.checkin_at != null ||
          cur?.checkout_at != null ||
          cur?.id_evento_checkin != null ||
          cur?.evento_checkin != null
        );
      if (!cur || catalogRicher) entry.propuesta = p;
    }
    return byArtist.get(id);
  };

  for (const p of opts.propuestas || []) {
    seedArtist(p);
  }
  ...
  for (const row of mealRows || []) {
    ...
    for (const p of props) {
      const artist = seedArtist(p);
      if (!artist) continue;
      artist.slots.push({ fecha, servicio, slotKey });
    }
  }
  ...
  for (const artist of byArtist.values()) {
    ...
    const stay = resolveFimbaPropuestaStayForCoverage(artist.propuesta);
    let expected = [];
    let first = null;
    let last = null;
    ...
    if (stay.checkin && stay.checkout) {
      expected = enumerateExpectedAmcSlotsForStay(
        stay.checkin,
        stay.checkout,
        { early: stay.early, late: stay.late },
      ).filter((e) => required.includes(e.servicio));
      windowSource = "stay";
      if (expected.length) {
        first = {
          fecha: expected[0].fecha,
          servicio: expected[0].servicio,
        };
        last = {
          fecha: expected[expected.length - 1].fecha,
          servicio: expected[expected.length - 1].servicio,
        };
      } else {
        first = { fecha: stay.checkin, servicio: "Check-in" };
        last = { fecha: stay.checkout, servicio: "Check-out" };
        note =
          "Estadía sin A/M/C esperados (p. ej. mismo día sin Early/Late)";
      }
    } else if (artist.slots.length) {
      const taggedFirst = artist.slots[0];
      const taggedLast = artist.slots[artist.slots.length - 1];
      first = {
        fecha: taggedFirst.fecha,
        servicio: taggedFirst.servicio,
      };
      last = {
        fecha: taggedLast.fecha,
        servicio: taggedLast.servicio,
      };
      expected = enumerateRequiredMealSlotsInWindow(
        taggedFirst.fecha,
        taggedFirst.servicio,
        taggedLast.fecha,
        taggedLast.servicio,
        required,
      );
      windowSource = "tagged";
      note =
        "Sin check-in/out: ventana por primera↔última comida tagueada";
    } else {
      skipped = true;
      note = "Sin estadía (check-in/out) ni comidas tagueadas";
      results.push({
        artistaId: artist.id,
        ...
        skipped: true,
        windowSource: "none",
        checkinAt: stay.checkin,
        checkoutAt: stay.checkout,
        note,
      });
      continue;
    }
