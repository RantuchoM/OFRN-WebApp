...
const CommentItem = ({
  ...
  const isUnread =
    new Date(comment.created_at).getTime() > lastReadAt.getTime() && !isMine;
  ...
  if (comment.fecha_limite && !comment.resuelto && !comment.deleted) {
    ...
    if (isBefore(d, new Date(now.setHours(0, 0, 0, 0))))
      deadlineClass = "bg-red-50 text-red-600 border-red-200 font-bold";
    else if (isToday(d))
      deadlineClass =
        "bg-orange-50 text-orange-600 border-orange-200 font-bold";
    else if (isBefore(d, addDays(now, 3)))
      deadlineClass = "bg-amber-50 text-amber-600 border-amber-200";
    else deadlineClass = "bg-indigo-50 text-indigo-600 border-indigo-200";
  }

  return (
    <div
      className={`flex flex-col ${isChild ? "ml-8 mt-2 pl-2 border-l-2 border-slate-100" : "mt-4"}`}
    >
      <div className={`flex gap-2 group ${isChild ? "items-start" : ""}`}>
        {/* AVATAR CON CLICK */}
        <div className="mt-1">
          <UserAvatar
            user={comment.integrantes}
            size={isChild ? "sm" : "md"}
            onClick={onPreviewImage} // Pasamos la función
          />
        </div>

        <div
          className={`flex-1 p-3 rounded-lg border shadow-sm transition-all relative 
                    ${
                      comment.deleted
        ...
        >
          <div className="flex justify-between items-start mb-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span
                className={`font-bold text-xs ${isMine ? "text-indigo-700" : "text-slate-800"}`}
              >
              ...
              {isUnread && (
                <span className="text-[8px] font-black bg-orange-400 text-white px-1.5 rounded animate-pulse">
                  NUEVO
                </span>
              )}
            </div>
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              {!comment.deleted && !isChild && (
                <button
                  ...
                  className={`p-1 rounded hover:bg-emerald-50 ${comment.resuelto ? "text-emerald-600" : "text-slate-300 hover:text-emerald-500"}`}
              ...
              {(isMine || user.rol_sistema === "admin") && !isChild && (
                <button
                  ...
                  title={comment.deleted ? "Restaurar" : "Archivar"}
                >
                  {comment.deleted ? (
                    <IconRefresh size={14} />
                  ) : (
          ...
          <p
            className={`text-sm whitespace-pre-wrap leading-relaxed ${comment.resuelto ? "text-slate-500 line-through" : "text-slate-700"}`}
          >
            {comment.contenido}
          </p>
          ...
          <div className="flex flex-wrap gap-2 mt-2 items-center">
            {comment.etiquetados?.map((tid) => {
              ...
              return u ? (
                <span
                  key={tid}
                  className="text-[9px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded border border-indigo-100 flex items-center gap-0.5"
                >
                  <IconAtSign size={10} /> {u.nombre}
                </span>
              ) : null;
            })}
            ...
            {comment.fecha_resolucion && (
              <span className="text-[9px] text-emerald-600 flex items-center gap-1 bg-emerald-50 px-1.5 py-0.5 rounded">
                <IconCheckCircle size={10} /> Resuelto
              </span>
            )}
          </div>
        </div>
...
export default function CommentsManager({
  ...
  const initialize = async () => {
    ...
    if (user?.id) {
      const { data } = await supabase
        .from("integrantes")
        .select("nombre, apellido, avatar_url, avatar_color")
        .eq("id", user.id)
        .single();
  ...
  const updateCommentDate = async (commentId, newDate) => {
    ...
    const { error } = await supabase
      .from("sistema_comentarios")
      .update({ fecha_limite: val })
      .eq("id", commentId);
    if (!error) {
      setComments((prev) =>
        prev.map((c) => (c.id === commentId ? { ...c, fecha_limite: val } : c)),
      );
      setEditingDateId(null);
      notifyUpdate();
    }
  ...
  const buildThreads = (list) => {
    ...
    return viewMode === "pending"
      ? roots.filter((root) => !root.deleted && !root.resuelto)
      : roots.filter((root) => root.resuelto || root.deleted);
  };
  ...
  const handleSend = async (e) => {
    ...
    const taggedIds = usersList
      .filter((u) => newComment.includes(`@${u.nombre}${u.apellido}`))
      .map((u) => u.id);
    const payload = {
      entidad_tipo: entityType,
      entidad_id: entityId.toString(),
  ...
  const toggleResolve = async (comment) => {
    ...
    comments.forEach((c) => {
      if (c.parent_id === comment.id) idsToUpdate.push(c.id);
    });
    setComments((prev) =>
      prev.map((c) =>
        idsToUpdate.includes(c.id)
          ? { ...c, resuelto: newVal, fecha_resolucion: resolveDate }
          : c,
      ),
    );
    await supabase
      .from("sistema_comentarios")
      .update({ resuelto: newVal, fecha_resolucion: resolveDate })
      .in("id", idsToUpdate);
  ...
  const insertMention = (u) => {
    ...
    words.pop();
    setNewComment(words.join(" ") + ` @${u.nombre}${u.apellido} `);
    setShowMentions(false);
  ...
  return (
    <div className="flex flex-col h-full bg-white shadow-2xl border-l border-slate-200 w-full md:w-[450px] animate-in slide-in-from-right duration-300 relative z-[60]">
      <div className="p-4 border-b border-slate-100 flex flex-col gap-3 bg-slate-50 shrink-0">
        <div className="flex justify-between items-start">
          <div>
            <h3 className="font-bold text-slate-800 flex items-center gap-2">
              <IconMessageCircle className="text-indigo-600" /> Comentarios
            </h3>
        ...
        <div className="flex p-1 bg-slate-200 rounded-lg">
          <button
            ...
            className={`flex-1 py-1 text-xs font-bold rounded-md transition-all ${viewMode === "pending" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
          >
            Pendientes
          </button>
          <button
            onClick={() => setViewMode("history")}
            className={`flex-1 py-1 text-xs font-bold rounded-md transition-all ${viewMode === "history" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
          >
            Historial
          </button>
        </div>
      ...
      >
        {loading && (
          <div className="text-center py-4">
            <IconLoader className="animate-spin text-indigo-500 inline" />
          </div>
        )}
      ...
      <div className="p-3 bg-white border-t border-slate-200 relative shrink-0">
        {replyingTo && (
          <div className="flex justify-between items-center bg-indigo-50 p-2 rounded text-xs text-indigo-700 mb-2 border border-indigo-100">
            <span>
              Respondiendo a <b>{replyingTo.integrantes?.apellido}</b>
            </span>
        ...
        {showMentions && (
          <div className="absolute bottom-full left-4 mb-2 w-64 bg-white border border-slate-200 rounded-lg shadow-xl max-h-48 overflow-y-auto z-50">
            {filteredUsers.map((u) => (
              <div
                key={u.id}
                onClick={() => insertMention(u)}
                className="px-3 py-2 hover:bg-indigo-50 cursor-pointer text-xs text-slate-700 border-b border-slate-50"
              >
                <b>
                  {u.nombre} {u.apellido}
                </b>
        ...
        <div className="flex gap-2 items-start">
          <div className="mt-1">
            <UserAvatar
              ...
              size="md"
              onClick={setPreviewImage} // También permitir ampliar mi propia foto
            />
          </div>
          ...
          <form onSubmit={handleSend} className="flex-1 flex gap-2 items-end">
            <textarea
              ref={inputRef}
              className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 resize-none h-10 min-h-[40px] max-h-24 transition-all focus:h-20"
              placeholder={
                replyingTo ? "Escribe una respuesta..." : "Nuevo tema..."
              }
              value={newComment}
