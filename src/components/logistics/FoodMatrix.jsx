...
export default function FoodMatrix({ roster }) {
  const { matrix, columns } = useMemo(() => {
    ...
    const dietTypes = [
      ...new Set(activeRoster.map((p) => canonicalizeMealDiet(p.alimentacion))),
    ].sort(compareMealDietLabels);
    ...
    activeRoster.forEach(p => {
      ...
      const type = canonicalizeMealDiet(p.alimentacion);
  ...
  return (
    <div className="inline-block bg-white border border-slate-200 rounded-lg p-2 shadow-sm select-none leading-tight">
      <table className="border-collapse">
        <thead>
          <tr className="text-slate-400 font-bold text-[9px] tracking-tighter">
            <th className="px-1"></th>
            {columns.map(col => (
              <th key={col} className={cellClass} title={dietDisplayLabel(col)}>
                {dietShortLabel(col)}
              </th>
            ))}
