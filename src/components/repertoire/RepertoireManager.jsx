...
import {
  IconMusic, IconPlus, IconTrash, IconSearch, IconLoader, IconCheck, IconX,
  IconLink, IconChevronDown, IconAlertCircle, IconEdit, IconYoutube, IconDrive
} from "../ui/Icons";
import { formatSecondsToTime } from "../../utils/time";
import { calculateInstrumentation, calculateTotalDuration } from "../../utils/instrumentation";
...
const SoloistSelect = ({ currentId, musicians, onChange }) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => { if (wrapperRef.current && !wrapperRef.current.contains(event.target)) { setIsOpen(false); setSearch(""); } };
        document.addEventListener("mousedown", handleClickOutside);
        return () => document.removeEventListener("mousedown", handleClickOutside);
    }, []);

    const filtered = musicians.filter(m => `${m.apellido}, ${m.nombre}`.toLowerCase().includes(search.toLowerCase()));

    return (
        <div className="relative w-full" ref={wrapperRef}>
            {!isOpen ? (
                <div onClick={() => setIsOpen(true)} className="w-full text-[10px] text-slate-700 truncate cursor-pointer hover:bg-indigo-50 p-1 rounded border border-transparent hover:border-indigo-100 min-h-[24px] flex items-center">
                    {selectedMusician ? <span className="font-bold text-indigo-700">{selectedMusician.apellido}, {selectedMusician.nombre}</span> : <span className="text-slate-400 italic">- Seleccionar -</span>}
                </div>
            ) : (
                <div className="absolute top-0 left-0 w-64 bg-white border border-indigo-200 shadow-xl rounded z-50 animate-in zoom-in-95 duration-100">
                    <input type="text" autoFocus placeholder="Buscar apellido..." className="w-full p-2 text-xs border-b border-slate-100 outline-none" value={search} onChange={(e) => setSearch(e.target.value)} />
                    <div className="max-h-48 overflow-y-auto">
                        <div onClick={() => { onChange(null); setIsOpen(false); }} className="p-2 text-xs text-slate-400 hover:bg-red-50 hover:text-red-600 cursor-pointer border-b border-slate-50 italic">- Quitar Solista -</div>
                        {filtered.map(m => (
                            <div key={m.id} onClick={() => { onChange(m.id); setIsOpen(false); }} className={`p-2 text-xs cursor-pointer hover:bg-indigo-50 flex justify-between ${currentId === m.id ? 'bg-indigo-50 font-bold text-indigo-700' : 'text-slate-600'}`}>
                                <span>{m.apellido}, {m.nombre}</span>
                                <span className="text-[9px] text-slate-400 ml-2 truncate max-w-[80px]">{m.instrumentos?.instrumento}</span>
                            </div>
                        ))}
                    </div>
                </div>
...
export default function RepertoireManager({ supabase, programId, giraId, initialData = [], isCompact = false }) {
  ...
  const [filters, setFilters] = useState({ compositor: "", titulo: "", arreglador: "" });
  const [workFormData, setWorkFormData] = useState({});

  useEffect(() => {
    if (!initialData.length && programId) fetchFullRepertoire();
    else if (initialData.length) setRepertorios(initialData.map((r) => ({ ...r, repertorio_obras: r.repertorio_obras?.sort((a, b) => a.orden - b.orden) || [] })));
  ...
  const fetchMusicians = async () => { const { data } = await supabase.from("integrantes").select("id, nombre, apellido, instrumentos(instrumento)").order("apellido"); if (data) setMusicians(data); };
  ...
  const fetchLibrary = async () => {
    ...
    const { data, error } = await supabase.from("obras").select(`*, obras_compositores (rol, compositores (apellido, nombre)), obras_palabras_clave (palabras_clave (tag)), obras_particellas (nombre_archivo, nota_organico, instrumentos (instrumento))`).order("titulo");
    if (!error && data) setWorksLibrary(data.map((w) => ({ ...w, compositor_full: getComposers(w), arreglador_full: getArranger(w) })));
  ...
  const openCreateModal = () => {
      setWorkFormData({ id: null, titulo: "", duracion_segundos: 0, link_drive: "", link_youtube: "", estado: "Solicitud" });
      setIsEditWorkModalOpen(true); 
  };
  ...
  const saveBlockName = async () => {
    ...
    setRepertorios(repertorios.map((r) => r.id === editingBlock.id ? { ...r, nombre: editingBlock.nombre } : r));
  ...
  const moveWork = async (repertorioId, workId, direction) => {
    ...
    if ((direction === -1 && workIndex === 0) || (direction === 1 && workIndex === obras.length - 1)) return;
  ...
  const addRepertoireBlock = async () => {
    ...
    if (!nombre) return;
    await supabase.from("programas_repertorios").insert([{ id_programa: programId, nombre, orden: repertorios.length + 1 }]);
    fetchFullRepertoire();
  };
  ...
  const updateWorkDetail = async (itemId, field, value) => {
      setRepertorios(repertorios.map(r => ({ ...r, repertorio_obras: r.repertorio_obras.map(o => o.id === itemId ? {...o, [field]: value} : o) })));
      await supabase.from("repertorio_obras").update({[field]: value}).eq("id", itemId);
      if (field === "id_solista" && value && giraId) {
          try { await supabase.from('giras_integrantes').upsert({ id_gira: giraId, id_integrante: value, rol: 'solista', estado: 'confirmado' }, { onConflict: 'id_gira, id_integrante' }); } catch (e) { console.error(e); }
      }
  };

  const getComposers = (obra) => obra.obras_compositores?.length > 0 ? obra.obras_compositores.filter(oc => !oc.rol || oc.rol === 'compositor').map(oc => `${oc.compositores?.apellido}, ${oc.compositores?.nombre}`).join(" / ") : (obra.compositores ? `${obra.compositores.apellido}, ${obra.compositores.nombre}` : "Anónimo");
  const getArranger = (obra) => { const arr = obra.obras_compositores?.find(oc => oc.rol === 'arreglador'); return arr ? `${arr.compositores.apellido}, ${arr.compositores.nombre}` : "-"; };
  
  const filteredLibrary = worksLibrary.filter(w => 
      (!filters.titulo || w.titulo.toLowerCase().includes(filters.titulo.toLowerCase())) &&
      (!filters.compositor || w.compositor_full?.toLowerCase().includes(filters.compositor.toLowerCase())) &&
      (!filters.arreglador || w.arreglador_full?.toLowerCase().includes(filters.arreglador.toLowerCase()))
  );
  ...
  return (
    <div className={containerClasses(isCompact)}>
              ...
              <div className="bg-indigo-50/50 p-2 border-b border-slate-200 flex justify-between items-center h-10">
                  <div className="flex items-center gap-2">
                      ...
                      {editingBlock.id === rep.id ? (
                        ...
                        <span className="font-bold text-slate-800 text-xs uppercase flex items-center gap-2 group cursor-pointer" onClick={() => isEditor && startEditBlock(rep)}>{rep.nombre} {isEditor && <IconEdit size={12} className="opacity-0 group-hover:opacity-100 text-slate-400"/>}</span>
                      )}
                  </div>
                  <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono font-bold text-slate-600 bg-white px-1.5 rounded border">Total: {calculateTotalDuration(rep.repertorio_obras)}</span>
                              ...
                              <tr key={item.id} className="hover:bg-yellow-50 group">
                                  <td className="p-1 text-center font-bold text-slate-500">
                                      <div className="flex flex-col items-center">
                                          ...
                                          <span>{idx+1}</span>
                                          {isEditor && !isCompact && <button onClick={() => moveWork(rep.id, item.id, 1)} disabled={idx === rep.repertorio_obras.length - 1} className="text-slate-300 hover:text-indigo-600 disabled:opacity-0 p-0.5"><IconChevronDown size={8}/></button>}
                                      </div>
                                  </td>
                                  <td className="p-1 text-center">
                                      {item.google_drive_shortcut_id ? <IconDrive className="w-3.5 h-3.5 mx-auto text-slate-600"/> : item.obras.link_drive ? <a href={item.obras.link_drive} target="_blank" className="block w-2 h-2 bg-amber-400 rounded-full mx-auto"></a> : <span className="text-slate-200">-</span>}
                                  </td>
                                  <td className="p-1 truncate text-slate-600" title={getComposers(item.obras)}>{getComposers(item.obras)}</td>
                                  <td className="p-1 font-bold text-slate-800 truncate" title={item.obras.titulo}>
                                      {item.obras.titulo}
                                      {item.obras.estado !== "Oficial" && <span className="ml-1 text-[8px] bg-amber-100 text-amber-700 px-1 rounded border border-amber-200">PEND</span>}
                                  </td>
                                  <td className="p-1 text-center whitespace-nowrap text-[10px] text-slate-500 font-mono">
                                      {item.obras.instrumentacion || calculateInstrumentation(item.obras.obras_particellas) || "-"}
                                  </td>
                                  <td className="p-1 text-center font-mono">{formatSecondsToTime(item.obras.duracion_segundos)}</td>
                                  <td className="p-0 border-l border-slate-100 align-middle">
                                      {isEditor ? <div className="px-1"><SoloistSelect currentId={item.id_solista} musicians={musicians} onChange={(newId) => updateWorkDetail(item.id, "id_solista", newId)}/></div> : <span className="block p-1 truncate text-[10px]">{item.integrantes ? `${item.integrantes.apellido}, ${item.integrantes.nombre}` : "-"}</span>}
                                  </td>
                                  <td className="p-1 truncate text-slate-500">{getArranger(item.obras)}</td>
                                  <td className="p-0 border-l border-slate-100">
                                      {isEditor ? <input type="text" className="w-full bg-transparent p-1 text-[10px] outline-none" placeholder="..." value={item.notas_especificas || ""} onChange={e => updateWorkDetail(item.id, "notas_especificas", e.target.value)}/> : <span className="block p-1 truncate">{item.notas_especificas}</span>}
                                  </td>
                                  <td className="p-1 text-center">{item.obras.link_youtube ? <a href={item.obras.link_youtube} target="_blank" className="text-red-600"><IconYoutube size={14}/></a> : <span className="text-slate-200">-</span>}</td>
                                  <td className="p-1 text-right">
                                      <div className="flex justify-end gap-1">
                                          ...
                                          {isEditor && <>
                                                  <button onClick={() => openEditModal(item)} className="text-slate-300 hover:text-indigo-600 p-1"><IconEdit size={12}/></button>
                                                  <button onClick={() => removeWork(item.id)} className="text-slate-300 hover:text-red-600 p-1"><IconX size={12}/></button>
                                          </>}
                                      </div>
                                  </td>
                              </tr>
        ...
        <ModalPortal>
            <div className="bg-white w-full max-w-5xl h-[80vh] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
                <div className="p-3 border-b flex justify-between items-center bg-slate-50">
                    <h3 className="font-bold text-slate-700 flex gap-2"><IconSearch size={18}/> Buscar Obra</h3>
                ...
                <div className="p-2 border-b grid grid-cols-1 md:grid-cols-5 gap-2 bg-white">
                    <input type="text" placeholder="Compositor..." autoFocus className="w-full p-1.5 border rounded text-xs outline-none focus:border-indigo-500" value={filters.compositor} onChange={e => setFilters({...filters, compositor: e.target.value})}/>
                    <input type="text" placeholder="Arreglador..." className="w-full p-1.5 border rounded text-xs outline-none focus:border-indigo-500" value={filters.arreglador} onChange={e => setFilters({...filters, arreglador: e.target.value})}/>
                    <div className="relative col-span-2"><IconSearch className="absolute left-2 top-2.5 text-slate-400" size={14}/><input type="text" placeholder="Título..." className="w-full pl-7 p-1.5 border rounded text-xs outline-none focus:border-indigo-500" value={filters.titulo} onChange={e => setFilters({...filters, titulo: e.target.value})}/></div>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredLibrary.map(w => (
                                    <tr key={w.id} className="hover:bg-indigo-50 group">
                                        <td className="p-2 text-slate-600 font-medium truncate">{w.compositor_full}</td>
                                        <td className="p-2 text-slate-500 truncate">{w.arreglador_full !== "-" ? w.arreglador_full : ""}</td>
                                        <td className="p-2 text-slate-800 font-bold truncate">{w.titulo}</td>
                                        <td className="p-2 text-center font-mono text-[10px] text-slate-400">{formatSecondsToTime(w.duracion_segundos)}</td>
                                        <td className="p-2 text-center font-mono text-[10px] text-slate-500 bg-slate-50/50 rounded">{w.instrumentacion || calculateInstrumentation(w.obras_particellas) || "-"}</td>
                                        <td className="p-2 text-center text-slate-500">{w.anio_composicion || "-"}</td>
                                        <td className="p-2 text-center">{w.link_drive ? <a href={w.link_drive} target="_blank" rel="noreferrer" className="text-blue-600 hover:text-blue-800 inline-block p-1 bg-blue-50 rounded-full"><IconDrive size={14}/></a> : <span className="text-slate-200">-</span>}</td>
                                        <td className="p-2 text-right"><button onClick={() => addWorkToBlock(w.id)} className="bg-white border border-indigo-200 text-indigo-600 px-2 py-0.5 rounded font-bold hover:bg-indigo-600 hover:text-white shadow-sm transition-colors text-[10px]">Seleccionar</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
      ...
      {commentsState && <div className="fixed inset-0 z-[80] flex justify-end bg-black/20 backdrop-blur-[1px]" onClick={() => setCommentsState(null)}><div onClick={e => e.stopPropagation()} className="h-full"><CommentsManager supabase={supabase} entityType={commentsState.type} entityId={commentsState.id} title={commentsState.title} onClose={() => setCommentsState(null)}/></div></div>}
    </div>
  );
}

const containerClasses = (isCompact) => isCompact ? "bg-white" : "space-y-8";
const tableHeaderClasses = (isCompact) => isCompact ? "hidden" : "bg-blue-200 text-slate-700 border-b-2 border-slate-400 font-bold uppercase tracking-tight";