...
import { IconCheck, IconLoader, IconFileText, IconClock, IconHistory } from '../ui/Icons'; 

export default function SectionStatusControl({ supabase, giraId, sectionKey, sectionLabel, currentUserId }) {
    // Estados: 'PENDING', 'IN_PROGRESS', 'COMPLETED'
    const [progresoId, setProgresoId] = useState(null); // ID real de la fila en DB
    ...
    const STATE_CONFIG = {
        'PENDING': {
            label: 'Sin Iniciar',
            colorClass: 'bg-slate-50 text-slate-500 hover:bg-slate-100 border-slate-200',
    ...
    // Cargar historial solo cuando se abre el popup
    useEffect(() => {
    ...
    const fetchHistory = async () => {
        ...
        // Traemos el historial y el nombre del integrante que modificó
    ...
    const saveToDb = async (payload) => {
        ...
        if (error) {
            ...
            setProgresoId(data.id); // Guardamos el ID por si era nuevo
        }
    };
    ...
    // Helper para traducir estados en el historial
    ...
    if (loading) return <div className="text-[10px] text-slate-400 flex items-center gap-1"><IconLoader className="animate-spin"/></div>;

    return (
        <div className="flex items-start gap-2 relative z-20">
            {/* CONTENEDOR PRINCIPAL */}
            <div className={`flex items-center gap-0 border rounded-lg overflow-hidden transition-all shadow-sm ${currentConfig.colorClass.split(' ')[0]} ${currentConfig.colorClass.split(' ').pop()}`}>
                ...
                <button 
                    ...
                    className={`px-2 py-1.5 hover:bg-black/5 flex items-center gap-1 relative ${observaciones ? 'text-indigo-600' : 'text-slate-400'}`}
                    title="Historial y Notas"
                >
                    <IconFileText size={16}/>
                    {observaciones && <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-indigo-500 rounded-full"></span>}
                </button>
            </div>

            {/* POPUP DE DETALLE (HISTORIAL + NOTAS) */}
            {isOpen && (
                <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-slate-200 p-0 animate-in fade-in slide-in-from-top-2 z-50 overflow-hidden">
                    ...
                    <div className="bg-slate-50/50 max-h-32 overflow-y-auto border-b border-slate-100 p-2">
                        {loadingHistory && <div className="text-center text-[10px] text-slate-400 py-2">Cargando historial...</div>}
                        
                        {!loadingHistory && history.length === 0 && (
                            <div className="text-center text-[10px] text-slate-400 py-2 italic">Sin cambios recientes.</div>
                        )}

                        {!loadingHistory && history.map((h, i) => (
                            <div key={i} className="flex gap-2 text-[10px] text-slate-600 mb-1.5 px-2">
                                <span className="text-slate-400 whitespace-nowrap">{formatDate(h.fecha_modificacion)}</span>
                                <div>
                                    <span className="font-bold text-indigo-600">
                                        {h.integrantes ? `${h.integrantes.nombre} ${h.integrantes.apellido}` : 'Usuario'}
                                    </span>
                                    <span className="mx-1 text-slate-400">Changed to</span>
                                    <span className={`font-bold ${h.estado_nuevo === 'COMPLETED' ? 'text-emerald-600' : 'text-slate-600'}`}>
                    ...
                    {/* Área de Notas */}
                    <div className="p-3">
                        <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Observaciones Generales</label>
                        <textarea 
                        ...
                        >
                            {saving ? <IconLoader className="animate-spin"/> : <IconCheck/>} Guardar Nota
                        </button>
                    </div>
                </div>
            )}
