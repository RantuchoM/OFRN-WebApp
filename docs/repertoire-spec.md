...
- **Título solo de este programa** (`repertorio_obras.titulo_concierto`, 2026-09-23): HTML opcional por fila (mismo editor que el título de obra: negrita, cursiva, subrayado, listas), análogo a `duracion_segundos_concierto`. Si está cargado, el listado de `ProgramRepertoire` y la sección Programa de Difusión muestran ese HTML. `NULL`, vacío o el mismo texto sin formato usa `obras.titulo` (en difusión el catálogo sigue quitando anotaciones `[...]`; el título del programa se publica tal cual). No actualiza `obras.titulo`. Al duplicar la gira (`manage-gira`) y al importar un bloque se copia la fila, no el catálogo. Las reservas siguen usando `titulo_placeholder`.
