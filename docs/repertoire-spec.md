...
- **Título solo de este programa** (`repertorio_obras.titulo_concierto`, 2026-09-23): texto opcional por fila, análogo a `duracion_segundos_concierto`. Si está cargado, el listado de `ProgramRepertoire` y la sección Programa de Difusión muestran ese título. `NULL` o vacío usa `obras.titulo` (en difusión se siguen quitando anotaciones `[...]`). No actualiza `obras.titulo`. Al duplicar la gira (`manage-gira`) y al importar un bloque se copia la fila, no el catálogo. Las reservas siguen usando `titulo_placeholder`.
