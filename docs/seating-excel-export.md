...
- **Estructura**:
  ...
  - **Sección 2 (Vientos/Asignaciones)**: Tabla cruzada entre Músico (filas) y Obras (columnas), mostrando el nombre de la particella asignada por obra. Filas ordenadas por `id_instr` y, dentro de cada familia, por número de parte asignada (ej. Fagot 1, 2; Corno 1, 2, 3, 4). Si en una obra la misma parte está duplicada en dos músicos del mismo instrumento, se usa la siguiente obra sin duplicados. En percusión (`id_instr` 13), `Perc Timp` siempre antes que el resto de perc auxiliar. Orden vía `sortWindMusiciansForSeating` en `seatingWindOrder.js`.
