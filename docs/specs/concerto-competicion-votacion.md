...
- Alta de participante: buscador de integrantes (nombre y apellido; sin vacantes `es_simulacion`), segundo integrante opcional, observaciones. La grilla muestra una fila por participante (el dúo sigue siendo una fila).
...
Modales con portal a `document.body` y `z-[100]`. Iconos solo de `src/components/ui/Icons.jsx`. Textos en español. El buscador de obras es el modal que ya usa Programación/Repertorio, sin el filtro inicial de orgánico de la gira: las obras del concurso no tienen por qué caber en el seating convocado.
