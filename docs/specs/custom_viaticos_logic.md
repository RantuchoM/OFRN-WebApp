...
- [x] **PDF dos líneas al cruzar franjas (2026-09-23):** solo `mode=viatico` y ≥2 segmentos. Plantilla `plantilla_viaticos_multiples.pdf`. Línea 1 (`dias_computados` / `valor_diario` / `porcentaje`) = franja vieja; línea 2 (`…1`) = franja nueva (mismo %). Un solo tramo sigue `plantilla_viaticos.pdf`. Destaque y rendición no usan la múltiple.
...
| PDF dual franja | `public/plantillas/plantilla_viaticos_multiples.pdf`, `src/utils/pdfFormExporter.js` |
