...
- [x] Tooltip de ítems: nombre + tamaño real en cm (instrumentos = catálogo DB; tarimas `Ancho W × Profundo D cm` vía `scaleX`/`scaleY`).
- [x] Formaciones reescaladas al lienzo cm + ítems ~40 cm (defaults ~3–3.6 m; marcador 15 cm, snap 20 cm)
- [x] Paleta Formaciones: drag-from-sidebar (ancla en drop) + clic = top-left del lienzo (`formationTopLeftInsertAnchor`, inset ~20 cm)
...
## Huella de instrumento — tamaño fijo desde catálogo DB

- Constantes (`stagePlotConstants.js`):
  - `STAGE_PLOT_INSTRUMENT_FOOTPRINT_WIDTH_CM=50`, `DEPTH_CM=50` (cuadrado base @ scale 1)
...
- **Sin resize en el lienzo** (solo instrumentos con huella):
  ...
  - Barra inferior: chip **solo lectura** `W × D cm` (catálogo); sin inputs Ancho/Profundo, sin candado de proporción, sin HUD live de resize.
  - Botones ± escala ocultos cuando la selección es solo instrumentos; `scaleSelected` no toca huellas.
  - Menú contextual: sin «Unificar tamaños» para tipos con huella.
- **Dónde editar tamaño**: Escenario → panel izquierdo → **Editor** (Ancho/Profundo cm del instrumento) o Datos → Instrumentos (`stage_plot_*_cm`). Toast: «aplica a todos los planos».
- Tarimas / décor / atriles / texto: resize y escalas del ítem **sin cambio**.
...
- Escenario: al renderizar / PDF, tamaño = `stage_plot_width_cm` × `stage_plot_height_cm` del catálogo (**50×50 → scale 1**); el `scale` del ítem en el plot se ignora para huellas.
...
- [x] Tamaño de huella fijo desde DB en todos los planos (sin resize en canvas)
...
- **Copiar formación** (`cloneStagePlotFormation` en `stagePlotFormations.js`; UI en barra inferior + menú contextual clic derecho):
  ...
  - Dropdown **Copiar…** de la barra inferior: portal (`StagePlotFormationCopyMenu`) para no quedar clipado por `overflow-x-auto` ni debajo del canvas.
...
- **Paleta → Formaciones** (panel izq., `canEdit`): lista **vertical** (una fila por kind), no chips en wrap. Cada fila: mini SVG esquemático (`FormationPaletteIcon` en `ProgramStagePlotEditor.jsx`, trazo índigo) + label. **Mismo gesto que instrumentos**: `onPointerDown` → `startFormationPalettePointerDrag` (umbral 4 px). **Arrastrar** al lienzo → ancla `(x,y)` en el punto de soltura (coords de escenario vía viewport). **Clic** (sin mover) → `addFormation` / `formationTopLeftInsertAnchor`: AABB de la guía (sin plazas/facing) con `minX`/`minY` a inset `STAGE_PLOT_FORMATIONATION_INSERT_INSET_PX` (~20 cm) desde el origen upstage-left del rectángulo del plot (no centro; lejos del director/radial downstage). Ghost de drag: `paletteDrag.mode === "formation"` + `FormationPaletteIcon`. Móvil (sheet Agregar): solo toque → mismo top-left (sin drag). Iconos por kind: **Arco** arco elíptico abierto abajo; **Semi-arco** alas rectas + arco; **Herradura** U con tope curvo; **Rectángulo** tres lados abiertos abajo; **Línea recta** segmento horizontal. Sin `IconPlus` genérico.
...
- **Floating toolbar (Copiar / Eliminar, + formato texto):** overlay HTML sobre el canvas (`pointer-events` solo en la pill), **después** del Stage Konva en el DOM y `z-[40]` (por encima de `.no-dark-invert` / filter del lienzo). Posición en **coords de pantalla** (`viewport.x/y + stageCoord × viewport.scale`). Regla de colocación (AABB de la selección, single/multi/texto):
...
- **Barra inferior de formación — Copiar…:** dropdown via portal a `document.body` (`StagePlotFormationCopyMenu`, `portalMenuZ`). La barra tiene `overflow-x-auto` (clip en Y) y el menú abre hacia arriba sobre el lienzo; sin portal quedaba tapado. Barra con `relative z-40`. Pill móvil formación: misma capa `z-[40]` post-Stage.
...
- Plots con `instrumentFootprintMigrated`: escalas custom viejas del Transformer en ítems-instrumento se **ignoran** al render y se limpian al normalizar (catálogo DB gana).
- `cat.w`/`cat.h` del catálogo estático siguen siendo aspect/fallback de paleta; el tamaño en escena de instrumentos con huella viene de `stage_plot_*_cm`.
