...
- `useAgendaData.js` (2026-09-14): las paradas del bus asignado **no se descartan** por `eventos_grupos` de otro grupo de convocatoria (p. ej. Charter Viedma 03:10 Villa Regina tagueado Crimson, músico Marley admitido por localidad). `mockPerson` lleva `grupo_ids` de la gira. Caché `v10`. Helper `isAssignedVehicleAgendaStop` en `agendaHelpers.js`.
