...
- **Chips** en `UnifiedAgenda` apilados en vertical en columna propia; el `IconTag` queda a la izquierda del primer chip (o solo, si no hay grupos). **Artistas FIMBA** tagueados (`eventos_fimba_propuestas`) se listan debajo de los grupos con el mismo estilo de chip (color de la propuesta).
...
- **Multi-select de agenda (editores, 2026-09-22):** en `UnifiedAgenda`, check bajo la hora (compacta + escritorio), **centrado bajo el texto** de la hora (no alineado a la columna). Músicos/consulta: sin check. Con ≥1 seleccionado, barra portal (`z-[90]`) **Eliminar** (papelera + confirm `z-[110]`), **Ocultar** (paradas: `visible_agenda=false`; resto: `tecnica=true`, mismo ojo/TÉC) y **Etiqueta de grupo** (`eventos_grupos` / `EventGruposAssignModal` en lote). Grupo solo si todos los visibles seleccionados son de la **misma gira** y esa gira tiene `giras_grupos`; si no, el botón se deshabilita con motivo (giras distintas / sin gira / sin grupos). Esc y la X limpian. La selección no salta el filtro de categorías: el conteo y las acciones usan `filteredItems` (Transporte destildado no “revive” paradas). `IconTrash` rojo a la derecha borra un evento (mismo confirm).
...
- **Edición masiva Comidas (2026-09-07):** barra flotante (portal `document.body`, `z-[100]`) con conteo + hora/lugar/convocados + Aplicar / limpiar selección; no tapa el thead sticky. **Dropdowns (2026-09-14):** locación (`GridLocationSelect`) y convocados (`MultiGroupSelect`) portal a `document.body` `z-[110]` con `getFixedMenuPosition` (abren hacia arriba si la barra está al fondo; no se recortan bajo la taskbar).
...
- **Catering en gestor:** categoría Catering entra al gestor junto a Comidas (`isMealRelatedEvent`); filtro **Todos | Comidas | Catering** + chip servicio **Cat**. El select **Servicio** de cada fila (`mealTypeSelectOptions`) lista **todos** los tipos Comidas y Catering (vivos: D/A/M/C + a bordo + Catering Almuerzo / Merienda / Cena). **Catering tipado (2026-09-15):** «Catering Merienda|Almuerzo|Cena» mapea a slot A/M/C (`mealAmcFromCateringLabel` / `mealServicioFromEvent`): aparece en el walk Merienda/…, filtro clase Comidas, chip Merienda y **cubre cobertura A/M/C** del artista; chip Cat también los lista. Bare «Catering» = slot sintético 4; cobertura logística OFRN de ese bare = **día calendario** dentro de `comida_inicio`..`comida_fin`. Inicio/fin de regla = slot día+tipo (`docs/specs/logistica-comida-slot.md`), no un `eventos.id`.
- **Alta con filtro Artista (2026-09-14):** vacante, `+` sibling y `saveRow` insert pre-asignan `eventos_fimba_propuestas` (hereda tags de la fila; si no hay, el filtro activo). Sin tag si el filtro está vacío o es Solo orquesta. Cobertura auto-create ya taguea el artista del hueco.
...
- **Asistencia / Reporte de comidas:** mismos ejes Locación + Artista + Todos|Comidas|Catering (+ servicios); embed `eventos_grupos` + prop `giraGrupos` para AND de elegibilidad OFRN (paridad Manager). Exports (PDF / Texto pedido / columnas asistencia) usan el set filtrado y siguen desglosando **tipos de alimentación** (`alimentacion` / dietas). **Texto pedido (2026-09-23):** sin cierre de hospedaje (`Fecha de ingreso y egreso` / `Hospedaje (no locales)` / `Grupo ingreso`). Cabeceras: `dietShortLabel` (`dietOptions.js`) — vacío/General/Estándar = **Estándar**; Sin Lactosa = **s/Lact.**; Sin Sal = **s/sal**; Sin TACC = **s/TACC**; FIMBA Regular no se mezcla con Estándar. FIMBA: pie **Especificaciones de comidas — artistas** (`nota_alimentacion` no vacío) en PDF, texto pedido, Excel/ZIP por artista y Excel/PDF/texto por estadía.
...
**Excepción — paradas del vehículo asignado (2026-09-14):** el tag `eventos_grupos` de una parada es editorial (p. ej. default del charter = Crimson). No oculta la parada al músico que **logística admite a ese bus** (localidad/región/persona/grupo/INTERNO) **mientras la categoría Transporte esté tildada**. Staff ya veía todas las paradas; la agenda del músico las filtraba y, con «Solo mi transporte», desaparecía su subida. Helper `isAssignedVehicleAgendaStop` + `eventVisibleForMusicianGrupos` (convocatoria, no categorías). **Filtro de categoría (2026-09-22):** `eventPassesAgendaCategoryFilter` — si Transporte está destildado, se ocultan todas las paradas/traslados; la excepción de vehículo **no** fuerza la fila. El cálculo de `myTransportLogistics` enriquece `grupo_ids` (paridad `enrichRosterWithGrupoIds`) para que admisión/rutas por Grupo coincidan con staff. Caché agenda `v11` slim `{ from, to, items }` (una clave; arrays legacy de agenda general se ignoran). Agenda general pagina de a 1000, cubre el Hasta del filtro / `monthsLimit`, y re-consulta al cambiar Desde/Hasta (spec `ajustes_interfaz_y_roles.md` §14).

**Dato gira 12 / Charter 222 (2026-09-14):** se quitaron 9 filas `eventos_grupos` del grupo Crimson (OFRN) en las paradas de ese vehículo (incl. 03:10 Villa Regina) y se pasó `audiencia_ofrn` de `grupos` a `tutti`. Sin tag, el evento es general y Vergara (Marley, admitido por localidad Villa Regina) ya ve la parada; la excepción de código cubre si alguien vuelve a taguear el charter.
...
- [x] Comidas: dropdowns bulk portal z-110 (locación/convocados abren hacia arriba); pin de fila en edición vs filtros
...
- [x] Comidas: select Servicio incluye tipos Catering (Almuerzo/Merienda/Cena) además de D/A/M/C (2026-09-14)
- [x] Catering tipado → slot A/M/C (grilla + cobertura artista; 2026-09-15)
- [x] Comidas: alta con filtro Artista pre-taguea `eventos_fimba_propuestas` (vacante / `+` / insert; 2026-09-14)
