...
**Excepción — paradas del vehículo asignado (2026-09-14):** el tag `eventos_grupos` de una parada es editorial (p. ej. default del charter = Crimson). No oculta la parada al músico que **logística admite a ese bus** (localidad/región/persona/grupo/INTERNO). Staff ya veía todas las paradas; la agenda del músico las filtraba y, con «Solo mi transporte», desaparecía su subida. Helper `isAssignedVehicleAgendaStop` + `eventVisibleForMusicianGrupos`. El cálculo de `myTransportLogistics` enriquece `grupo_ids` (paridad `enrichRosterWithGrupoIds`) para que admisión/rutas por Grupo coincidan con staff. Caché agenda `v10`.
