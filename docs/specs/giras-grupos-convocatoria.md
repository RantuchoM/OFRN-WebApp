...
- **Catering en gestor:** categoría Catering entra al gestor junto a Comidas (`isMealRelatedEvent`); filtro **Todos | Comidas | Catering** + chip servicio **Cat**. El select **Servicio** de cada fila (`mealTypeSelectOptions`) incluye los subtipos del servicio actual **y** los tipos cat. Catering (vivos: Catering Almuerzo / Merienda / Cena). Cobertura logística de Catering = **día calendario** dentro de `comida_inicio`..`comida_fin` (el slot sintético 4 no se compara contra D–C). Inicio/fin de regla = slot día+tipo (`docs/specs/logistica-comida-slot.md`), no un `eventos.id`.
