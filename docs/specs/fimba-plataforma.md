...
- **UX 2026-09-05 — Sin `window.alert` / `confirm` / `prompt` en FIMBA:** decisiones (eliminar, duplicar, regenerar tokens, conflicto de parada, bajar todo, descartar dirty, etc.) → `useConfirmDialog` → `ConfirmDialog`/`ConfirmModal` (portal `document.body`, z-100; z-110 si está encima de otro modal). Avisos informativos / errores de export-copy → `sonner` toast. `useConfirmDialog.alert()` = un botón «Entendido» (`hideCancel`). Clipboard fallido al copiar enlace de consulta agenda → diálogo con la URL en el mensaje (antes `window.prompt`).
...
- **Reporte comidas FIMBA (texto + cuadro, 2026-09-07):** pestaña **Reporte** reusa `MealsReport` con `fimbaMode` + `giraGrupos` / `eventos_grupos` (misma elegibilidad OFRN que el Gestor). **Cuadro** = tabla por servicio (fecha/hora/lugar + Total + columnas de dieta OFRN + dietas artistas FIMBA desde nominados). **Texto** = botón **Texto pedido** → modal portal z-100 (copiar clipboard). **Exportar PDF** = print del cuadro. Exports respetan filtros activos (clase/servicio/locación/artista/convocados). Asistencia = `MealsAttendance` (misma gira OFRN + grupos). El modal Hotelería **Reportes comidas** (`FimbaComidasReportModal`) sigue siendo el reporte por **estadía**/regímenes nominados (distinto del MealsReport por evento).
...
| `src/views/Giras/MealsManager.jsx` | Matriz comidas OFRN (+ Catering + deducción grupo↔orquesta + modo FIMBA) |
| `src/views/Giras/MealsReport.jsx` | Reporte cuadro + Texto pedido (+ `fimbaMode` dietas artistas desde participantes / Art. residual / skin magenta) |
| `src/views/Giras/MealsAttendance.jsx` | Asistencia por evento (reusado en FIMBA Comidas) |
| `src/utils/mealLogistics.js` | `isMealRelatedEvent` / catering / `mealCoincidenceKey` / deducción orquesta |
