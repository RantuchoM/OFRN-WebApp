...
- pestaña **Orquesta OFRN**: embebe `StopRulesManager` **inline** (`embedded` + **`allowMultipleAssignments`**) en la misma modal (sin segundo full-screen; evita stack z-[70] detrás del backdrop FIMBA). Props: `event`, `type`, `transportId` (vehículo), `giraId`, `passengers`/`admissionRules`/`regions`/`localities` OFRN, `giraGrupos` (`giras_grupos` + integrantes), `sortedEvents` (secuencia del vehículo), `supabase`. Tabla: `giras_logistica_rutas` (IDs integrantes numéricos; **`es_chofer` por regla de subida/trayecto**, no en `integrantes`). Modal standalone OFRN Trayectos **no** pasa el flag (sigue «Reemplazar» ante conflicto). Confirms embebidos `z-[110]`. Jerarquía de match = misma que Giras (no reimplementada).
  ...
  - **Multi-asignación OFRN en FIMBA (2026-09-08):** con `allowMultipleAssignments`, agregar Categoría/Grupo/etc. otra vez en la misma parada **INSERT** otra fila (varios chips del mismo alcance); si el alcance ya tiene ↑/↓ en **otro** evento, también INSERT (sin «Reemplazar»). Cerrar ride abierto (↓ de un ↑ sin bajada) sigue siendo UPDATE. `FimbaEventoFormModal` Orquesta usa el mismo flag.
  - **Espejo Grupo ↑ → ↓ (2026-09-05):** en `StopRulesManager` (OFRN Trayectos + pestaña Orquesta de `FimbaStopRulesManager`): (1) en **Reglas Activas** de una subida Grupo sin `id_evento_bajada`, acción **Crear bajada** + select de paradas posteriores del vehículo (`sortedEvents`); (2) al agregar Grupo ↑, checkbox **También asignar bajada en…** con el mismo picker. Persistencia vía `upsertOfrnGrupoRutaStop` type=down (no duplica si ya hay ↓ Grupo en esa parada/vehículo; cierra ride abierto). Misma auto-admisión de miembros.
...
- [x] **Multi-asignación boarding (2026-09-08):** varias ↑/↓ del mismo artista FIMBA y misma categoría/grupo OFRN sin «Reemplazar»; `allowMultiple` / `allowMultipleAssignments` (OFRN Trayectos standalone intacto)
