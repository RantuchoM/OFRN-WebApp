...
- **Cobertura A/M/C por artista (2026-09-08):** en **Comidas → Gestor** (`MealsManager` `fimbaMode`), `findFimbaArtistMealCoverageGaps` (`mealLogistics.js`) exige Almuerzo/Merienda/Cena desde el primer tagged meal hasta el último (bookends: no pide servicios antes del inicio ni después del fin; Desayuno/Catering anclan borde pero no se exigen). Omite artistas con `requiere_comidas === false`. Panel `FimbaMealCoveragePanel`: lista artista + huecos; **Crear automáticamente** por hueco y **Crear todas (n eventos)**; defaults A 12:30 / M 17:00 / C 21:30, `hora_fin` null, `convocados=[GRP:NONE]`, `audiencia_ofrn=none`, tag artista, locación inferida de siblings del día (`fimbaMealCoverageCreate.js`). Post-create refresca la grilla del Gestor. En **Reporte**, solo un aviso corto con enlace «Ir a Gestor».
...
- [x] **Comidas por artista + cobertura A/M/C (2026-09-08):** export Excel/ZIP por artista; alertas de huecos con Crear automáticamente / Crear todas; H. Fin editable sin auto-fill; wipe `hora_fin` comidas gira 12 (29 filas).
...
| `src/utils/mealLogistics.js` | meal-related / cobertura gaps / `resolveEventHoraFinForSave` |
