...
2. **Trayectos** — planilla cronológica; columnas de boarding por unidad (filtrar un vehículo para la secuencia completa). **Modo edición** (staff, no RO): fecha / com / actividad+obs / **Origen** (`LocationSelectWithCreate` → `id_locacion`) / **Vuelo** inline / vehículo FIMBA 0–1; semáforo sticky a la izquierda. **H. Llegada** + **Destino** calculadas (tono secundario). Fuera de modo edición: doble clic en la **fila** → edición completa + tilde confirma / X·Esc cancela; lápiz = modal completo.
...
- Locación (parada actual / columna planilla **Origen**): `locaciones.nombre` (+ ciudad) vía `eventos.id_locacion`. Editable en **`FimbaEventoFormModal`** y en planilla Transportes (doble clic / modo edición) con `LocationSelectWithCreate` (label **Locación (parada actual)** en modal transporte; header planilla **Origen**). Persiste con Guardar (`saveFimbaEvento.id_locacion`) o `patchFimbaEventoPlanilla` (null limpia). **No** usar texto `Destino:` en `descripcion` para transporte. Filtros Agenda/Backline (multi-select de locaciones existentes) **no** crean locaciones — ahí `MultiSelectDropdown` está bien.
- **H. Llegada (planilla Transportes):** columna propia entre Origen y Destino; display = `hora_fin_display` (`resolveHoraFinDisplay` = hora com del next asignado al vehículo). Calculada, no editable; tono secundario (gris). Sin next → **—**.
