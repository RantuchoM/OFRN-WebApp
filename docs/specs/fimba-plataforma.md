...
| `eventos.tecnica` | Boolean OFRN: oculta el evento en **Agenda OFRN** para músicos / `consulta_general` (`canSeeTechEvents`). **FIMBA no filtra por este flag** (Agenda / Transportes / Comidas / consulta artista): muchos conciertos solo-FIMBA lo usan precisamente para no aparecer en la orquesta. Ver § Agenda · `tecnica`. |
...
- **Planilla trayectos (UI):** scroll en `.fimba-planilla-scroll` (`overflow: auto` ambos ejes) dentro de `.fimba-planilla-card` con `max-height: min(70vh, calc(100dvh - 11rem))` + `overflow: hidden` (mismo patrón sticky que Comidas: el scrollport es la planilla, no el documento; `thead th` `position:sticky; top:0; z-index:20` + fondo sólido + hairline shadow — sin `top` distinto de 0 para no tapar la 1ª fila). Tabla `width: max-content`. Columnas: Origen (org FIMBA/OFRN) · Fecha · **H.Inic.** (`hora_inicio`) · **Locación** (parada) · + · **H. Llegada** (calculada) · **Destino** (calculada) · **Detalle** · **Vuelo** · Vehículo · **Artistas** · **Subidas** · **Bajadas** · Tránsito/cap · acciones. Sticky izq.: Origen org + Fecha + H.Inic. (**compactas**: checkbox ~1.75rem; Origen ~4.25rem + badge chico; Fecha ~4.5rem; H.Inic. ~3.5rem; `left` sticky recalculado). **H. Llegada** y **Destino** usan tono secundario (`.fimba-planilla-col-secondary`: texto `#64748b`, fondo `#f1f5f9`). **Destino** (calculada): max ~7.5rem, texto truncado + `title` (CSS: `.fimba-planilla-destino` `overflow:visible` + inner `display:flex; width:100%; min-width:0; overflow:hidden` + text `flex:1 1 0%` / `text-overflow:ellipsis` + hairline padding — estable al zoom; no `overflow:hidden` en el `td` ni `inline-flex` shrink-to-fit). **Acciones:** sticky derecha (`right:0`, min ~2.75rem) + kebab ⋮ (`IconMoreVertical`, menú portal z-110) con Editar / Duplicar / **Crear movimientos intermedios** / Eliminar; en edición de fila siguen visibles **tilde** + **X** (fuera del kebab). **Vuelo** = línea `Vuelo:` de `eventos.descripcion` (`decodeFimbaTrasladoDescripcion` / `ev.vuelo`; paridad con Agenda). **Artistas** = tags `eventos_fimba_propuestas` (paridad chips con Agenda). Subidas/Bajadas = boarding por parada (`fimba_propuesta_rutas` + reglas OFRN). **Tránsito/cap** = a bordo al salir / capacidad; hover (portal z-110) lista grupos/artistas + Orquesta + Reserva del evento. Toolbar: **Programar transporte** + **Ver otros eventos** (ver bullets Trayectos). Columna **+**: alta inline de parada intermedia (no modal; spinner en el botón mientras crea). Shell FIMBA (`.fimba-main` / `.fimba-header-inner`): `width: 90%` + `max-width: 90vw` (sin tope fijo en px; en mobile ≤640px usa 100% + padding).
- **Modo edición (Transportes):** mismo toggle que Artistas (`Modo edición` / `Salir de modo edición`, `IconPencil`, magenta). Off = vista; on = celdas inline + semáforo por fila (`fimba-sync-*`, sin leyenda). **Consulta / token RO** (`readOnly`): sin toggle. **Fuera de modo edición:** doble clic **en cualquier parte de la fila** (excepto checkbox, boarding Sube/Baja, «+», acciones, Destino) entra a **edición de fila completa**: todas las celdas inline-editables (Fecha / H.Inic. / Detalle+obs / Locación=`id_locacion` / Vuelo) quedan editables a la vez; **no** autosave en blur/cambio. A la derecha: **tilde** (`IconCheck`) confirma y persiste (`patchFimbaEventoPlanilla`); **X** (`IconX`) o **Esc** descarta el borrador. **Guard dirty (2026-09-09):** con fila dirty (draft ≠ baseline planilla + vehículo inline si aplica) no se cambia a otra fila ni se entra a modo planilla sin resolver; `ConfirmDialog` Guardar / Cancelar / Descartar (`resolveLeaveDirtyInlineRowEdit`; mismo patrón Agenda). El kebab **Editar** (antes lápiz suelto) abre `FimbaEventoFormModal` completo. **Locación** usa `LocationSelectWithCreate` (buscar + crear in situ → `id_locacion`), nunca un `<select>` plano. **En modo edición (planilla):** autosave al blur/Enter (texto) o al cambiar (fecha, locación, selects) como antes. Patch liviano `patchFimbaEventoPlanilla` (fecha, horas, actividad, vuelo, obs., **`id_locacion`**; `stripDestino: true` en transporte) — **no** reescribe flota/tags/grupos/`id_gira_transporte` ni `FimbaStopRulesManager`. Vehículo inline solo FIMBA puro con 0–1 unidad (`setFimbaEventoTransportes`, conserva plazas). **H. Llegada** / **Destino** (columnas calculadas): no se editan en la fila; Destino doble clic / IconEdit abre flujo «crear siguiente parada» (`FimbaDestinoStopModal`). **«+»** crea la parada intermedia al instante (spinner en el botón; `clientValidated` salta hard-block de libres; inserta fila optimistic en estado local + limpia `hora_fin` del tramo previo; soft refresh eventos/rutas en background) y abre **edición de fila** con foco locación (vía `beginRowEdit`, respeta dirty); tags/vehículo heredados de la fila. Mientras crea, los demás **+** quedan deshabilitados; error → banner `fimba-error`. Tránsito y Subidas/Bajadas = chips + modal. Flota: catálogo, nota OFRN, categoría y plazas inline con el mismo semáforo.
...
OFRN en **Giras** (Trayectos standalone): `calculateLogisticsSummary` colapsa a un
      `subidaId`/`bajadaId` ganador por persona×unidad (primera/última). No tocar.
OFRN en **FIMBA** (planilla Transportes / Subidas/Bajadas / tránsito / libres):
      cada fila de `giras_logistica_rutas` es un hop (`extractOfrnRidesFromRouteRules`).
      Subidas cuenta en **cada** `id_evento_subida`; Bajadas en **cada** `id_evento_bajada`.
      Parada intermedia con ↑ y ↓ muestra ambos. `en_transito` / libres / `sumRidesOccupyingWindow`
      no doble-cuentan a la misma persona si varios hops se solapan (`uniqueOfrnSeatsMatching`).
      Chips Orquesta (`summarizeOfrnStopRules`) = match de *esa* regla en *este* extremo
      (no exige que la ↑/↓ ganadora de logística caiga aquí).
      ...
      es_chofer en la regla de **esa** subida → capacitySeats=0 (rideCapacitySeats); display sí cuenta
FIMBA (rutas explícitas): fimba_propuesta_rutas (id_propuesta, id_gira_transporte, plazas, es_chofer, id_evento_subida, id_evento_bajada)
      ...
      ride abierto = subida sin bajada → ocupa bus hasta la bajada; hop-off + subida luego = nuevo ride
      tope artista (disp. Sube) = plazas ya asignadas como ↑ en *esta parada de este vehículo*
      (un hop anterior, otro vehículo o un tramo posterior no bloquean un nuevo Sube;
      el mismo artista puede subir/bajar muchas veces en la misma unidad y en otras)
...
- pestaña **Artistas FIMBA**: lista de **reglas activas** (`fimba_propuesta_rutas`: grupo/artista + cantidad + **asientos/obs. equipaje** + badge **Chofer** si `es_chofer` **de esa subida**). Cantidad, equipaje y **toggle Es chofer (en esta subida)** editables inline → `upsertFimbaPropuestaRutaStop` con **`rutaId`**. Alta: siempre **Agregar regla** (aditivo; no «Actualizar» / no «Reemplazar»). **Multi-asignación (2026-09-08):** varias ↑/↓ del **mismo artista** en la misma unidad/parada = filas distintas (`allowMultiple` default true). Fila **Reserva del evento** = plazas técnicas `fimba_evento_transportes` (editable en subida vía `upsertFimbaEventoTransportePlazas`); residual = reserva − Σ reglas de artista en esa parada. En bajada, el residual técnico que aligera se muestra (lectura). **Borrar regla:** la card usa `.fimba-row-deleting` (opacity + pulse) y deshabilita × hasta `clearFimbaPropuestaRutaStop` termina. Botón **Bajar todo** (solo bajadas): cierra todos los rides FIMBA **abiertos** a bordo en ese vehículo/parada (`alightAllFimbaAboardAtStop`); reserva residual sintética ya baja en hop. Orquesta OFRN se baja aparte (pestaña Orquesta → **Bajar todo** / `alightAllOfrnAboardAtStop`).
  - **Subida:** dropdown = artistas con tope restante **en esta parada de este vehículo** (`disp. remaining/tope` = planificada + extra equip. − plazas ya asignadas como ↑ *aquí* en *esta unidad*). Un hop anterior a bordo, un ride abierto de otro vehículo o de un tramo **posterior** (p.ej. 18/09) **no** bloquean un nuevo Sube (p.ej. Alba Carmona 15/09 10:30 Toyota aunque haya ↑ 09:50 en Chevrolet). Alta aditiva: **INSERT** aunque ya haya ↑ del mismo artista aquí o ride abierto a bordo (sin diálogo «Reemplazar»). El mismo artista puede subir y bajar **muchas veces** en la misma unidad y en otras. Reserva técnica (`plazas=0`) **no** aparece como regla activa ni consume asientos. Chofer: skip assert de cupo vehículo.
  - **Bajada — A bordo (arriba):** `listFimbaAboardAtStop` (misma lógica que opciones de bajada). Lista nombre + plazas + badge Chofer; botón **Bajar** si `canAlightHere` (ride abierto **o** bajada posterior aún no alcanzada — badge «baja más adelante») → `upsertFimbaPropuestaRutaStop` type=down (cierra o adelanta el próximo ride presente; multi no fusiona una ↓ ya fijada aquí). Formulario manual y **Bajar todo** (también adelanta cerrados presentes). Dropdown = quienes **ya están a bordo** de *este* vehículo. Tras bajar, `isOnBoardAfterStop` deja de contarlos y el tope del artista se libera. Quien nunca subió a esta unidad va al final, deshabilitado. Multi-vehículo: cada unidad tiene su propio ride; hop-off + subida posterior = nuevo ride.
- pestaña **Orquesta OFRN**: embebe `StopRulesManager` **inline** (`embedded` + **`allowMultipleAssignments`**) en la misma modal (sin segundo full-screen; evita stack z-[70] detrás del backdrop FIMBA). Props: `event`, `type`, `transportId` (vehículo), `giraId`, `passengers`/`admissionRules`/`regions`/`localities` OFRN, `giraGrupos` (`giras_grupos` + integrantes), `sortedEvents` (secuencia del vehículo), `supabase`. Tabla: `giras_logistica_rutas` (IDs integrantes numéricos; **`es_chofer` por regla de subida/trayecto**, no en `integrantes`). Modal standalone OFRN Trayectos **no** pasa el flag (sigue «Reemplazar» ante conflicto). Confirms embebidos `z-[110]`. Jerarquía de match = misma que Giras (no reimplementada).
  - **Alcances de regla:** General · Región · Localidad · Categoría · **Grupo** · Persona. **Grupo** = `giras_grupos` de la gira (`target_ids = [id_grupo]` text; fuerza 4, paridad Categoría). Match = miembros de `giras_grupos_integrantes` (roster enriquecido con `grupo_ids`; ausentes excluidos). Al crear **trayecto** Grupo, auto-incluye miembros en `giras_logistica_admision` Persona del vehículo (paridad Persona; boarding). **Admisión nativa Grupo (2026-09-07):** `TransportAdmissionModal` también ofrece alcance **Grupo** y persiste filas `giras_logistica_admision` con `alcance: "Grupo"` / `target_ids: [id_grupo]` (prio 4) sin expandir a Persona; match = mismo `matchesRule`/`personMatchesGrupoRule`. UI trayecto: «Agregar Nueva Regla» → ALCANCE **Grupo** → picker multi de grupos. Chips planilla muestran el nombre del grupo.
  - **Multi-asignación OFRN en FIMBA (2026-09-08 / 2026-09-09):** con `allowMultipleAssignments`, agregar Categoría/Grupo/**Persona**/etc. otra vez en la misma parada **INSERT** otra fila (varios chips del mismo alcance); si el alcance ya tiene ↑/↓ en **otro** evento, también INSERT (sin «Reemplazar»). Nueva ↑ Persona y **nueva ↑ Grupo** siempre INSERT (aunque haya ride abierto en esa unidad). `upsertOfrnGrupoRutaStop({ allowMultiple })` ya no mueve la subida previa. Cerrar ride abierto (↓ de un ↑ sin bajada) sigue siendo UPDATE. `alightOfrnPeopleAtStop({ allowMultiple })` no mueve una bajada previa a otro evento. `FimbaEventoFormModal` Orquesta usa el mismo flag.
  - **Espejo Grupo ↑ → ↓ (2026-09-05 / bajada 2026-09-08):** en `StopRulesManager` (OFRN Trayectos + pestaña Orquesta de `FimbaStopRulesManager`): (1) en **Reglas Activas** de una subida Grupo sin `id_evento_bajada`, acción **Crear bajada** + select de paradas posteriores del vehículo (`sortedEvents`); (2) al agregar Grupo ↑, checkbox **También asignar bajada en…** con el mismo picker; (3) en vista **Bajadas**, bloque **Grupos a bordo (sin bajada)** (`listOpenOfrnGrupoRidesAtStop`) con **Bajar grupo** → `alightOfrnGrupoAtStop` / `upsertOfrnGrupoRutaStop` type=down (cierra el ride Grupo, **no** expande a N Persona). **Bajar todo** prioriza cerrar esos Grupos y solo después Persona para el resto. Persistencia vía `upsertOfrnGrupoRutaStop` type=down (no duplica si ya hay ↓ Grupo en esa parada/vehículo; cierra ride abierto). Misma auto-admisión de miembros.
  ...
  - **Bajadas — a bordo:** lista `listOfrnPeopleAboardAtStop` + **Grupos a bordo** (`listOpenOfrnGrupoRidesAtStop`). Botón **Bajar grupo** cierra alcance Grupo; personas «vía grupo» no ofrecen Bajar Persona (evitar N reglas). **Bajar** Persona / **Bajar todo** → `alightOfrnPeopleAtStop` / `alightAllOfrnAboardAtStop` (Grupos primero, luego Persona fuerza 5). Soft refresh `onRefresh('ofrn')` → slice logistics.
...
- **UX 2026-09-05 — Borrar evento con ↑/↓ programadas:** al Eliminar (Agenda, Transportes, ConsultaAgenda editable, y lote desde la barra flotante) se consultan extremos en `fimba_propuesta_rutas` + `giras_logistica_rutas` (`findBoardingLinksForEvent(s)`). Sin vínculos → confirm simple **Eliminar**. Con vínculos → título «Eliminar evento(s) y subidas/bajadas», mensaje con desglose FIMBA/Orquesta, CTA **Eliminar y quitar ↑/↓** (`ConfirmDialog` / `useConfirmDialog`). Cascade: `clearBoardingLinksForEvent` (FIMBA: borra ride al quitar subida o extremos únicos; clear bajada deja ride abierto; OFRN: desvincula o borra regla; también limpia `giras_logistica_reglas_transportes`) y luego `deleteFimbaEvento({ clearBoarding })`. Evita huérfanos SET NULL / bloqueo FK OFRN.
- **UX 2026-09-05 — Sin `window.alert` / `confirm` / `prompt` en FIMBA:** decisiones (eliminar, duplicar, regenerar tokens, bajar todo, descartar dirty, etc.) → `useConfirmDialog` → `ConfirmDialog`/`ConfirmModal` (portal `document.body`, z-100; z-110 si está encima de otro modal). **No** se ofrece «Reemplazar» en boarding FIMBA (multi-asignación aditiva). Avisos informativos / errores de export-copy → `sonner` toast. `useConfirmDialog.alert()` = un botón «Entendido» (`hideCancel`). Clipboard fallido al copiar enlace de consulta agenda → diálogo con la URL en el mensaje (antes `window.prompt`).
...
2. **Trayectos** — planilla cronológica; columnas de boarding por unidad (filtrar un vehículo para la secuencia completa). **Modo edición** (staff, no RO): fecha / com / actividad+obs / **Locación** (`LocationSelectWithCreate` → `id_locacion`) / **Vuelo** inline / vehículo FIMBA 0–1; semáforo sticky a la izquierda. **H. Llegada** + **Destino** calculadas (tono secundario). Fuera de modo edición: doble clic en la **fila** → edición completa + tilde confirma / X·Esc cancela; lápiz = modal completo.
...
- **Detalle (modal + Agenda row-edit):** Quill `FimbaRichTextEditor` (toolbar compact: B/I/U, listas, link). Obligatorio (texto visible). Persistido como parte libre del encode FIMBA junto a `Destino:` / `Vuelo:`. Preview: `FimbaEventDetallePreview` con `sanitizeFimbaRiderHtml` + **clamp** (~3–4 líneas). Tooltip portal `z-[110]` (`.fimba-detalle-tooltip`) **solo si hay overflow real** (`scrollHeight`/`scrollWidth` > client*, re-check ResizeObserver / resize / cambio de valor); hover/focus + `cursor:help`/`tabIndex` solo entonces. HTML sanitizado completo en el tooltip; texto plano usa `white-space: pre-wrap`. Columna `.fimba-detalle-cell` (~14–22 rem). **UX 2026-09-05 — Agenda row-edit:** toolbar Quill **flotante** (absolute sobre el editor, sin inflar altura de fila); fila en edición conserva lavado magenta + **borde dorado** inset (`--fimba-gold` / `.fimba-agenda-row--editing`). Cards móviles siguen abriendo modal (sin Quill inline). Planilla Transportes: preview clamp; Modo edición sigue texto plano / markup → modal.
...
- Locación (parada actual / columna planilla **Locación**): `locaciones.nombre` (+ ciudad) vía `eventos.id_locacion`. Editable en **`FimbaEventoFormModal`** y en planilla Transportes (doble clic / modo edición) con `LocationSelectWithCreate` (label **Locación (parada actual)** en modal transporte; header planilla **Locación**). Persiste con Guardar (`saveFimbaEvento.id_locacion`) o `patchFimbaEventoPlanilla` (null limpia). **No** usar texto `Destino:` en `descripcion` para transporte. Filtros Agenda/Backline (multi-select de locaciones existentes) **no** crean locaciones — ahí `MultiSelectDropdown` está bien.
- **H. Llegada (planilla Transportes):** columna propia entre Locación y Destino; display = `hora_fin_display` (`resolveHoraFinDisplay` = hora com del next asignado al vehículo). Calculada, no editable; tono secundario (gris). Sin next → **—**.
- **Destino (planilla Transportes + modal transporte)**: **calculado**, no persistido en el evento actual. Fuente = siguiente parada **asignada a este vehículo** (`fimba_evento_transportes` / `eventos.id_gira_transporte`), no un endpoint de ride que vive en otra unidad. Helper: `nextAssignedStopInVehicleSequence`. Label: `formatNextStopDestino(next)` → locación del next; sin locación → **`(Sin locación)`**; sin next asignado → **`Sin siguiente parada`**. Tono secundario junto a H. Llegada. La secuencia de boarding (quién sube/baja) sigue incluyendo endpoints para el Δ.
...
- **Columna «+» entre H. Llegada y Destino**: botón *Insertar evento intermedio* (solo si la fila tiene vehículo primary; oculto/`disabled` en `readOnly`). Abre `FimbaEventoFormModal` en **create** prefíillado: mismo `id_gira` (vía edición), mismo `id_gira_transporte` → `fimba_evento_transportes` (plazas 0), `audiencia_ofrn = none`, tipo por `eventTypeIdForCategoria` del bus (11/12/35), actividad «Parada intermedia», locación vacía. **Fecha/horas** vía `defaultGapFillEventSchedule` (completar hueco **hasta→desde**):
...
- **Hora fin / H. Llegada (planilla Transportes)**: **no** se toma de `eventos.hora_fin` guardada. Display = `hora_inicio` del siguiente evento **asignado a este vehículo** (`hora_fin_display.isCalculated` / `source: next_event`) en columna **H. Llegada** (tono secundario; itálica si calculada). Sin next con hora → **—** (`source: missing`); no se muestra un fin huérfano como si viniera de la agenda. Helper: `resolveHoraFinDisplay` + `nextAssignedStopInVehicleSequence`. Sticky **Com** solo muestra/edita hora de comienzo. `saveFimbaEvento` con `usa_transporte` persiste `hora_fin` null. **Elegir destino** usa esa hora (o fallback) como `hora_inicio` de la parada nueva y **limpia** `hora_fin` del actual. El botón «+» gap-fill no escribe `hora_fin` en vecinos.
...
**UI:** `FimbaHoteleriaReports` (hub = `RoomingReportsHubModal` OFRN) · `FimbaComidasReportModal` · `FimbaTransportReportsMenu` (por vehículo en flota **y** en chip Vehículo seleccionado; modal rango = `CnrtExportModal`; PDF/Excel).
...
| **Reporte de habitaciones** (RoomingReport) | Print/PDF + **Excel** | Hub → Reporte habitaciones; tarjeta Hotelería / `FimbaRoomingPanel` / Artista → Rooming PDF / **Excel rooming** | Print/PDF + Excel 2 hojas (+ hoja por hotel si Todos) + texto clipboard | **Filtro por hotel** (selector Todos / hotel). OFRN: `ofrnRoomingExport.js`. FIMBA: hoja **Habitaciones** = 1 fila/hab. con ocupantes `(IN → OUT)`; **Rooming plazas** = 1 fila/cama; con varios hoteles y filtro Todos, hoja extra por hotel. Spec: `docs/specs/rooming-export-por-hotel.md`. |
...
| **MealsReport** por evento (OFRN) | Print/PDF + texto + **cuadro** dietas | **Comidas** → pestaña **Reporte** (`MealsReport` `fimbaMode`) | Print/PDF + Texto pedido + tabla | Misma lógica OFRN (filtros compartidos Gestor↔Asistencia↔Reporte); en `fimbaMode` desglosa dietas de artistas desde `fimba_participantes.tipo_alimentacion` (+ «Otros» agregado); columna **Art.** = solo residuo planificada−nominados |
| **Texto pedido** (comidas por estadía) | Clipboard | Hotelería / Artista → **Reportes comidas** | Clipboard | Resumen regímenes nominados + detalle (check-in/out) |
| **Reporte comidas por estadía** | Print/PDF + texto | Hotelería / Artista → **Reportes comidas** | Print/PDF + texto + Excel | Cubiertos por día (check-in/out) + regímenes; **sin** asistencia por evento OFRN |
| **Excel comidas** (estadía) | — | Hotelería / Artista / modal comidas | Excel 2 hojas | Ya existía |
| **Exportar CNRT** | PDF/Excel | Transportes → menú ⬇ por vehículo (tabla flota) **y** ⬇ en chip Vehículo seleccionado → Exportar CNRT | PDF/Excel (`downloadStyledPassengers`) | OFRN = DNI personal; FIMBA = nominados del artista hasta plazas, resto sintético «Plaza N». Aviso post-export |
| **Cronograma de paradas** | PDF (/Excel vía handler) | Menú vehículo / chip → Cronograma de paradas | PDF/Excel (`generateStopsOnly*`) | Secuencia `buildVehicleBoardingSequence.sortedEvents` |
| **Hoja de ruta** | PDF/Excel | Menú vehículo / chip → Hoja de ruta | PDF/Excel (`buildRoadmapExportData` + generate*) | Sin alinear viáticos OFRN; ups/downs desde rides |
...
**Gaps honestos (UI tooltip / modal):** boarding FIMBA es por **plazas de artista** (`fimba_propuesta_rutas`), no nómina de participantes en el bus. CNRT/hoja de ruta rellenan con participantes de la propuesta (heurística por orden) y documentan plazas sin nominar. El reporte por estadía (`FimbaComidasReportModal`) no desglosa asistencia OFRN por evento; el **MealsReport** de la pestaña Comidas → Reporte sí (paridad Logistics).
...
- **`eventos.tecnica` vs FIMBA (2026-09-07 — audit):** en OFRN, `tecnica: true` se oculta a músicos/`consulta_general` vía `deriveAgendaPermissions.canSeeTechEvents` + filtro Téc en `UnifiedAgenda` / `useAgendaFilters`. En FIMBA **no hay** exclusión por `tecnica` en `listFimbaAgenda`, planilla Agenda/Transportes, consulta artista/token ni `MealsManager` (`fimbaMode`). Motivo: el flag se reutiliza para esconder eventos solo-FIMBA de la agenda de orquesta; filtrarlos en FIMBA rompería Agenda de artistas y Comidas. Visibilidad FIMBA sigue solo por tags/flota/estadía + `include_ofrn` / chips origen (**default Todos** muestra FIMBA+OFRN; **Solo FIMBA** oculta pure-OFRN, técnicos o no). Staff editores: ver logística OFRN marcada Téc → chip **Todos** / **Solo OFRN** o Tutti/grupo. Artista/consulta: sin cambio (tampoco se ocultan por `tecnica`).
...
- **Agenda de artista** (`id_propuesta` en `listFimbaAgenda` / ficha `FimbaConsultaAgenda` / filtro artista en planilla):
  - Eventos tagged al artista (no-transporte) **+** paradas de transporte donde el artista tiene **↑ o ↓** (`fimba_propuesta_rutas`).
  - **Con filtro artista:** los traslados intermedios (piernas del vehículo sin boarding propio del artista, aunque estén tagged o el artista vaya «a bordo») **no** se listan. Sin filtro artista: la agenda muestra todos los transportes como hasta ahora.
  ...
  - Filtro: `eventMatchesPropuestaRouteFilter` / `eventMatchesAgendaEntityFilter` (tags no-transporte ∪ ↑/↓ boarding). Transporte = solo boarding (`isTransportTipoEvent`); tags heredados no alcanzan. **Regresión 2026-09-02:** `isFimbaRideAboardAtStop` con secuencia exige que el evento esté en la secuencia del vehículo (ride abierto ya no marca conciertos/check-ins ajenos → bug «171 de 171»). **Product 2026-09-09 — solo ↑/↓ en transporte:** el filtro artista dejó de incluir paradas intermedias «a bordo»; `listFimbaAgenda` solo amplía ids con extremos de ruta (no todas las paradas del vehículo).
  ...
  - **UX 2026-09-04 — Planilla Transportes más compacta:** sticky Origen/Fecha/Com·Fin + checkbox más angostos; Destino max ~7.5rem con ellipsis+title; acciones en kebab ⋮ (Editar/Duplicar/**Crear movimientos intermedios**/Eliminar, portal z-110); ✓/X de row-edit siguen fuera del menú.
  ...
  - **Fix 2026-09-05 — Ellipsis/kebab zoom-stable (Agenda + Transportes):** (1) Destino: `overflow:hidden` solo en `.fimba-planilla-destino-inner` / `.fimba-planilla-destino-text` (no en el `td` — el doble clip subpíxel comía la «…»); `min-width:0` + hairline `padding-right:1px` en el truncador; botones/badge del Destino `flex-shrink:0`. Agenda Destino usa `.fimba-agenda-destino` + mismo `.fimba-planilla-destino-text`. (2) Chips boarding: chip `overflow:visible`; ellipsis solo en label; tags/× no shrinkean. (3) Acciones ⋮: sticky `right:0` + `min-width`/`width` 2.75rem (crece a 4.5rem con ✓/X) en `.fimba-planilla-actions` / `.fimba-agenda-actions` para que el kebab no salga del scrollport ni se colapse al zoom 90–150%.
  ...
  - **Feature 2026-09-07 — Crear movimientos intermedios (kebab fila):** opción en menú ⋮ de planilla Transportes. Abre el mismo `FimbaRecorridoIntermedioModal` con `variant: 'aroundEvent'`. Defaults: salida = locación de la parada anterior del vehículo a **inicio − 30′**; medio = evento ancla (solo lectura, no se duplica); retorno = misma locación anterior a **fin + 30′**. Fin = `hora_fin` persistida o +60′ desde inicio (**no** `resolveHoraFinDisplay` / com del next — fix 2026-09-08: eso colocaba el retorno después del next y solo se veía la ida). Si fin+30 no entra antes del next, default = punto medio ancla→next. Locación/fecha/hora de salida y retorno editables. Persiste con `createMovimientosIntermediosAroundEvent` (2 paradas vía `createDestinoStopEvent`; valida prev < salida < ancla < retorno < next antes de crear; error parcial si falla la 2ª). Deshabilitado si no hay prev / sin locación / misma locación que prev / sin hueco (`no_gap`). Aviso si ya hay paradas entre prev y ancla. Helpers `buildMovimientosIntermediosDefaults` / `createMovimientosIntermediosAroundEvent`.
  - **Fix 2026-09-08 — Movimientos intermedios ida+vuelta:** defaults dejaban de usar Hora Fin de planilla (next.com); validación de hueco y mensaje si solo se crea la salida.
  ...
  - Carga consulta/API: `listFimbaAgenda` amplía ids con extremos ↑/↓ de las rutas del artista y filtra con `eventMatchesPropuestaRouteFilter` (transporte = solo boarding).
  - Planilla staff: carga FIMBA (`listFimbaAgenda({ include_ofrn: false })`) + filtro artista **en memoria** con `propuestaRoutes` (↑/↓; sin piernas intermedias a bordo). Orquesta solo si el usuario marca Tutti o un grupo.
...
- **Feature 2026-09-04 — Multi-select + «Editar en lote» (Agenda + Transportes):** columna checkbox a la izquierda de cada fila (oculto en `readOnly` / consulta). Header = seleccionar todos los **visibles filtrados**. **UX 2026-09-05 — barra flotante:** al tildar ≥1 evento aparece `FimbaBulkSelectionBar` (portal `document.body`, **bottom-left**, z-80, debajo de modales z-100): contador + **Editar en lote** + **Eliminar** + × limpiar; se oculta al vaciar la selección. (La toolbar rosa inline del header se retiró.) Modal portal z-100 (`FimbaBulkEditModal`). **Eliminar (lote):** confirm `ConfirmDialog`; si alguno tiene ↑/↓ FIMBA/OFRN, botón **Eliminar y quitar ↑/↓** + resumen agregado (`findBoardingLinksForEvents` / `deleteFimbaEventosBulk` / `clearBoardingLinksForEvent`). Secciones del modal (horarios / tags / vehículo) arrancan **desmarcadas**; Aplicar solo corre las que el usuario activa y queda disabled si ninguna tiene acción. **Horarios (ambos tabs):** (A) anclar nueva fecha/hora del evento más temprano y conservar offsets relativos; (B) desplazar todos ± horas/minutos. Persistencia: `hora_inicio` (+ `fecha` si rola día); `hora_fin` solo si no es fin derivado de transporte (`eventUsesDerivedHoraFin`). APIs: `bulkShiftFimbaEventosSchedule`. **Agenda-only tags:** agregar/quitar artistas (`eventos_fimba_propuestas` vía `setEventoFimbaPropuestas`) y grupos OFRN (`eventos_grupos` + `audiencia_ofrn` vía `bulkPatchFimbaEventosTags` / `setEventoGrupos`). **Transportes-only:** **Mudar** a otro vehículo (`bulkReassignFimbaEventosVehiculo`). **Bugfix 2026-09-09:** antes solo reescribía `fimba_evento_transportes` (delete+insert) y **no** migraba `fimba_propuesta_rutas.id_gira_transporte` / `giras_logistica_rutas.id_transporte_fisico` → los chips ↑/↓ «desaparecían» del vehículo nuevo (reglas huérfanas en el id viejo). Ahora: (1) reasigna flota preferiendo UPDATE in-place del `id_gira_transporte`; (2) **migra** las reglas ↑/↓ de los extremos seleccionados al vehículo destino (no borra ni recrea paradas); (3) conserva plazas técnicas; omite pure-OFRN y filas contexto; SIN SERVICIO limpia flota y **deja** ↑/↓ intactas. **UX preview:** al Aplicar con mudanza activa, `ConfirmDialog` z-[110] muestra conteo de paradas, Antes→Después (vehículo actual(es) → destino), lista por fila (fecha/hora/origen) y nota de cuántas ↑/↓ FIMBA/Orquesta se migran; confirmar aplica. Selección se poda al cambiar filtros.
- Filtro planilla: **Todos / Solo FIMBA / Solo OFRN** (chips; **default Todos** — `include_ofrn` al montar; ocultos cuando hay Tutti o un grupo OFRN). Banner **Filtros activos** compacto (una fila en desktop: chips + contador + acciones; padding reducido; wrap solo si hace falta) cuando hay algún filtro no por defecto (chips resumen + contador «X de Y eventos» donde **Y = agenda base cargada** (`eventos.length`) y **X = tras todos los filtros** + **Limpiar filtros**; con filtro artista/grupo/tutti activo, staff `ofrn` / `editor_general` ve también **Copiar enlace de consulta** junto a Limpiar; hint corto inline solo si «Solo FIMBA», sin grupos OFRN y **sin** filtro artista — no fuerza segunda línea). Multi-select con **checkboxes** (`MultiSelectDropdown`) de **categoría de tipo** (`id_categoria` / `categorias_tipos_eventos`; vacío = todas). Opciones = **tabla `categorias_tipos_eventos`** (catálogo vivo de Datos) ∪ tipos OFRN ∪ categorías de filas cargadas (`mergeFimbaAgendaCategories`). Alta nueva en BD (p.ej. **Catering**, **Reunión**) aparece aunque no haya tipo ni eventos. Multi-select de **locación** (`id_locacion` de filas cargadas; vacío = todas; sin `id_locacion` se ocultan si el filtro está activo). Multi-select de **artista** (`fimba_propuestas.id`; vacío = toda la edición). Multi-select de **grupos OFRN** (`giras_grupos.id` de la gira enlazada + opción **Tutti** sentinel `tutti`, **off por defecto**; placeholder «Ninguno»). Triggers acotados al ancho del contenedor (~220px; grupos ~180px); panel desplegable max 320px con labels truncados (`…`). **Toolbar UI:** fila superior `fimba-agenda-toolbar-head` con título «Planilla» + acciones (**Copiar enlace de consulta** solo staff `ofrn` / `editor_general` + **Descargar PDF**); debajo, **todos los filtros en una fila horizontal** (`fimba-agenda-filters-row`: Buscar → chips origen → categoría → locación → artista → grupos; `flex-wrap: nowrap` desde 1100px; wrap solo en viewports estrechos). **Búsqueda** debounced 250ms (patrón UnifiedAgenda: pill + clear) sobre actividad, tipo, categoría, locación/ciudad/dirección, destino calculado, vuelo, obs., artistas, grupos y vehículos. **Artista** acota la agenda FIMBA (no fuerza origen Todos ni incluye Tutti). **Tutti / grupos OFRN** son opt-in: **incluyen** esas convocatorias orquesta (unión con FIMBA / artista) y `listFimbaAgenda({ include_ofrn: true })`. Chip **Todos** / **Solo OFRN** también pide orquesta. **Carga:** con default **Todos**, mount pide `include_ofrn: true`; **Solo FIMBA** omite orquesta. El resto de filtros (artista, grupo, origen, categoría, locación, búsqueda) aplican **en memoria**. Solo el mount inicial muestra «Cargando agenda…» y **no** espera logistics OFRN ni `fimba_propuesta_rutas` (van en background; Destino/As.Equipaje se completan al llegar); mutaciones (guardar/eliminar) y el opt-in OFRN usan soft refresh con «Actualizando…» inline. **Días sin filas:** la planilla no inserta separadores por fecha; si un día no tiene eventos visibles (p. ej. solo OFRN ocultos por «Solo FIMBA»), ese día no aparece — no es hueco de datos.
- **UX 2026-09-05 — Feedback al aplicar filtros (Agenda + Transportes):** controles actualizan al instante; la planilla usa `useDeferredValue` sobre los inputs de filtro (Agenda: artista/grupo/origen/categoría/locación/búsqueda; Transportes: artista client-side + chips vehículo + origen + «otros eventos»). Mientras el valor diferido está stale (o Agenda `refreshing` por opt-in OFRN), tras **≥50ms** (`useFimbaFilterPending`) se muestra «Aplicando filtro…» inline junto al título/select y overlay no bloqueante (`.fimba-filter-applying` / `FimbaFilterApplyingOverlay`, `pointer-events: none`) sobre `.fimba-filter-pending-host` (scroll Agenda / card planilla Transportes). Evita flash en filtros instantáneos. Lo lento: Agenda = `eventMatchesAgendaEntityFilter` + re-render filas; Transportes = filtro ruta artista + recompute Subidas/Bajadas/tránsito (`boardingMetricsForEventRow`) al acotar vehículo.
...
- **Columnas planilla Agenda / consulta artista:** **Evento** (badges FIMBA/OFRN; antes «Origen») · Fecha · Com·Fin · Tipo · Detalle · **Origen** · **Destino** · **Vuelo** · Vehículo · As. Equipaje · …
  ...
  - **UX 2026-09-09 — Guard row-edit dirty (Agenda + Transportes):** no cambiar de fila inline dirty sin Guardar/Descartar/Cancelar (`ConfirmDialog` z-100; `useConfirmDialog.secondaryAction`).
...
- Visual: badges origen FIMBA / OFRN; filas pure-OFRN muting cyan; filas contexto grises; columna convocatoria (Tutti / chips de grupo) en Agenda; en Transportes columnas origen + vehículo(s).
  ...
  - **Feature 2026-09-07 — Admisión nativa por Grupo OFRN:** `TransportAdmissionModal` / `giras_logistica_admision` con alcance **Grupo** (prio 4, `target_ids = [id_grupo]`). No expande a Persona desde la UI de admisión; el auto-Persona al crear trayecto Grupo en `StopRulesManager` se mantiene.
  - **Feature 2026-09-05 — Espejo Grupo ↑→↓:** desde Gestionar Subidas (regla Grupo activa o al asignar), crear la misma regla en bajada en una parada posterior del vehículo sin duplicar. Ver bullet Orquesta «Espejo Grupo ↑ → ↓».
...
- **Catering** (2026-09): categoría `categorias_tipos_eventos.nombre = 'Catering'` (id 9) + tipo `tipos_evento.nombre = 'Catering'` (id 34, color `#ea580c`). **No** es Comidas (`id_categoria = 4`): `isMealEvent` sigue siendo solo cat. 4 / D-A-M-C. **Sí** entra al **gestor de comidas** OFRN/FIMBA vía `isMealRelatedEvent` / `isCateringEvent` (slot sintético `Catering`). Filtro de clase en matriz: **Todos | Comidas | Catering** (default Todos). Subtipos en Datos → Tipos de Evento. Filtros agenda FIMBA leen la tabla de categorías.
- **Gestor de comidas FIMBA** (`/fimba/edicion/:id/comidas`, tab **Comidas**): pestañas locales **Gestor | Asistencia | Reporte** (paridad Logistics). **Gestor** embebe `MealsManager` en `fimbaMode` sobre la gira `fimba_ediciones.id_gira`. Lee los mismos `eventos` OFRN (Comidas + Catering); roster vía `useLogistics` (ausentes fuera). Columna **Artistas** = `eventos_fimba_propuestas` (`FimbaEventArtistasTagsCell` → `saveRow`/`refreshGridData` preservan tags); columna **Grupos** → `setEventoGrupos` en `saveRow` al cambiar `selectedGrupos` (debounce vía `handleGridChange`). **Grupos sin Convocados** cuentan OFRN (`mealRowHasOfrnAudience` + AND); `GRP:NONE` / ambos vacíos → 0 OFRN. Pax artistas = Σ `cantidad_planificada` con `requiere_comidas !== false` (**aditivo**; no afecta deducción OFRN). Badge Comensales = `formatComensalesBadgeLabel(n OFRN · m artistas)` + `−N` deducción si aplica (sin badge magenta `+n` ni `×N mismo slot`). Filtros matriz: **Locación** + **Artista** multi-select + **Todos|Comidas|Catering** + servicios; **Limpiar filtros** resetea todos. `mealFilters` compartidos entre las 3 pestañas (`createDefaultMealFilters` / `filterMealManagerRows`). `grid` completo en memoria; filtros solo ocultan y re-ordenan con `sortMealManagerGrid` (fecha → D/A/M/C/Catering → hora → id) para que Catering no quede pegado al final del walk. Renombrar: sin «Nadie» + sufijo siglas ` - XX / YY`. Delete: fila gris + spinner sin hang.
- **Artistas sin comida (`requiere_comidas === false`, 2026-09-08):** flag en `fimba_propuestas` (UI: «no se les provee comida» / Sin comidas en Hotelería, Artistas, Rider). En **Comidas** no aparecen: catálogo picker (`mealsOnly` / `filterFimbaPropuestasForMeals`), chips de tags, filtros Artista (Gestor/Asistencia/Reporte), export por artista, cobertura A/M/C ni alertas. Pax/dietas/siglas ya los omitían. Guardar tags desde Comidas **conserva** esos tags en el evento (siguen visibles en Agenda/Transport). Helper: `fimbaPropuestaRequiereComidas` / `filterFimbaPropuestasForMeals` (`mealLogistics.js`); paridad `propuestaRequiereComidas` en `fimbaService`.
- **Reporte comidas FIMBA (texto + cuadro, 2026-09-07):** pestaña **Reporte** reusa `MealsReport` con `fimbaMode` + `giraGrupos` / `eventos_grupos` (misma elegibilidad OFRN que el Gestor). **Cuadro** = tabla por servicio (fecha/hora/lugar + Total + columnas de dieta OFRN + dietas artistas FIMBA desde nominados). **Texto** = botón **Texto pedido** → modal portal z-100 (copiar clipboard). **Exportar PDF** = print del cuadro. **Por artista (2026-09-08):** modal → Excel multi-hoja (Índice + cuadro por artista) y/o ZIP de textos pedido; respeta filtros excepto que el batch itera artistas (o el filtro Artista activo); omite `requiere_comidas === false`. Exports respetan filtros activos (clase/servicio/locación/artista/convocados). Asistencia = `MealsAttendance` (misma gira OFRN + grupos). El modal Hotelería **Reportes comidas** (`FimbaComidasReportModal`) sigue siendo el reporte por **estadía**/regímenes nominados (distinto del MealsReport por evento).
- **Cobertura A/M/C por artista (2026-09-08, estadía 2026-09-08):** en **Comidas → Gestor** (`MealsManager` `fimbaMode`), `findFimbaArtistMealCoverageGaps` (`mealLogistics.js`) exige Almuerzo/Merienda/Cena pendientes. **Ventana preferida = estadía Hotelería**: `stayDateFromEventOrMirror` sobre `id_evento_checkin`/`id_evento_checkout` (tipos 22/23) o espejo `checkin_at`/`checkout_at` de `fimba_propuestas` (+ Early/Late). Slots esperados vía `enumerateExpectedAmcSlotsForStay` / `amcFlagsForStayDay` (`fimbaMealsStay.js`): llegada = Merienda+Cena (+Almuerzo si Early); intermedios = A+M+C; salida = Almuerzo solo si Late (checkout canónico 10:00; check-in 14:00 → Merienda en llegada). **Sin check-in/out** → fallback primera↔última comida tagueada (bookends previos); sin estadía ni tags → omitido con nota. Catálogo `propuestas` alimenta artistas sin tags. Omite `requiere_comidas === false`. Panel `FimbaMealCoveragePanel`: lista artista + pendientes; **Crear automáticamente** / **Crear todas (n eventos)**; defaults A 12:30 / M 17:00 / C 21:30, `hora_fin` null, `convocados=[GRP:NONE]`, `audiencia_ofrn=none`, tag artista, locación inferida (`fimbaMealCoverageCreate.js`). Post-create refresca la grilla. En **Reporte**, aviso corto con enlace «Ir a Gestor» (pasa `propuestas` para la misma lógica).
- **Hora Fin en MealsManager (OFRN + FIMBA, 2026-09-08):** columnas **H. Inic.** + **H. Fin** (junto a Servicio); vacío por defecto; persiste `eventos.hora_fin` (null si vacío). Comidas/Catering **no** inventan `hora_fin` al crear (`resolveEventHoraFinForSave`); agenda UnifiedAgenda / calendarios semanal-músico tampoco copian inicio→fin en tipos meal-related. Temp rows / auto-create coverage: `hora_fin` vacío/null.
- **"+" sibling add (OFRN + FIMBA, 2026-09-08):** el `+` de fila ya no crea a ciegas mismo día/servicio. Abre mini-modal portal `z-[100]` (`SiblingMealAddModal`) con **Fecha** + **Servicio/tipo** pre-cargados de la fila, editables antes de **Crear fila**. Temp row: `hora_fin` vacío; **sin** copiar tags artistas ni convocados (mismo patrón previo). Tras crear, `sortMealManagerGrid` ubica la fila.
- **Fix 2026-09-07 — Dietas artistas en MealsReport:** antes `fimbaMode` volcaba todo el pax (`cantidad_planificada`) en columna **Art.** sin discriminar. Ahora carga `fimba_participantes` de las propuestas tagueadas y reparte en columnas Regular / Vegetariano / Vegano / Celíaco / Sin TACC / Otros (`fimbaArtistMealDietBreakdown` en `mealLogistics.js`). **Art.** queda solo para cupos sin nominar (`planificada − activos`). Con filtro Artista, el desglose se recalcula solo con esas propuestas (OFRN del servicio se mantiene). Notas libres de `otro` se agrupan en **Otros** (detalle fino sigue en Hotelería / Excel comidas).
- **Deducción orquesta ↔ grupo (OFRN + FIMBA):** si un evento **orquesta** (sin `eventos_grupos`) coincide con uno o más eventos **de grupo** en la misma clave `fecha|servicio|id_locacion` (`mealCoincidenceKey`), los integrantes elegibles del/los grupo(s) se **restan** del headcount/listado orquesta (`deductGrupoMembersFromOrchestraEligible`). UI: badge `−N` en Comensales + bloque «Descontados» en el modal. Ausentes = sin logística. Artistas FIMBA no se restan.
- **Nadie (orquesta no come):** convocado exclusivo `GRP:NONE` / `ROSTER_CATEGORIES.NONE` en columna Convocados OFRN (chip «Nadie»). Persiste en `eventos.convocados`; 0 integrantes OFRN; artistas FIMBA siguen contando vía tags. **No** setea `audiencia_ofrn=none` (no oculta filas). Sin alerta naranja de convocados vacíos. Renombrar **no** escribe la palabra «Nadie» en el título.
- **Sobre-inclusión mismo turno:** turno = `fecha|servicio` (`mealTurnoKey`; distinto de deducción que usa locación). Detecta integrantes OFRN (post-deducción) o tags FIMBA en ≥2 comidas del turno → banner resumen + `IconAlertTriangle` por fila + modal detalle (`findMealTurnoOverInclusions`).
- **Fusionar comidas (2026-09-08):** bulk **Fusionar** en el panel flotante del Gestor (≥2 filas reales, misma `fecha`+`servicio`). Survivor = más completo (locación/hora/…) o menor `id`. Une `eventos_fimba_propuestas`, `eventos_grupos` y `convocados` (unión positiva; `GRP:NONE` cede si hay otros). Hard-delete del resto + recalc turno. Spec: `docs/specs/meals-fusionar-comidas.md`.
- **Recalc turno al editar comida:** mutar convocados/grupos/artistas (u otros campos pax) rematerializa hermanas del mismo turno en el grid (`MealsManager` + `MEAL_PAX_AFFECTING_FIELDS` / `mealTurnoKey`); sin full reload salvo flujos que ya lo hacían.
- **UX FIMBA Comidas (`fimbaMode`):** tinte de fila ~8% del color de servicio (`getMealServiceStyle().print.border`; dirty/saving/selected ganan); separadores de día (`fimba-day-divider-row`, no sticky); thead sticky en scrollport `overflow-auto` (`sticky top-0 z-20` + `bg-slate-100` sólido); headers two-line «Convocados / OFRN» y «Grupos / OFRN»; columna Lugar estrecha (`w-28` / `max-w-[7.5rem]`, truncate + tooltip).
- **Filas solo-artista FIMBA:** eventos reales con tag distinto por comida; Manager OFRN las oculta (`isFimbaArtistOnlyMealEvent`); FIMBA Comidas las conserva. Tags en select embebido; chips RO en OFRN / editables en `fimbaMode`. **Visibilidad Agenda:** toggle «con FIMBA» / `isFimbaOnlyAgendaEvent`. No mass-delete.
...
- **Tokens CSS** (`--fimba-*`): definidos en `.fimba-root` **y** `.fimba-modal-backdrop` porque los modales usan `createPortal(..., document.body)` y salen del árbol de `.fimba-root`. Sin eso, `var(--fimba-*)` se invalida en el portal → botones selected/primary con `color: #fff` quedaban invisibles (blanco sobre blanco). Incluye `--fimba-magenta` (= `--fimba-accent` `#d73289`). **MealsReport** chips Todos|Comidas|Catering (y botones `fimbaMode` con texto blanco) usan fill sólido `#d73289` — no `bg-[var(--fimba-magenta)]` sin fallback — para no repetir blanco-sobre-blanco si el token falta.
...
- **Buscar Artistas o Integrantes** (`FimbaArtistaPersonSearchField`): en planilla Artistas y toolbar Hotelería. Debounce ~180ms; `matchesFimbaArtistaPersonSearch` / `scoreFimbaArtistaPersonSearch` (tokens AND, sin tildes; **ranking** por score — ver `docs/specs/busqueda-texto.md`). Campos: nombre de propuesta + `apellido`/`nombre` de participantes (Hotelería = `row.personas` batcheadas; Artistas = un `listFimbaParticipantesForPropuestas` al montar/cambiar lista). AND con select Artista (Hotelería). Query vacía = todas las filas/cards en orden alfabético; con query = mejores matches primero.
...
- [x] Planilla muestra orquesta OFRN (tutti/grupos) + badges origen + filtro Todos/FIMBA/OFRN (**default Todos**) + categoría (dropdown multi). **Actividades Tutti** en Grupos OFRN (off por defecto); orquesta se carga con Todos/OFRN o al marcar Tutti/grupo
...
- [x] **Conteo por hop FIMBA (2026-09-08):** Subidas/Bajadas y tránsito cuentan **cada** ↑/↓ de `giras_logistica_rutas` + `fimba_propuesta_rutas`, no solo la primera subida y última bajada colapsadas de `calculateLogisticsSummary`. Giras OFRN Trayectos standalone sin cambio. Occupancy OFRN unique-by-persona si hops se solapan.
- [x] **Multi-asignación boarding (2026-09-08):** varias ↑/↓ del mismo artista FIMBA y misma categoría/grupo/**Persona** OFRN sin «Reemplazar»; `allowMultiple` / `allowMultipleAssignments` (OFRN Trayectos standalone intacto); bajada de Grupo abierto como alcance Grupo (no N Persona)
- [x] Bajada FIMBA libera plazas: dropdown a bordo / cierra ride (`id_evento_bajada`); tope Sube = ↑ **de esta parada en este vehículo** (no through-traffic ni otros vehículos); un hop anterior / ride abierto posterior / otra unidad no bloquean Sube; **2026-09-04** adelantar bajada + merge endpoints Concierto fuera de planilla; **2026-09-09** varios hops mismo o distinto transporte (Alba Carmona 15/09 10:30 Toyota vs ride Chevrolet)
...
- [x] **Buscar Artistas o Integrantes** (Artistas planilla + Hotelería): input debounced ~180ms; match sin tildes + ranking (`scoreFimbaArtistaPersonSearch` / `docs/specs/busqueda-texto.md`) sobre `propuesta.nombre` y nómina (`apellido`/`nombre` de `fimba_participantes`; join OFRN si viniera embebido). Hotelería filtra cards con `personas` ya batcheadas; Artistas indexa con un `listFimbaParticipantesForPropuestas` al cargar (sin N+1 por tecla) y compone AND con filtros existentes (select Artista en Hotelería). Vacío = lista alfabética completa.
...
- [x] **Conteo Subidas/Bajadas por hop (2026-09-08):** cada parada con ↑/↓ muestra boarders/alighters de *ese* stop (OFRN `giras_logistica_rutas` + FIMBA `fimba_propuesta_rutas`); no colapsar a primera ↑ / última ↓. Giras Trayectos OFRN intacto.
- [x] Agenda unificada planilla (fecha, horas, tipo, actividad, origen / destino calculado / vuelo, vehículos, PAX, tags)
- [x] Planilla agenda: badges FIMBA/OFRN + convocatoria + filtro origen (default **Todos**) + banner filtros activos + multi-select **categoría**, **locación**, **artista** y **grupos OFRN** (incluye **Actividades Tutti** off) + búsqueda debounced (tipo/actividad/lugar/personas/vehículos)
- [x] Categoría **Catering** (`20260901140559_catering_categoria_tipo`): catálogo + tipo. Filtros de agenda/modal leen **`categorias_tipos_eventos`**. Gestor de comidas OFRN/FIMBA incluye Catering vía `isMealRelatedEvent` + filtro Todos/Comidas/Catering.
- [x] **Gestor Comidas FIMBA** (`/fimba/edicion/:id/comidas`): `FimbaComidasPage` + tab nav; `MealsManager` `fimbaMode`; tags artistas + grupos auto-persist; deducción orquesta↔grupo; badge `n OFRN · m artistas`; turno over-inclusion; `GRP:NONE`; polish sticky/tinte/separadores.
- [x] **Comidas Reporte texto + cuadro (2026-09-07):** pestañas **Gestor | Asistencia | Reporte** en `FimbaComidasPage`; reusa `MealsReport` / `MealsAttendance` + `mealFilters` compartidos; `fimbaMode` desglosa dietas artistas desde participantes (Art. = residuo); Texto pedido + PDF + tabla dietas.
- [x] **Comidas por artista + cobertura A/M/C (2026-09-08):** export Excel/ZIP por artista; alertas de pendientes (ventana estadía check-in/out + fallback tags) con Crear automáticamente / Crear todas; H. Fin editable sin auto-fill; wipe `hora_fin` comidas gira 12 (29 filas).
- [x] **"+" sibling add editable (2026-09-08):** mini-modal fecha + tipo (defaults fila); sin auto `hora_fin`; sin copiar tags artistas.
...
- [x] UX 2026-09-09 — Agenda + Transportes: no cambiar fila inline dirty sin Guardar/Descartar/Cancelar (`resolveLeaveDirtyInlineRowEdit`)
...
- [x] Rooming exportable **por hotel** (selector Todos/hotel; Excel + PDF; OFRN `ofrnRoomingExport` + FIMBA `exportFimbaRoomingExcel` con hojas por hotel) — `docs/specs/rooming-export-por-hotel.md`
...
- [x] Consulta token: **agenda de read-only** del artista (`listFimbaAgenda(edicion, { id_propuesta })` → tags no-transporte **+** extremos ↑/↓ `fimba_propuesta_rutas`; sin piernas intermedias a bordo; sin merge pure-OFRN como staff al filtrar artista)
...
- [x] Agenda artista: transporte filtrado = solo ↑/↓ del artista; planilla staff / consulta / `listFimbaAgenda({ id_propuesta })` alineados (`eventMatchesPropuestaRouteFilter`)
- [x] Agenda planilla: multi-filtro artistas + grupos OFRN + **Actividades Tutti** opt-in + query params compartibles (`fimbaAgendaUrlParams`, `tutti=1`) + **Copiar enlace de consulta** (staff) + carga `listFimbaAgenda({ include_ofrn })` solo con Tutti/grupo o chip Todos/OFRN
- [x] **Actividades Tutti en Grupos OFRN (2026-09-08):** opción arriba del listado de grupos (label `Actividades Tutti`; sentinel `tutti`); off por defecto; match = OFRN sin tags de grupo (`eventMatchesTuttiAudiencia`: `audiencia_ofrn=tutti` o sin `eventos_grupos`); **aditivo (OR)** con grupos nombrados y con artistas FIMBA; planilla default **Todos**; marcar Tutti o un grupo carga `include_ofrn` (unión con agenda FIMBA)
...
- [x] **Filtro artista transporte solo ↑/↓ (2026-09-09):** oculta piernas intermedias a bordo / tagged sin boarding; `listFimbaAgenda` no amplía con todas las paradas del vehículo
...
- [x] Paridad reportes OFRN→FIMBA: pedido/texto/detalle/rooming (print+Excel); **detalle con IN/OUT por persona**; **rooming Excel 2 hojas** (habitación + plazas) + texto para Word; comidas por estadía (texto/PDF/Excel + **cubiertos por día** check-in/out); **MealsReport por evento** en Comidas → Reporte (texto + cuadro dietas + filtros); CNRT + paradas + hoja de ruta (PDF/Excel) por vehículo
...
- [x] Toggles por artista `requiere_hotel` / `requiere_comidas` (default true): planilla Artistas, ficha, modal Hotelería; exclusiones en totales/exportaciones hotel y comidas; **Comidas** oculta `requiere_comidas === false` (picker/filtros/export/cobertura; Agenda/Transport sin cambio)
...
6. Columnas planilla: **Origen** (org) · **Fecha** · **H.Inic.** · **Locación** · **+** · **H. Llegada** · **Destino** · **Detalle** · **Vuelo** · **Vehículo** · **Artistas** · **Subidas** · **Bajadas** · **Tránsito/cap** · acciones. Sticky izq. Origen/Fecha/H.Inic. Scroll en `.fimba-planilla-scroll` (`overflow:auto`, card con max-height) para thead sticky. Chips Subidas/Bajadas (clic → modal; × quita FIMBA; Reserva del evento **solo si residual > 0**). **H. Llegada** = hora inicio del next **asignado** (columna secundaria); **—** si no hay. Alerta **Sobre cupo** si en_transito > capacidad; Tránsito/cap hover = desglose a bordo.
7. Paridad OFRN: músico con `plaza_extra` cuenta 2 plazas. En FIMBA cada regla de ruta es un hop (contar ↑/↓ en **cada** parada asignada); Giras Trayectos sigue colapsando a primera/última.
...
12. **Tope Sube en parada (2026-09-02 / 2026-09-09):** disp. = plazas ya asignadas como ↑ **en esta parada de este vehículo**, no Σ rides abiertos de toda la edición ni through-traffic de un hop anterior. El mismo artista puede subir/bajar muchas veces en la misma unidad y en otras. Evento **4363** (15/09 10:30 Toyota Aeropuerto→Hotel): el ride de Alba en Chevrolet (↑ 3867 → ↓ 3911) **no** deja disp. 0. Evento **3910** (15/09 Chevrolet AF599YN, sin horas propias): el 09:50 cian era `hora_inicio` del evento **3867** (Toyota AB808YX, endpoint de otra ruta). Fin del tramo ya no toma endpoints ajenos ni `hora_fin` huérfana. Reserva `plazas=0` no figura como regla activa.
...
**Carga (`FimbaTransportPage`):** spinner full-page solo en la **primera** visita. `listFimbaTraslados` / `listFimbaPropuestaRutas` aceptan `edicion` + `propuestas` (+ `flota`) cacheados para no re-fetch. Participantes CNRT = batch liviano (`listFimbaParticipantesForPropuestas`, sin habitaciones) en background. Tras editar: refresh quirúrgico — rutas (↑↓ FIMBA), eventos+rutas (reserva/modal evento), logistics (Orquesta OFRN); la planilla permanece visible. **Filtro Artista (2026-09-05):** client-side (paridad Agenda) con `eventMatchesPropuestaRouteFilter` + `propuestaRoutes` + `sequencesByVehicle` — **no** refetch de trayectos/logistics al cambiar el select. `useDeferredValue` mantiene el header usable; mientras el recompute de planilla está pendiente: select `disabled` + «Filtrando…» inline + overlay local sobre la planilla (`.fimba-planilla-filter-busy`), sin blankear la página.
...
| `src/views/Fimba/FimbaComidasPage.jsx` | Comidas: pestañas Gestor / Asistencia / Reporte; `mealFilters` compartidos; MealsManager `fimbaMode` |
| `src/views/Fimba/FimbaMealCoveragePanel.jsx` | Alertas cobertura A/M/C + Crear automáticamente / Crear todas (montado en Gestor) |
| `src/views/Giras/MealsManager.jsx` | Matriz comidas OFRN (+ Catering + H. Fin + deducción grupo↔orquesta + modo FIMBA + cobertura A/M/C) |
| `src/views/Giras/MealsReport.jsx` | Reporte cuadro + Texto + Por artista; aviso cobertura → Gestor |
| `src/views/Giras/MealsAttendance.jsx` | Asistencia por evento (reusado en FIMBA Comidas) |
| `src/utils/mealLogistics.js` | meal-related / cobertura gaps (estadía o tags) / `resolveEventHoraFinForSave` |
| `src/utils/fimbaMealsStay.js` | Noches/comidas Hotelería + `amcFlagsForStayDay` / slots A/M/C cobertura |
| `src/utils/fimbaMealCoverageCreate.js` | Auto-create huecos A/M/C (defaults hora, GRP:NONE, sin hora_fin) |
| `src/utils/mealsReportByArtistExport.js` | Excel multi-hoja + ZIP textos por artista |
| `src/utils/mealsReportText.js` | Texto pedido compartido (reporte / export) |
...
Los deploys en Vercel **no** deben forzar reload mid-form. Banner «Nueva versión — Actualizar versión» (un tap: skipWaiting + controllerchange, o last resort unregister/caches); `FimbaEventoFormModal` dirty y planillas (`.fimba-row-dirty`) bloquean auto-apply. Detalle: `docs/specs/pwa-version-updates.md`.
