...
Equivalentes de lectura: `?artistas=5,7&grupo=Alba` · entry `/fimba/c/:token/agenda?…` → `FimbaEdicionConsultaEntry` valida token, persiste `localStorage.fimba_consulta_edicion`, redirige a `/fimba/edicion/:id/agenda?…` en shell RO (sin create/edit/delete).
...
8. **Multi-filtro + enlace:** en dropdowns **Artista** y **Grupos OFRN**, tildar varios (checkboxes) → URL `?propuestas=5,7&grupos=3&origen=all`; **Copiar enlace** y abrir en otra pestaña (o `/fimba/c/{token}/agenda?…`) → misma vista (unión OR: eventos tagged + convocatoria grupo + rides de artistas).
