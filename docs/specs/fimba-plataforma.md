...
- **Agenda de artista** (`id_propuesta` en `listFimbaAgenda` / ficha `FimbaConsultaAgenda` / filtro artista en planilla):
  ...
  - **Feature 2026-09-07 — Crear movimientos intermedios (kebab fila):** opción en menú ⋮ de planilla Transportes. Abre el mismo `FimbaRecorridoIntermedioModal` con `variant: 'aroundEvent'`. Defaults: salida = locación de la parada anterior del vehículo a **inicio − 30′**; medio = evento ancla (solo lectura, no se duplica); retorno = misma locación anterior a **fin + 30′** (`resolveHoraFinDisplay` / `hora_fin`; si no hay fin → asume +60′ desde inicio luego +30′). Locación/fecha/hora de salida y retorno editables. Persiste con `createMovimientosIntermediosAroundEvent` (2 paradas vía `createDestinoStopEvent`). Deshabilitado si no hay prev / sin locación / misma locación que prev (pausa → usar recorrido intermedio). Aviso si ya hay paradas entre prev y ancla. Helpers `buildMovimientosIntermediosDefaults` / `createMovimientosIntermediosAroundEvent`.
