...
- [x] **Menores no firman (2026-09-25, corrección 2026-09-25):** menor de firmas = instrumento **Menor** (`integrantes.id_instr` → `instrumentos.instrumento`, label exacto «Menor», case-insensitive). La persona **sigue en la hoja** (nombre + DNI) pero **sin recuadro ni imagen de firma**. Etiqueta gris «Menor» en el lugar de la firma. Quien tenga menos de 18 años pero **no** sea instrumento Menor **sí firma**. CNRT / lista de pasajeros (`transportExport.js`, columna MENOR) **sigue** usando `fecha_nac` edad &lt; 18; no comparte helper con el cuadro. Si falta el join, se resuelve el nombre por `id_instr` en `instrumentos` (respeta override de gira) y, si no hay ID, se rehidrata desde `integrantes`. Sin nombre no se asume menor. Familia sale de `instrumentos`; IDs de integrantes son numéricos.
