...
- La persistencia se realiza mediante un botón de 'Guardar' en la misma fila de draft, similar a la lógica de `RepertoireManager`. ✅ Botón **"Guardar encargo"** / **"Asignar a..."** (admin) o **"Guardar"** (arreglador self) inserta en `public.obras` (estado `"Para arreglar"`) y en `public.obras_compositores` (compositor + **arreglador** del integrante asignado vía `syncObraArregladorFromIntegrante`).
