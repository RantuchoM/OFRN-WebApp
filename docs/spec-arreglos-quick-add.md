...
- **Referencias de material:** Tabla `public.arreglos_referencias` (`id_obra`, `titulo`, `id_obra_referencia` opcional, `link` opcional; al menos uno de obra o link). Columna **Ref.** en `ArreglosDashboard` y botón carpeta en header de `WorkForm` (estados Solicitud / Para arreglar / Entregado) abren `ArreglosReferenciasModal` (portal `z-[100]`). Obra del archivo: `RepertoireWorkPickerModal`. Al crear **Nuevo arreglo** desde `WorkForm`, se inserta automáticamente la obra origen (`seedArregloReferenciaObraOrigen`).
