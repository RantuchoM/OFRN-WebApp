...
- **Referencias de material:** Tabla `public.arreglos_referencias` (`id_obra`, `titulo`, `id_obra_referencia` opcional, `link` opcional; al menos uno de obra o link). Columna **Ref.** abre `ArreglosReferenciasModal` (portal `z-[100]`) con íconos de carpeta para abrir enlaces; editores pueden agregar/quitar referencias. Obra del archivo: `RepertoireWorkPickerModal` («Buscar obra»). Al crear **Nuevo arreglo** desde `WorkForm`, se inserta automáticamente la obra origen como primera referencia (`seedArregloReferenciaObraOrigen`).
